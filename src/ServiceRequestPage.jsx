...
{label:'غرامة تأخير التجديد',amount:lateFine},
...
...(fine>0?[{label:'غرامة تأخير التجديد',amount:fine}]:[]),
...
}else if(selSvc==='exit_reentry_visa'){
if(!fields.exit_type){setErr(T('يرجى اختيار نوع التأشيرة','Please select the visa type'));return false}
// عند التمديد: رقم التأشيرة إلزامي ويجب أن يكون 9 أرقام بالضبط — يبقى الزر معطّلاً دون تلوين الحقل.
if(fields.op_mode==='extend'&&(fields.visa_number||'').length!==9){setErr(T('رقم التأشيرة يجب أن يكون 9 أرقام','Visa number must be 9 digits'));return false}
if(!fields.duration_months){setErr(T('يرجى اختيار المدة','Please select the duration'));return false}
}else{
...
const items=arr.filter(l=>l&&typeof l.label==='string'&&(Number(l.amount)||0)>0).map(l=>({label:l.label,amount:Number(l.amount)||0,...(l.discount?{discount:true}:{})}))
...
<div className='branch-grid' style={{flex:1,minHeight:0,display:'grid',gridTemplateColumns:`repeat(${cols},1fr)`,gridTemplateRows:`repeat(${rows},minmax(0,1fr))`,gap:8}}>
...
const boxes=<div className='cli-boxes' style={{display:'flex',gap:8,flexShrink:0}}>{c.id_number&&infoBox(CreditCard,T('رقم الهوية','ID number'),c.id_number)}{c.phone&&infoBox(Phone,T('الجوال','Phone'),fmtPhone(c.phone))}</div>
...
<label style={{...lblS,marginBottom:0}}>{T('رقم التأشيرة','Visa No')}</label>
<input value={fields.visa_number||''} maxLength={9} inputMode="numeric"
onChange={e=>setFields(p=>({...p,visa_number:e.target.value.replace(/\D/g,'').slice(0,9)}))}
...
{!selKafalaQuote&&<div className="wz-search-row" style={{position:'relative'}}>
<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--tx4)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{position:'absolute',top:'50%',left:14,transform:'translateY(-50%)',pointerEvents:'none',transition:'stroke .2s'}}><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
<input value={kafalaQuoteQ} onChange={e=>setKafalaQuoteQ(e.target.value)} placeholder={isMob?T('اسم العامل، الإقامة أو رقم الحسبة','Worker, Iqama or quote no'):QL.search} onFocus={e=>{e.currentTarget.previousElementSibling.style.stroke=C.bentoGold}} onBlur={e=>{e.currentTarget.previousElementSibling.style.stroke='var(--tx4)'}} style={{...fkSF,padding:'0 14px 0 40px',textAlign:isAr?'right':'left',border:'1px solid transparent',boxShadow:'none'}}/>
</div>}
...
:matches.length>0?matches.map(qt=>isMob?<WzRow key={qt.id} onClick={()=>pickQuote(qt)} chevron
lead={<span className="wz-flag" style={{width:44,height:44}}><Receipt size={20} strokeWidth={1.8}/></span>} title={qt.worker_name||'—'}
meta={[qt.iqama_number&&{icon:<CreditCard size={13} strokeWidth={1.9}/>,text:qt.iqama_number,ltr:true},qt.quote_no&&{icon:<Hash size={13} strokeWidth={1.9}/>,text:noDash(qt.quote_no),ltr:true}]}
trailing={<span className="wz-amt"><bdi>{fmtPrice(qt.client_charge)}</bdi><small>{T('ريال','SAR')}</small></span>}/>:(
...
{showBroker&&<div className='wz-broker' style={{border:'1.5px solid rgba(176,125,0,.35)',borderRadius:12,padding:brokerOpen?'12px 14px 14px':'12px 14px',marginTop:8,display:'flex',flexDirection:'column',gap:0,transition:'padding .34s cubic-bezier(.4,0,.2,1)'}}>
{/* Header row — in-flow (no negative top offset) so the toggle is never clipped by the modal's top edge */}
<div className='wz-broker-head' style={{display:'flex',alignItems:'center',gap:7,fontFamily:F}}>
...
{!brokerOpen&&<div className='wz-broker-hint' style={{fontSize:11,color:'var(--tx5)',fontFamily:F,textAlign:'center',padding:'6px 0 0',transition:'opacity .25s'}}>{isMob?T('اضغط + لإضافة وسيط للفاتورة','Tap + to add an agent'):T('اضغط على + لإضافة وسيط','Click + to add an agent')}</div>}
...
{!selBroker&&brokerMode!=='new'&&<div className="wz-search-row" style={{display:'flex',alignItems:'stretch',gap:8,flexShrink:0}}>
<div style={{position:'relative',flex:1,minWidth:0}}>
<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--tx4)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{position:'absolute',top:'50%',left:14,transform:'translateY(-50%)',pointerEvents:'none',transition:'stroke .2s'}}><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
...
const boxes=<div className='cli-boxes' style={{display:'flex',gap:8,flexShrink:0}}>{b.id_number&&infoBox(CreditCard,T('رقم الهوية','ID number'),b.id_number)}{b.phone&&infoBox(Phone,T('الجوال','Phone'),fmtPhone(b.phone))}</div>
// flexShrink:0 — البطاقات داخل منطقة تمرير ذات ارتفاع ثابت: بلا هذا تنضغط بدل أن تُمرَّر.
const wrapSel={position:'relative',border:`1px solid ${G.selB}`,background:G.sel,boxShadow:'var(--shadow-md)',transition:'all .22s ease',padding:'11px',borderRadius:14,display:'flex',flexDirection:'column',gap:9,flexShrink:0}
if(isMob)return<WzRow key={b.id} selected={sel} onClick={sel?undefined:()=>setSelBroker(b)}
lead={<WzFlag url={flagUrl} label={natLabel} name={b.name_ar||b.name_en}/>} title={b.name_ar||b.name_en||'—'} sub={b.name_ar&&b.name_en?b.name_en:null}
meta={[b.id_number&&{icon:<CreditCard size={13} strokeWidth={1.9}/>,text:b.id_number,ltr:true},b.phone&&{icon:<Phone size={13} strokeWidth={1.9}/>,text:fmtPhone(b.phone),ltr:true}]}
trailing={sel?<button type="button" className="wz-change" onClick={deselect}>{T('تغيير','Change')}</button>:null} chevron={!sel}/>
