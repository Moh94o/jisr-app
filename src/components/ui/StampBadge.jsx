...
export default function StampBadge({children,color='#B07D00',rotate=-7,fontSize=13,letterSpacing=3,padding='7px 20px'}){
  return<>
    <span className="stamp-badge" style={{position:'relative',display:'inline-block',padding,color,fontWeight:600,fontSize,letterSpacing:`${letterSpacing}px`,fontFamily:"'Cairo','Tajawal',sans-serif",transform:`rotate(${rotate}deg)`,opacity:.92,background:'transparent','--stamp-color':color,whiteSpace:'nowrap',lineHeight:1.15}}>
...
.stamp-badge::before{
  top:-2px;right:-2px;
  border-top:2px solid var(--stamp-color,#B07D00);
  border-right:2px solid var(--stamp-color,#B07D00);
}
.stamp-badge::after{
  bottom:-2px;left:-2px;
  border-bottom:2px solid var(--stamp-color,#B07D00);
  border-left:2px solid var(--stamp-color,#B07D00);
}
