...
const C = { gold: '#B07D00', red: '#c0392b', ok: '#27a046', blue: '#3483b4', warn: '#eab308' }
...
const CATS = [
  { code: 'sbc',     ar: 'المركز السعودي',   en: 'Saudi Business Center', c: '#B07D00', Icon: Landmark },
...
const inpS = { width: '100%', minWidth: 0, borderRadius: 8, color: C.gold, fontFamily: F, fontSize: 20, fontWeight: 600, textAlign: 'center', outline: 'none', padding: '3px 2px', marginTop: 1, letterSpacing: '-.5px', fontVariantNumeric: 'tabular-nums', direction: 'ltr', lineHeight: 1, whiteSpace: 'nowrap', boxSizing: 'border-box' }
...
const EditTab = ({ onClick }) => (
  <button type="button" onClick={onClick} title="تعديل" style={{ position: 'absolute', top: -11, insetInlineEnd: 14, height: 22, padding: '0 10px', borderRadius: 6, border: `1px dashed ${C.gold}`, background: '#1a1a1a', color: C.gold, fontFamily: F, fontSize: 10, fontWeight: 600, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 5, transition: '.15s', zIndex: 2 }}
...
const EditActionTabs = ({ onSave, onCancel, saving }) => (
  <div style={{ position: 'absolute', top: -11, insetInlineEnd: 14, display: 'inline-flex', alignItems: 'center', gap: 6, zIndex: 2 }}>
    <button type="button" onClick={onCancel} style={{ height: 22, padding: '0 10px', borderRadius: 6, border: '1px solid rgba(255,255,255,.2)', background: '#1a1a1a', color: 'rgba(255,255,255,.75)', fontFamily: F, fontSize: 10, fontWeight: 600, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 5 }}>
      <span>إلغاء</span>
    ...
    <button type="button" onClick={onSave} disabled={saving} style={{ height: 22, padding: '0 12px', borderRadius: 6, border: `1px solid ${C.ok}`, background: '#1a1a1a', color: C.ok, fontFamily: F, fontSize: 10, fontWeight: 600, cursor: saving ? 'wait' : 'pointer', display: 'inline-flex', alignItems: 'center', gap: 5 }}>
    ...
    {options.map(o => {
      ...
      return (
        <button key={o.v} type="button" disabled={disabled} onClick={() => onChange(o.v)}
          style={{ height: 28, padding: '0 12px', borderRadius: 7, border: '1px solid ' + (sel ? (o.c || C.gold) + '66' : 'transparent'), background: sel ? (o.c || C.gold) + '1f' : 'transparent', color: sel ? (o.c || C.gold) : 'var(--tx3)', fontFamily: F, fontSize: 11, fontWeight: 600, cursor: disabled ? 'not-allowed' : 'pointer', display: 'inline-flex', alignItems: 'center', gap: 5, transition: '.15s' }}>
          {o.icon}
          {o.l}
        </button>
      )
  ...
  const renderEditor = (isNew) => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
        <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--tx3)' }}>{T('التصنيف', 'Category')}</span>
        <Seg value={draft.category || 'other'} onChange={v => setDraft(p => ({ ...p, category: v }))}
          options={CATS.map(c => ({ v: c.code, l: T(c.ar, c.en), c: c.c }))} />
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
        <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--tx3)' }}>{T('نوع الرسم', 'Fee type')}</span>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: draft.amount_type === 'fixed' ? '1fr' : '1fr 1fr', gap: 10 }}>
        {draft.amount_type === 'fixed' ? (
          <div style={tileStyle}>
            <span style={{ fontSize: 10.5, fontWeight: 600, color: 'rgba(255,255,255,.82)' }}>{T('المبلغ الثابت', 'Fixed amount')}</span>
            ...
            <span style={{ fontSize: 8.5, fontWeight: 600, color: 'var(--tx5)' }}>{T('ريال', 'SAR')}</span>
          </div>
        ) : (
          <>
            <div style={tileStyle}>
              <span style={{ fontSize: 10.5, fontWeight: 600, color: 'rgba(255,255,255,.82)' }}>{T('الحد الأعلى للإدخال', 'Max input')}</span>
              <input type="text" inputMode="decimal" value={draft.max_amount ?? ''} placeholder={T('بدون حد', 'no cap')}
                ...
                style={{ ...inpS, border: `1px solid ${C.gold}66`, background: 'rgba(0,0,0,.3)' }} />
              <span style={{ fontSize: 8.5, fontWeight: 600, color: 'var(--tx5)' }}>{T('ريال — اتركه فارغاً بلا حد', 'SAR — empty = no cap')}</span>
            </div>
            <div style={{ ...tileStyle, justifyContent: 'center', gap: 8 }}>
              <span style={{ fontSize: 10.5, fontWeight: 600, color: 'rgba(255,255,255,.82)' }}>{T('عند تجاوز الحد', 'When over the cap')}</span>
      ...
      {isNew && (
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
          <button type="button" onClick={cancelEdit} style={{ height: 34, padding: '0 14px', borderRadius: 9, border: '1px solid rgba(255,255,255,.12)', background: 'transparent', color: 'var(--tx3)', fontFamily: F, fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>{T('إلغاء', 'Cancel')}</button>
          <button type="button" onClick={saveDraft} disabled={saving} style={{ height: 34, padding: '0 18px', borderRadius: 9, border: `1px solid ${C.gold}88`, background: 'rgba(176,125,0,.1)', color: C.gold, fontFamily: F, fontSize: 12, fontWeight: 600, cursor: saving ? 'wait' : 'pointer' }}>{saving ? T('جارٍ الحفظ…', 'Saving…') : T('إضافة الرسم', 'Add fee')}</button>
        </div>
      )}
    </div>
  )
  ...
  const renderFeeRow = (r, accent) => {
    ...
    return (
      <div key={r.id} style={{ ...feeRowStyle, borderInlineEnd: `5px solid ${accent}`, opacity: r.is_active ? 1 : .55 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
          <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--tx)', letterSpacing: '-.2px' }}>{isAr ? r.label_ar : (r.label_en || r.label_ar)}</span>
          <span style={{ fontSize: 9.5, fontWeight: 600, color: 'var(--tx5)', fontFamily: 'monospace', direction: 'ltr', padding: '2px 8px', borderRadius: 5, background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.06)' }}>{r.code}</span>
          <span style={{ fontSize: 10, fontWeight: 600, padding: '3px 10px', borderRadius: 999, color: variable ? C.blue : C.gold, background: (variable ? 'rgba(52,131,180,.12)' : 'rgba(176,125,0,.1)'), border: '1px solid ' + (variable ? 'rgba(52,131,180,.35)' : 'rgba(176,125,0,.3)') }}>
          ...
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            {!variable && (
              <div style={{ ...tileStyle, minWidth: 170, flex: '0 1 220px' }}>
                ...
                <span style={{ fontSize: 8.5, fontWeight: 600, color: 'var(--tx5)' }}>{T('ريال — يُرسل مباشرة بدون إدخال', 'SAR — sent directly, no input')}</span>
              </div>
            )}
            {variable && (
              <>
                <div style={{ ...tileStyle, minWidth: 170, flex: '0 1 220px' }}>
                  ...
                  <span style={{ fontSize: 8.5, fontWeight: 600, color: 'var(--tx5)' }}>{r.max_amount != null ? T('ريال', 'SAR') : T('بلا حد', 'no cap')}</span>
                </div>
                {r.max_amount != null && (
                  <div style={{ ...tileStyle, minWidth: 200, flex: '0 1 260px', gap: 6 }}>
                    <span style={{ fontSize: 10.5, fontWeight: 600, color: 'rgba(255,255,255,.82)' }}>{T('عند تجاوز الحد', 'Over the cap')}</span>
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12, fontWeight: 600, color: r.over_max_action === 'review' ? C.warn : C.red }}>
            ...
            return (
              <div key={cat.code} style={{ ...GLASS_CARD, padding: 18, display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, paddingBottom: 12, borderBottom: '1px solid rgba(255,255,255,.06)' }}>
                  ...
                  <span style={{ fontSize: 16, fontWeight: 600, color: cat.c, letterSpacing: '-.2px' }}>{T(cat.ar, cat.en)}</span>
                  <span style={{ fontSize: 10.5, fontWeight: 600, padding: '2px 10px', borderRadius: 999, color: catRows.length ? cat.c : 'var(--tx5)', background: catRows.length ? cat.c + '14' : 'rgba(255,255,255,.04)', border: '1px solid ' + (catRows.length ? cat.c + '40' : 'rgba(255,255,255,.08)'), fontVariantNumeric: 'tabular-nums' }}>
                ...
                {can(user, 'admin_fees.create') && (adding ? (
                  <div style={{ ...feeRowStyle, borderInlineEnd: `5px solid ${cat.c}` }}>
                    <div style={{ fontSize: 13, fontWeight: 600, color: cat.c, display: 'flex', alignItems: 'center', gap: 7 }}>
                  ...
                  <button type="button" onClick={() => startNew(cat.code)}
                    style={{ height: 40, borderRadius: 10, background: 'transparent', border: '1px dashed ' + cat.c + '55', color: cat.c, cursor: 'pointer', fontFamily: F, fontSize: 12, fontWeight: 600, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 7, transition: '.15s' }}
