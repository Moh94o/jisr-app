...
export default function SyncHub({ sb, toast, user, lang }) {
  ...
  return (
    <div style={{ fontFamily: F, paddingTop: 16 }}>
      ...
      {personMgr && (
        <PersonManager
          sb={sb} toast={toast} lang={lang}
          persons={persons}
          onClose={() => setPersonMgr(false)}
          onChanged={() => { setPersonMgr(false); setRefreshTick(t => t + 1) }}
          T={T}
        />
      )}
      ...
      {syncModal && (
        <SyncInstallModal
          ...
          personId={syncModal.personId}
          personName={personName(syncModal.personId)}
  ...
  return (
    <div style={{ marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16, flexWrap: 'wrap' }}>
      <div style={{ flex: 1, minWidth: 240 }}>
        ...
        <div style={{ fontSize: 12, color: 'rgba(255,255,255,.5)', marginTop: 6, lineHeight: 1.7 }}>
          {description}
        ...
        {!focusedSource && (
          <div style={{ fontSize: 10.5, color: 'rgba(255,255,255,.4)', marginTop: 4 }}>
...
function PersonTabs({ persons, active, onChange, onManage, T }) {
  // Underline-style tabs that match the OTP Messages design:
  // bottom-border container, active tab draws a 2px gold underline flush to the border,
  // settings gear sits on the far left in the same row.
  return (
    <div style={{ display: 'flex', borderBottom: '1px solid rgba(255,255,255,.15)', marginBottom: 18, justifyContent: 'space-between', alignItems: 'stretch', gap: 8 }}>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 0 }}>
        {[{ id: 'all', name_ar: T('الكل', 'All') }, ...persons].map(p => {
          const isActive = active === p.id
          return (
            <div key={p.id} onClick={() => onChange(p.id)} style={{ padding: '10px 22px 9px', cursor: 'pointer', color: isActive ? C.gold : 'rgba(255,255,255,.5)', fontSize: 14, fontWeight: isActive ? 800 : 600, borderBottom: isActive ? `2px solid ${C.gold}` : '2px solid transparent', marginBottom: -1, transition: '.15s', fontFamily: F }}>
              {p.name_ar}
            </div>
          )
        })}
      </div>
      <button onClick={onManage} title={T('إدارة الأشخاص', 'Manage people')} style={{ alignSelf: 'center', width: 34, height: 34, borderRadius: 8, border: '1px solid rgba(212,160,23,.3)', background: 'rgba(212,160,23,.06)', color: C.gold, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          ...
          <div key={s.id} style={{ padding: 18, borderRadius: 14, background: bg, border: `1px solid ${accent}22`, opacity: available ? 1 : 0.55, transition: 'transform .18s, box-shadow .18s', cursor: available ? 'pointer' : 'default', position: 'relative', overflow: 'hidden' }}
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: 14 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 10, color: 'rgba(255,255,255,.5)', marginTop: 3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.description_ar || s.name_en}</div>
              </div>
              {!available && <span style={{ fontSize: 9, padding: '3px 8px', borderRadius: 5, background: 'rgba(255,255,255,.06)', color: 'rgba(255,255,255,.55)', fontWeight: 700, whiteSpace: 'nowrap', flexShrink: 0 }}>{T('قريباً', 'Soon')}</span>}
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, marginBottom: 10 }}>
              ...
              <span style={{ fontSize: 11, color: 'rgba(255,255,255,.5)', fontWeight: 600 }}>{T('سجل', 'records')}</span>
            </div>
            ...
            {/* Last sync line */}
            <div style={{ fontSize: 10.5, color: 'rgba(255,255,255,.55)', marginBottom: 14, display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            {available && (
              <div style={{ display: 'flex', gap: 6 }}>
                <button onClick={e => { e.stopPropagation(); onOpen(s.id) }} style={{ flex: 1, padding: '8px 10px', border: '1px solid rgba(255,255,255,.1)', background: 'rgba(255,255,255,.03)', color: 'rgba(255,255,255,.85)', borderRadius: 8, cursor: 'pointer', fontSize: 11.5, fontWeight: 700, fontFamily: F, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 5, transition: '.15s' }}
...
// ═══════════════════════════════════════════════════════════════
function ChangeTimeline({ changes, personName, sourceName, sourceAccent, T, lang }) {
  ...
  if (!changes.length) {
    return (
      <div style={{ padding: 20, borderRadius: 12, background: 'rgba(255,255,255,.02)', border: '1px dashed rgba(255,255,255,.08)', textAlign: 'center', fontSize: 12, color: 'rgba(255,255,255,.5)' }}>
    ...
    <div style={{ padding: 16, borderRadius: 12, background: 'rgba(255,255,255,.02)', border: '1px solid rgba(255,255,255,.06)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14, flexWrap: 'wrap', gap: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          {[['all', T('الكل', 'All')], ['added', T('مُضاف', 'Added')], ['modified', T('مُعدّل', 'Modified')], ['removed', T('محذوف', 'Removed')]].map(([k, l]) => {
            ...
            return (
              <button key={k} onClick={() => setTypeFilter(k)} style={{ padding: '4px 10px', border: '1px solid ' + (on ? 'rgba(212,160,23,.4)' : 'rgba(255,255,255,.08)'), background: on ? 'rgba(212,160,23,.12)' : 'transparent', color: on ? C.gold : 'rgba(255,255,255,.6)', borderRadius: 6, cursor: 'pointer', fontSize: 10.5, fontWeight: 700, fontFamily: F }}>{l}</button>
            )
          })}
        </div>
      </div>
          ...
          return (
            <div key={c.id} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '10px 4px', borderBottom: i < list.length - 1 ? '1px solid rgba(255,255,255,.04)' : 'none' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 10.5, color: 'rgba(255,255,255,.45)', marginTop: 3, display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                  <span style={{ color: srcClr, fontWeight: 700 }}>{sourceName(c.source_id)}</span>
                  <span>{personName(c.person_id)}</span>
      ...
      {changes.length > limit && (
        <div style={{ textAlign: 'center', marginTop: 10 }}>
          <button onClick={() => setLimit(l => l + 30)} style={{ padding: '6px 14px', border: '1px solid rgba(255,255,255,.08)', background: 'transparent', color: 'rgba(255,255,255,.6)', borderRadius: 7, cursor: 'pointer', fontSize: 11, fontWeight: 700, fontFamily: F }}>{T('عرض المزيد', 'Show more')}</button>
        </div>
      )}
    </div>
  )
...
function changeLabel(c, T) {
  ...
  return (
    <>
      ...
      <span>{field}</span>{' '}
      <span style={{ color: 'rgba(255,255,255,.5)' }}>{T('من', 'from')}</span>{' '}
      <span style={{ color: '#ef4444' }}>{oldV}</span>{' '}
      <span style={{ color: 'rgba(255,255,255,.5)' }}>{T('إلى', 'to')}</span>{' '}
      ...
      {filteredChanges.length > 0 && (
        <div style={{ marginTop: 20, padding: 16, borderRadius: 12, background: 'rgba(255,255,255,.02)', border: '1px solid rgba(255,255,255,.06)' }}>
          ...
          {filteredChanges.slice(0, 20).map((c, i) => (
            <div key={c.id} style={{ fontSize: 11.5, padding: '6px 0', borderBottom: i < Math.min(filteredChanges.length, 20) - 1 ? '1px solid rgba(255,255,255,.04)' : 'none', color: 'rgba(255,255,255,.8)' }}>
              {changeLabel(c, T)} <span style={{ color: 'rgba(255,255,255,.4)', fontSize: 10 }}>· {personName(c.person_id)} · {fmtRelative(c.detected_at, lang)}</span>
            </div>
          ))}
        </div>
      )}
  ...
  return (
    <div>
      ...
      <div style={{ padding: 40, borderRadius: 14, background: 'rgba(255,255,255,.02)', border: '1px dashed rgba(255,255,255,.08)', textAlign: 'center' }}>
        ...
        <div style={{ fontSize: 12, color: 'rgba(255,255,255,.55)' }}>{T('قيد التطوير — سيتم تفعيل المزامنة قريباً.', 'Under development — sync will be enabled soon.')}</div>
      </div>
    </div>
  )
}
...
// ═══════════════════════════════════════════════════════════════
function PersonManager({ sb, toast, lang, persons, onClose, onChanged, T }) {
  const [form, setForm] = useState({ name_ar: '', name_en: '' })
  const [editingId, setEditingId] = useState(null)
  const [saving, setSaving] = useState(false)

  const save = async () => {
    if (!form.name_ar.trim()) return toast?.(T('اسم عربي مطلوب', 'Arabic name required'))
    setSaving(true)
    try {
      if (editingId) {
        await sb.from('sync_persons').update({ name_ar: form.name_ar, name_en: form.name_en || null }).eq('id', editingId)
      } else {
        const maxOrder = (persons[persons.length - 1]?.sort_order || 0) + 10
        await sb.from('sync_persons').insert({ name_ar: form.name_ar, name_en: form.name_en || null, sort_order: maxOrder })
      }
      setForm({ name_ar: '', name_en: '' }); setEditingId(null)
      onChanged()
  ...
  const remove = async (id) => {
    if (!confirm(T('حذف هذا الشخص؟', 'Delete this person?'))) return
    await sb.from('sync_persons').update({ deleted_at: new Date().toISOString() }).eq('id', id)
    onChanged()
  }

  const startEdit = (p) => { setEditingId(p.id); setForm({ name_ar: p.name_ar, name_en: p.name_en || '' }) }
  const cancelEdit = () => { setEditingId(null); setForm({ name_ar: '', name_en: '' }) }
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.7)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div onClick={e => e.stopPropagation()} style={{ width: '100%', maxWidth: 520, maxHeight: '88vh', overflowY: 'auto', background: '#1a1a1a', borderRadius: 14, border: '1px solid rgba(212,160,23,.25)', padding: 22 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <div style={{ fontSize: 17, fontWeight: 800, color: C.gold }}>{T('إدارة الأشخاص', 'Manage People')}</div>
          <button onClick={onClose} style={{ padding: '6px 12px', border: '1px solid rgba(255,255,255,.1)', background: 'rgba(255,255,255,.04)', color: 'var(--tx)', borderRadius: 7, cursor: 'pointer', fontSize: 11, fontWeight: 700, fontFamily: F }}>{T('إغلاق', 'Close')}</button>
        </div>

        <div style={{ padding: 14, borderRadius: 10, background: 'rgba(212,160,23,.04)', border: '1px solid rgba(212,160,23,.15)', marginBottom: 14 }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: C.gold, marginBottom: 10 }}>{editingId ? T('تعديل شخص', 'Edit person') : T('إضافة شخص', 'Add person')}</div>
          <div style={{ display: 'flex', gap: 8 }}>
            <input value={form.name_ar} onChange={e => setForm(f => ({ ...f, name_ar: e.target.value }))} placeholder={T('الاسم بالعربي', 'Arabic name')} style={{ flex: 1, padding: '9px 12px', borderRadius: 8, background: 'rgba(0,0,0,.3)', border: '1px solid rgba(255,255,255,.1)', color: 'var(--tx)', fontFamily: F, fontSize: 12, outline: 'none' }} />
            <input value={form.name_en} onChange={e => setForm(f => ({ ...f, name_en: e.target.value }))} placeholder={T('بالإنجليزي (اختياري)', 'English (optional)')} style={{ flex: 1, padding: '9px 12px', borderRadius: 8, background: 'rgba(0,0,0,.3)', border: '1px solid rgba(255,255,255,.1)', color: 'var(--tx)', fontFamily: F, fontSize: 12, outline: 'none', direction: 'ltr' }} />
          </div>
          <div style={{ display: 'flex', gap: 6, marginTop: 10 }}>
            <button onClick={save} disabled={saving} style={{ padding: '8px 16px', border: 'none', background: C.gold, color: '#1a1a1a', borderRadius: 7, cursor: 'pointer', fontSize: 12, fontWeight: 800, fontFamily: F, opacity: saving ? .6 : 1 }}>{saving ? T('جارٍ الحفظ…', 'Saving…') : (editingId ? T('تحديث', 'Update') : T('إضافة', 'Add'))}</button>
            {editingId && <button onClick={cancelEdit} style={{ padding: '8px 12px', border: '1px solid rgba(255,255,255,.1)', background: 'transparent', color: 'rgba(255,255,255,.7)', borderRadius: 7, cursor: 'pointer', fontSize: 11, fontWeight: 700, fontFamily: F }}>{T('إلغاء', 'Cancel')}</button>}
          </div>
        </div>

        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {persons.map(p => (
            <div key={p.id} style={{ padding: '10px 14px', borderRadius: 9, background: 'rgba(255,255,255,.03)', border: '1px solid rgba(255,255,255,.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10 }}>
              <div>
                <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--tx)' }}>{p.name_ar}</div>
                {p.name_en && <div style={{ fontSize: 10, color: 'rgba(255,255,255,.5)', direction: 'ltr', marginTop: 2 }}>{p.name_en}</div>}
              </div>
              <div style={{ display: 'flex', gap: 6 }}>
                <button onClick={() => startEdit(p)} style={{ padding: '5px 10px', border: '1px solid rgba(255,255,255,.08)', background: 'transparent', color: 'rgba(255,255,255,.7)', borderRadius: 6, cursor: 'pointer', fontSize: 10.5, fontWeight: 700, fontFamily: F }}>{T('تعديل', 'Edit')}</button>
                <button onClick={() => remove(p.id)} style={{ padding: '5px 10px', border: '1px solid rgba(192,57,43,.25)', background: 'rgba(192,57,43,.08)', color: C.red, borderRadius: 6, cursor: 'pointer', fontSize: 10.5, fontWeight: 700, fontFamily: F }}>{T('حذف', 'Delete')}</button>
              </div>
            </div>
          ))}
        </div>
...
// ═══════════════════════════════════════════════════════════════
// Sync Install Modal — renders a per-person bookmarklet
// ═══════════════════════════════════════════════════════════════
function SyncInstallModal({ sourceId, personId, personName, onClose, T }) {
  // Each source has its own bookmarklet builder + portal.
  const isQiwa = sourceId === 'qiwa'
  const accent = isQiwa ? C.ok : C.gold
  const portalUrl = isQiwa ? 'https://qiwa.sa' : 'https://www.saudibusiness.gov.sa'
  const portalLabel = isQiwa ? 'qiwa.sa' : 'saudibusiness.gov.sa'
  const bookmarklet = useMemo(() => (isQiwa ? buildQiwaBookmarklet({ sourceId, personId }) : buildBookmarklet({ sourceId, personId })), [isQiwa, sourceId, personId])
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.7)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div onClick={e => e.stopPropagation()} style={{ width: '100%', maxWidth: 620, maxHeight: '90vh', overflowY: 'auto', background: '#1a1a1a', borderRadius: 14, border: `1px solid ${accent}40`, padding: 24 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
          <div style={{ fontSize: 17, fontWeight: 800, color: accent }}>{T('مزامنة', 'Sync')} — {personName}</div>
          <button onClick={onClose} style={{ padding: '6px 12px', border: '1px solid rgba(255,255,255,.1)', background: 'rgba(255,255,255,.04)', color: 'var(--tx)', borderRadius: 7, cursor: 'pointer', fontSize: 11, fontWeight: 700, fontFamily: F }}>{T('إغلاق', 'Close')}</button>
        </div>
        <div style={{ fontSize: 12, color: 'rgba(255,255,255,.7)', lineHeight: 1.8, marginBottom: 14 }}>
          {T('هذه الإشارة المرجعية تربط المزامنة بـ', 'This bookmark links the sync to')} <b style={{ color: accent }}>{personName}</b>{T('. كل السجلات المستوردة ستُسجَّل تحت اسمها مع وقت الاستيراد والفوارق.', '. All imported records will be recorded under their name with timestamps and diffs.')}
        </div>
        <div style={{ padding: 14, borderRadius: 10, background: `${accent}10`, border: `1px solid ${accent}33`, marginBottom: 14 }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: accent, marginBottom: 10 }}>{T('الخطوة 1: ثبّت الإشارة', 'Step 1: Install')}</div>
          <ol style={{ fontSize: 11.5, color: 'rgba(255,255,255,.75)', lineHeight: 1.8, margin: 0, paddingInlineStart: 20 }}>
            <li>Ctrl+Shift+B {T('لإظهار شريط الإشارات', 'to show bookmark bar')}</li>
            <li>{T('اسحب الزر إلى الشريط', 'Drag the button to the bar')}</li>
          </ol>
          <div style={{ marginTop: 12, display: 'flex', justifyContent: 'center' }}>
            <BookmarkletLink href={bookmarklet} style={{ background: accent }}>
              ...
              {isQiwa ? T('جسر قوى — ', 'Jisr Qiwa — ') : T('مزامنة جسر — ', 'Jisr Sync — ')}{personName}
            </BookmarkletLink>
          </div>
        </div>
        <div style={{ padding: 14, borderRadius: 10, background: 'rgba(52,131,180,.06)', border: '1px solid rgba(52,131,180,.2)' }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: C.blue, marginBottom: 10 }}>{T('الخطوة 2: استخدم', 'Step 2: Use')}</div>
          <ol style={{ fontSize: 11.5, color: 'rgba(255,255,255,.75)', lineHeight: 1.8, margin: 0, paddingInlineStart: 20 }}>
            <li>{T('افتح', 'Open')} <a href={portalUrl} target="_blank" rel="noreferrer" style={{ color: accent, fontWeight: 700 }}>{portalLabel}</a> {T('وسجّل الدخول بنفاذ باسم', 'and log in with Nafath as')} <b>{personName}</b></li>
            <li>{isQiwa
              ? T('اضغط الإشارة من صفحة قائمة المنشآت (لقائمة كاملة) أو من لوحة تحكم منشأة (لتفاصيلها)', 'Click the bookmark on the workspaces page (full list) or a company dashboard (its details)')
              : <>{T('اضغط إشارة «مزامنة جسر — ', 'Click the "Jisr Sync — ')}{personName}» {T('المثبّتة', 'bookmark')}</>}</li>
            <li>{T('انتظر رسالة «تمت المزامنة». كل الفوارق ستُسجّل تلقائياً.', 'Wait for "Sync done". All diffs will be logged automatically.')}</li>
          </ol>
        </div>
      </div>
    </div>
