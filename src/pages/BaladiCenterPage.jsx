...
function HeroStat({ tone, label, value, footer }) {
  return (
    <div style={{ position: 'relative', padding: '18px 22px', borderRadius: 16, background: 'var(--card-grad2)', border: '1px solid var(--bd)', boxShadow: 'var(--shadow-sm)', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', overflow: 'hidden', minHeight: 150 }}>
  ...
  if (isMobile) {
    ...
    return (
      <div className="mvg" style={{ fontFamily: F }}>
        ...
        <MSearch value={q} onChange={setQ} placeholder="المرجع، المنشأة، رقم الرخصة…" />
        {dist.length > 1 && (
          <MChips value={mType} onChange={setMType} options={[{ value: '', label: 'الكل', count: filtered.length }, ...dist.map(d => ({ value: d.code, label: d.m.ar, count: filtered.filter(r => byType(r) === d.code).length }))]} />
        )}
        <MCardList rows={list.map(r => {
          const m = meta(byType(r))
    ...
    <div style={{ fontFamily: F, paddingTop: 0 }}>
      ...
      <div className="sbc-hero-grid">
        ...
        {(() => {
          ...
          let offset = 0
          return (
            <div style={{ borderRadius: 16, background: 'var(--card-grad2)', border: '1px solid var(--bd)', boxShadow: 'var(--shadow-sm)', padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 10, minHeight: 150 }}>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 18, alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        {(() => {
          ...
          return (
            <button onClick={() => setAdvOpen(o => !o)} style={{ height: 44, padding: '0 16px', borderRadius: 12, flexShrink: 0, background: active ? 'var(--accent-soft)' : 'var(--search-bg)', border: active ? '1px solid var(--accent-bd)' : '1px solid transparent', color: active ? 'var(--accent)' : 'var(--tx2)', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: F, display: 'flex', alignItems: 'center', gap: 8, boxSizing: 'border-box', transition: '.2s', boxShadow: active ? 'var(--shadow-sm)' : 'none' }}>
              تصفية
              {hasFilters ? (
...
function BaladiDetail({ sb, user, toast, row, type, statuses, onBack, onChanged }) {
  ...
  return (
    <div className="m-crd" style={{ fontFamily: F, paddingTop: 0 }}>
