...
(async () => {
  ...
  try {
    ...
    if (activeCompany && activeCompany.data) {
      ...
      if (transferReceived)        { patch.transfer_received_total         = transferReceived.data?.meta?.totalCount ?? (transferReceived.data?.requests?.length ?? 0);         patch.transfer_received_raw = transferReceived; }
      if (transferReceivedPending) { patch.transfer_received_pending_total = transferReceivedPending.data?.meta?.totalCount ?? (transferReceivedPending.data?.requests?.length ?? 0); patch.transfer_received_pending_raw = transferReceivedPending; }
      if (transferSent)            { patch.transfer_sent_total             = transferSent.data?.meta?.totalCount ?? (transferSent.data?.requests?.length ?? 0);                     patch.transfer_sent_raw = transferSent; }
      if (visaStatuses) {
      ...
      const pickReqId = (r) => String(
        r.id ?? r.requestId ?? r.request_id ?? r.referenceNumber ?? r.reference_number ??
        r.transferRequestId ?? r.transferRequestNumber ?? r.requestNumber ?? r.uuid ?? ''
      );
      // Qiwa wraps the list differently across endpoints/versions — hunt for the
      // first array among the known shapes (data.requests, requests, data.data,
      // data.items, items, or a bare top-level array).
      const pickReqList = (resp) => {
        const cands = [
          resp?.data?.requests, resp?.requests, resp?.data?.data,
          resp?.data?.items, resp?.items, resp?.data?.content, resp?.content,
          Array.isArray(resp?.data) ? resp.data : null,
          Array.isArray(resp) ? resp : null,
        ];
        return cands.find(Array.isArray) || [];
      };
      const collectTransfers = (resp, direction) => {
        const items = pickReqList(resp);
        for (const r of items) {
          const id = pickReqId(r);
          if (!id) continue;
          const emp = r.employee || r.laborer || r.worker || {};
          allTransferRequests.push({
            company_id: companyIdForWp,
            request_id: id,
            direction,
            status: r.status?.label || r.statusLabel || (typeof r.status === 'string' ? r.status : null) || null,
            status_id: r.status?.value ?? r.statusId ?? r.status_id ?? null,
            status_ar: r.status?.ar || r.statusAr || r.status?.nameAr || null,
            status_en: r.status?.en || r.statusEn || r.status?.nameEn || null,
            employee_id: String(r.employeeId ?? r.laborerId ?? r.iqamaNumber ?? r.employeeNumber ?? emp.id ?? emp.iqamaNumber ?? '') || null,
            employee_name: r.employeeName || r.laborerName || r.workerName || emp.name || emp.fullName || emp.nameAr || null,
            current_employer_name: r.currentEmployerName || r.fromEstablishmentName || r.currentEstablishmentName || r.releasingEstablishmentName || r.oldEmployerName || null,
            new_employer_name: r.newEmployerName || r.toEstablishmentName || r.requestingEstablishmentName || r.receivingEstablishmentName || null,
            release_date: r.releaseDate || r.release_date || null,
            created_at_qiwa: r.createdAt || r.created_at || r.requestDate || r.creationDate || null,
    ...
    // Resume: skip any company whose detail was synced within the last 2 hours,
    ...
    try {
      ...
      if (dr.ok) {
        ...
        if (Array.isArray(da)) {
          const doneSet = new Set(da.map(r => String(r.company_id)));
          const before = sweepList.length;
          sweepList = sweepList.filter(c => !doneSet.has(String(c.company_id)));
          skippedRecent = before - sweepList.length;
        }
      }
    } catch (e) {}
...
export function buildQiwaBookmarklet({ sourceId, personId, force = false, resetAt = '' }) {
...
// Chrome extension build (qiwa-sync-extension/build.mjs). The extension runs
// the SAME sweep from a background service worker, which (unlike a bookmarklet)
// can reach every Qiwa subdomain in one run because host_permissions bypass CORS.
export function qiwaSweepBody({ sourceId = 'qiwa', personId = '' } = {}) {
  return body({ sourceId, personId })
}
