...
:null
const metaDates=(isStyled&&form._id&&(form.created_at||form.updated_at))?(()=>{const fmt=(iso)=>{if(!iso)return'';const d=new Date(iso);const p=n=>String(n).padStart(2,'0');return`${d.getFullYear()}/${p(d.getMonth()+1)}/${p(d.getDate())} ${p(d.getHours())}:${p(d.getMinutes())}`};return<div style={{marginTop:14,paddingTop:10,borderTop:'1px dashed var(--bd)',display:'flex',justifyContent:'center',gap:18,fontSize:10,color:'var(--tx4)',flexWrap:'wrap',fontWeight:500}}>
{form.created_at&&<span>{isAr?'تم الإنشاء':'Created'}: <span style={{direction:'ltr',fontFamily:'monospace',color:'var(--tx3)'}}>{fmt(form.created_at)}</span></span>}
{form.updated_at&&<span>{isAr?'آخر تحديث':'Last update'}: <span style={{direction:'ltr',fontFamily:'monospace',color:'var(--tx3)'}}>{fmt(form.updated_at)}</span></span>}
</div>})():null
...
// enforcement: كل تعديل في الإعدادات (حفظ نظام/مقيم + مفاتيح التفعيل + إعادة الترتيب) يتطلب صلاحية settings_fields.edit
const canEditSettings=can(user,'settings_fields.edit')
const saveSetting=async(key,val)=>{if(!canEditSettings)return;const{error}=await sb.from('system_settings').update({setting_value:val,updated_at:new Date().toISOString()}).eq('setting_key',key);if(error)toast((isAr?'خطأ: ':'Error: ')+(error.message||'').slice(0,80));else toast(isAr?'تم الحفظ':'Saved')}
const saveMuqeemCreds=async()=>{if(!canEditSettings)return;const u=(muqeemInputs.username||'').trim();const p=(muqeemInputs.password||'').trim();if(!u||!p){toast(isAr?'املأ الحقلين':'Fill both fields');return}setMuqeemSaving(true);const{error}=await sb.from('muqeem_credentials').upsert({id:'default',username:u,password:p,updated_at:new Date().toISOString(),updated_by:user?.id||null});setMuqeemSaving(false);if(error){toast((isAr?'خطأ: ':'Error: ')+error.message?.slice(0,80));return}setMuqeemCreds({username:u,password:p,updated_at:new Date().toISOString()});setMuqeemEditing(false);setMuqeemShowPw(false);toast(isAr?'تم حفظ بيانات دخول مقيم':'Muqeem credentials saved')}
...
<div style={{padding:'10px 14px',fontSize:11,color:'var(--tx4)',lineHeight:1.7,background:PILL_BG,border:'1px solid var(--bd)',borderRadius:10,boxShadow:PILL_SHADOW}}>{isAr?'يستخدمها البوت على DigitalOcean لتسجيل الدخول إلى مقيم كل 10 دقائق. أي تغيير هنا يتم تطبيقه في الدورة التالية للبوت تلقائياً.':'Used by the DigitalOcean bot to log into Muqeem every 10 minutes. Changes here are picked up on the bot\'s next cycle automatically.'}</div>
<div style={{display:'flex',alignItems:'center',gap:14,padding:12,borderRadius:10,background:PILL_BG,border:'1px solid var(--bd)',boxShadow:PILL_SHADOW}}>
<div style={{flex:1,minWidth:0}}>
...
<div style={cardS}>{items.map((s,i)=><div key={s.id} style={{display:'flex',alignItems:'center',padding:'14px 22px',borderBottom:i<items.length-1?'1px solid var(--bd)':'none',gap:14}}>
<div style={{flex:1,minWidth:0}}>
...
const onDrop=async(e,dropIdx)=>{if(!canEditSettings)return;
  e.preventDefault()
...
{citiesOpen&&<div style={{background:'rgba(52,131,180,.03)',borderBottom:'1px solid var(--bd)',padding:'6px 16px 10px'}}>
...
rc.map(c=>{const cActive=c.is_active!==false;const toggleCity=async()=>{if(!canEditSettings)return;const next=!cActive;setCities(p=>p.map(x=>x.id===c.id?{...x,is_active:next}:x));const{error}=await sb.from('cities').update({is_active:next}).eq('id',c.id);if(error){setCities(p=>p.map(x=>x.id===c.id?{...x,is_active:cActive}:x));toast&&toast(isAr?'فشل تحديث الحالة':'Failed to update status')}};const cdAll=districtsList.filter(d=>d.city_id===c.id);const citySelfMatch=q&&((c.name_ar||'').includes(q)||(c.name_en||'').toLowerCase().includes(qLower)||(c.code||'').includes(q));const cd=q&&!citySelfMatch?cdAll.filter(d=>distMatchSet.has(d.id)):cdAll;const distKey='c_'+c.id;const distOpen=!!open[distKey]||(q&&cityMatchDist.has(c.id));return<div key={c.id} style={{borderBottom:'1px solid var(--bd)'}}>
<div className="jisr-list-row" role="button" tabIndex={0} onClick={()=>toggle(distKey)} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();toggle(distKey)}}} style={{display:'flex',alignItems:'center',gap:10,padding:'7px 14px 7px 20px',borderBottom:distOpen?'1px dashed rgba(39,160,70,.18)':'1px dashed var(--bd2)',opacity:cActive?1:0.55,flexWrap:'wrap',cursor:'pointer'}}>
...
cd.map(d=>{const dActive=d.is_active!==false;const toggleDist=async()=>{if(!canEditSettings)return;const next=!dActive;setDistrictsList(p=>p.map(x=>x.id===d.id?{...x,is_active:next}:x));const{error}=await sb.from('districts').update({is_active:next}).eq('id',d.id);if(error){setDistrictsList(p=>p.map(x=>x.id===d.id?{...x,is_active:dActive}:x));toast&&toast(isAr?'فشل تحديث الحالة':'Failed to update status')}};return<div key={d.id} className="jisr-list-row" style={{display:'flex',alignItems:'center',gap:10,padding:'6px 12px',borderBottom:'1px dotted var(--bd2)',opacity:dActive?1:0.5,flexWrap:'wrap'}}>
...
{shown.map((it,idx)=>{const active=it.is_active!==false;const toggleActive=async()=>{if(!canEditSettings)return;const next=!active;setOccupationsList(p=>p.map(o=>o.id===it.id?{...o,is_active:next}:o));const{error}=await sb.from('occupations').update({is_active:next}).eq('id',it.id);if(error){setOccupationsList(p=>p.map(o=>o.id===it.id?{...o,is_active:active}:o));toast&&toast(isAr?'فشل تحديث الحالة':'Failed to update status')}};return<div key={it.id} className="jisr-list-row" draggable={!q} onDragStart={e=>onDragStart(e,idx)} onDragOver={onDragOver} onDrop={e=>onDrop(e,idx)} style={{display:'flex',alignItems:'center',gap:10,padding:'12px 14px',borderBottom:'1px solid var(--bd)',cursor:q?'default':'grab',opacity:active?1:0.55,flexWrap:'wrap'}}>
{!q&&<svg className="jisr-drag-handle" width="16" height="24" viewBox="0 0 16 24" fill="var(--tx4)" style={{flexShrink:0,cursor:'grab'}} aria-label="drag"><circle cx="3" cy="6" r="1.1"/><circle cx="8" cy="6" r="1.1"/><circle cx="13" cy="6" r="1.1"/><circle cx="3" cy="12" r="1.1"/><circle cx="8" cy="12" r="1.1"/><circle cx="13" cy="12" r="1.1"/><circle cx="3" cy="18" r="1.1"/><circle cx="8" cy="18" r="1.1"/><circle cx="13" cy="18" r="1.1"/></svg>}
<div style={{width:6,height:6,borderRadius:'50%',background:C.gold,flexShrink:0}}/>
<div style={{display:'flex',flexDirection:'column',lineHeight:1.6,minWidth:80,flexShrink:0,alignItems:'flex-start'}}>
<div style={{display:'flex',alignItems:'center',gap:8}}>
...
{embOpen&&<div style={{background:'rgba(52,131,180,.03)',borderBottom:'1px solid var(--bd)',padding:'6px 16px 10px'}}>
...
embs.map(e=>{const eAct=e.is_active!==false;const toggleEmb=async()=>{if(!canEditSettings)return;const next=!eAct;setEmbList(p=>p.map(x=>x.id===e.id?{...x,is_active:next}:x));const{error}=await sb.from('embassies').update({is_active:next}).eq('id',e.id);if(error){setEmbList(p=>p.map(x=>x.id===e.id?{...x,is_active:eAct}:x));toast&&toast(isAr?'فشل تحديث الحالة':'Failed to update status')}};return<div key={e.id} className="jisr-list-row" style={{display:'flex',alignItems:'center',gap:10,padding:'7px 14px 7px 44px',borderBottom:'1px dashed var(--bd2)',opacity:eAct?1:0.55,flexWrap:'wrap'}}>
<div style={{width:5,height:5,borderRadius:'50%',background:'rgba(52,131,180,.7)',flexShrink:0}}/>
<div style={{display:'flex',flexDirection:'column',lineHeight:1.6,minWidth:60,flexShrink:0,alignItems:'flex-start'}}>
<div style={{display:'flex',alignItems:'center',gap:8}}>
...
<div style={{position:'relative',flex:'1 1 200px',minWidth:160,maxWidth:460}}>
<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--tx4)" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" style={{position:'absolute',top:'50%',right:isAr?14:'auto',left:isAr?'auto':14,transform:'translateY(-50%)',pointerEvents:'none'}}><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
<input value={q} onChange={e=>setQ(e.target.value)} placeholder={isAr?'ابحث بالعربي أو الإنجليزي أو المفتاح...':'Search AR/EN/key...'} style={{width:'100%',height:40,padding:isAr?'0 36px 0 14px':'0 14px 0 36px',background:'var(--inputBg)',border:'1px solid var(--bd)',borderRadius:11,fontFamily:F,fontSize:14,fontWeight:400,color:'var(--tx)',outline:'none',boxShadow:'0 2px 8px rgba(0,0,0,.18), inset 0 1px 0 rgba(255,255,255,.05)',transition:'.2s',minWidth:0,boxSizing:'border-box'}}/>
</div>
...
docs.map((d,i)=><tr key={d.id} style={{borderBottom:'1px solid var(--bd)'}}>
<td style={{padding:'14px 22px',fontSize:11,color:'var(--tx5)'}}>{i+1}</td>
