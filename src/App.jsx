...
useEffect(()=>{if(!sb)return;sb.from('branches').select('id,branch_code,name_ar').is('deleted_at',null).then(({data})=>{if(data){const sorted=[...data].sort((a,b)=>{const na=parseInt((a.branch_code||'').match(/\d+/g)?.pop()||'0',10);const nb=parseInt((b.branch_code||'').match(/\d+/g)?.pop()||'0',10);return na-nb});setRegBranches(sorted)}})},[sb]);
...
],null,'professional',['occupation_name_ar','sponsor_changes','change_profession','new_occupation_name_ar','hrsd_worker_status','resident_status_ar','iqama_expiry_gregorian','iqama_expiry_hijri','renewal_months'])}
