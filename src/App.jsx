...
const timeout=setTimeout(()=>setView('login'),8000);client.from('system_settings').select('setting_key,setting_value').eq('setting_key','gm_setup_complete').single().then(({data,error})=>{if(error){clearTimeout(timeout);setView('login');return}const done=data?.setting_value==='true';setGmDone(done);client.auth.getSession().then(({data:{session}})=>{clearTimeout(timeout);if(session){client.from('users').select('*,roles:role_id(name_ar,name_en,color)').eq('auth_id',session.user.id).single().then(({data:u})=>{if(u){if(u.preferred_lang)setLangPersist(u.preferred_lang);setUser(u);setView('app')}else setView('login')}).catch(()=>setView('login'))}else setView('login')}).catch(()=>{clearTimeout(timeout);setView('login')})}).catch(()=>{clearTimeout(timeout);setView('login')})},[]);const handleLogin=async(email,pass)=>{const{data,error}=await sb.auth.signInWithPassword({email,password:pass});if(error)throw error;const{data:u,error:e2}=await sb.from('users').select('*,roles:role_id(name_ar,name_en,color)').eq('auth_id',data.user.id).single();if(e2||!u)throw new Error('User not found');if(!u.is_active){await sb.auth.signOut();throw new Error(lang==='ar'?'حسابك قيد المراجعة — يرجى انتظار موافقة المسؤول':'Your account is under review — please wait for admin approval')}sb.from('users').update({last_login_at:new Date().toISOString()}).eq('id',u.id).then(()=>{});setUser(u);if(u.preferred_lang)setLangPersist(u.preferred_lang);tt(lang==='ar'?'تم تسجيل الدخول بنجاح':'Logged in successfully');setView('app')};const handleSetup=async(form)=>{
