...
// ═══ OTP Global Notifications (works on any page) ═══
...
const otpInterval=setInterval(()=>{sb.from('otp_messages').select('*').order('received_at',{ascending:false}).limit(5).then(({data})=>{if(!data)return;const newCount=data.length;if(lastOtpCount>0&&data.length>0){const latest=data[0];const age=(Date.now()-new Date(latest.received_at).getTime())/1000;if(age<15){const title=latest.otp_code?'رمز تحقق: '+latest.otp_code:'رسالة تحقق جديدة';const body=(latest.person_name||'')+ ' — '+(latest.phone_from||'')+'\n'+(latest.message_body||'').substring(0,80);if('Notification' in window&&Notification.permission==='granted')new Notification(title,{body,icon:'/icons/icon-192.png',tag:latest.id,requireInteraction:true});tt(latest.person_name+': '+(latest.otp_code||'رسالة جديدة')+' — '+(latest.phone_from||''))}}sb.from('otp_messages').select('id',{count:'exact',head:true}).then(({count})=>{lastOtpCount=count||0})})},10000);
