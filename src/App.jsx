...
return <FKModal open onClose={()=>{if(approveSaving)return;setApproveForm(null);if(approveSaved){setApproveSaved(false);setDetailsRow(null)}}} width={560} variant="edit"
 ...
 <div style={{gridColumn:'1/-1',display:'flex',alignItems:'center',gap:10,padding:'12px 14px',borderRadius:9,background:'rgba(176,125,0,.08)',border:'1px solid rgba(176,125,0,.3)',minHeight:44}}><span style={{fontSize:13,fontWeight:600,color:'var(--tx2)'}}>{T('الإجمالي بعد الخصم','Total after discount')}</span><span style={{flex:1}}/><span style={{fontSize:16,fontWeight:600,color:FKC.gold,direction:'ltr',fontVariantNumeric:'tabular-nums'}}>{nm(Math.max(0,f._subtotal-(f._absher||0)-_appliedDisc))+' '+T('ريال','SAR')}</span></div>
 </div>
 </ModalSection></>)}
 ]}/>
})()}
