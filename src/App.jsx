...
const C = { dk:'#171717', md:'#222222', fm:'#1e1e1e', gold:'#B07D00', gl:'#B07D00', brd:'rgba(255,255,255,.13)', red:'#c0392b', blue:'#3483b4', ok:'#27a046' }
...
const ICO = {
  email: <svg width="18" height="18" viewBox="0 0 24 24" fill="none"><rect x="2" y="4" width="20" height="16" rx="3" fill="rgba(176,125,0,.15)" stroke="rgba(176,125,0,.5)" strokeWidth="1.5"/><path d="m22 7-8.97 5.7a1.94 1.94 0 01-2.06 0L2 7" stroke="rgba(176,125,0,.7)" strokeWidth="1.5"/></svg>,
  lock: <svg width="20" height="20" viewBox="0 0 24 24" fill="none"><rect x="3" y="11" width="18" height="11" rx="2.5" stroke="#B07D00" strokeWidth="1.5"/><path d="M7 11V7a5 5 0 0110 0v4" stroke="#B07D00" strokeWidth="1.5" strokeLinecap="round"/><circle cx="12" cy="16" r="1.5" fill="#B07D00"/></svg>,
  ...
  bolt: <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z" fill="rgba(176,125,0,.2)" stroke="rgba(176,125,0,.6)" strokeWidth="1.5" strokeLinejoin="round"/></svg>,
  eyeOn: <svg width="20" height="20" viewBox="0 0 24 24" fill="none"><path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7z" fill="rgba(176,125,0,.1)" stroke="#B07D00" strokeWidth="1.8"/><circle cx="12" cy="12" r="3" fill="rgba(176,125,0,.12)" stroke="#B07D00" strokeWidth="1.8"/></svg>,
  eyeOff: <svg width="20" height="20" viewBox="0 0 24 24" fill="none"><path d="M17.94 17.94A10.07 10.07 0 0112 20c-7 0-11-8-11-8a18.45 18.45 0 015.06-5.94" stroke="#B07D00" strokeWidth="1.8"/><path d="M9.9 4.24A9.12 9.12 0 0112 4c7 0 11 8 11 8a18.5 18.5 0 01-2.16 3.19" stroke="#B07D00" strokeWidth="1.8"/><line x1="1" y1="1" x2="23" y2="23" stroke="#B07D00" strokeWidth="1.8"/></svg>,
}
...
const DT = (clr) => ({
  ...
  attach: <svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M5 4h8l5 5v11a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1V5a1 1 0 0 1 1-1Z" fill={clr} fillOpacity=".12" stroke={clr} strokeWidth="1.5" strokeLinejoin="round" opacity=".55"/><path d="M13 4v5h5" stroke={clr} strokeWidth="1.5" strokeLinejoin="round" opacity=".55"/><path d="M20.5 11.5v5.5a3 3 0 0 1-6 0V10a1.8 1.8 0 0 1 3.6 0v6.5a.6.6 0 0 1-1.2 0V11" stroke={clr} strokeWidth="1.5" strokeLinecap="round"/></svg>,
  subscription: <svg width="18" height="18" viewBox="0 0 24 24" fill="none"><rect x="3" y="5" width="18" height="15" rx="2.5" fill={clr} fillOpacity=".12" stroke={clr} strokeWidth="1.5"/><path d="M8 3v4M16 3v4" stroke={clr} strokeWidth="1.5" strokeLinecap="round" opacity=".5"/><path d="M15.5 12.2a3.6 3.6 0 1 1-1.1-2.5" stroke={clr} strokeWidth="1.5" strokeLinecap="round"/><path d="M14.6 8.4v1.6h-1.6" stroke={clr} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>,
...
// ── مرونة الشبكة ────────────────────────────────────────────────────────────
// الخادم يتباطأ أحياناً تحت الحمل: صار يرجع 522/504 بزمن أصل يتجاوز الدقيقة.
// مهلة العشر ثوانٍ الصارمة كانت تُسقط تسجيل الدخول فوراً وتعرض «تحقق من
// الإنترنت» والإنترنت سليم تماماً. الآن: مهلة 25 ثانية + إعادة محاولة تلقائية
// للأخطاء العابرة وحدها — بيانات الدخول الخاطئة تفشل من أول مرة بلا انتظار.
...
const netCall=(p,ms=NET_TIMEOUT_MS)=>retryTransient(async()=>{const r=await netTimeout(typeof p==='function'?p():p,ms);if(r&&r.error&&isTransientErr(r.error))throw r.error;return r})
// التفرقة الصادقة: انقطاع إنترنت فعلي مقابل خادم مشغول. الرسالة القديمة كانت
...
const rt=(p,ms=NET_TIMEOUT_MS)=>netCall(p,ms);
...
function Splash(){return<div style={{position:'fixed',inset:0,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:28,background:'radial-gradient(ellipse 120% 90% at 50% 42%,var(--sf),var(--bg) 72%)',fontFamily:F,direction:'rtl'}}>
  ...
  <div style={{position:'relative',display:'flex',flexDirection:'column',alignItems:'center',gap:7,animation:'fadeIn .9s ease'}}>
    <div style={{fontSize:20,fontWeight:600,color:'var(--tx)',letterSpacing:'.3px',fontFamily:"'Reem Kufi','Cairo',sans-serif"}}>تأشيرة البناء والإنشاء</div>
...
useEffect(()=>{const prev=document.body.style.background;document.body.style.background='#faf8f3';const m=document.querySelector('meta[name="theme-color"]');const pm=m?.getAttribute('content');if(m)m.setAttribute('content','#faf8f3');return()=>{document.body.style.background=prev;if(m&&pm)m.setAttribute('content',pm)}},[]);
const go=async e=>{
...
return <FKModal open onClose={()=>{setShowReg(false);setRegDone(false);setRegSubmitErr('')}} width={640}
 ...
 <FKSelect label={ar?'المكتب':'Branch'} req error={regErr.branch_id} placeholder={ar?'اختر':'Select'} value={reg.branch_id} options={regBranches} getKey={b=>b.id} getLabel={b=>branchLabel(b)} onChange={v=>setReg(p=>({...p,branch_id:v}))}/>
 </div>
 </ModalSection>)},
...
<div style={{fontSize:'clamp(18px,3vw,22px)',fontWeight:600,color:'var(--tx)',marginBottom:6}}>{lang==='ar'?'تم تغيير كلمة المرور':'Password changed!'}</div>
...
<div style={{width:'min(420px,92vw)',background:'var(--sf)',borderRadius:20,padding:'clamp(24px,4vw,36px) clamp(20px,4vw,28px)',boxShadow:'0 20px 60px rgba(0,0,0,.5)',position:'relative',border:'1px solid rgba(176,125,0,.12)'}}>
<GoldBar/>
<div style={{textAlign:'center',marginBottom:24}}>
<div style={{width:56,height:56,borderRadius:'50%',background:'rgba(176,125,0,.1)',border:'1.5px solid rgba(176,125,0,.12)',display:'flex',alignItems:'center',justifyContent:'center',margin:'0 auto 14px'}}>{ICO.lock}</div>
<div style={{fontSize:'clamp(16px,2.5vw,20px)',fontWeight:600,color:'var(--tx)'}}>{lang==='ar'?'كلمة مرور جديدة':'New Password'}</div>
<div style={{fontSize:12,color:'var(--tx3)',marginTop:6}}>{lang==='ar'?'أدخل كلمة المرور الجديدة':'Enter your new password'}</div>
</div>
<div style={{display:'flex',flexDirection:'column',gap:14}}>
<div><div style={{fontSize:12,fontWeight:600,color:'var(--tx3)',marginBottom:6}}>{lang==='ar'?'كلمة المرور الجديدة':'New Password'}</div>
<input value={pw} onChange={e=>setPw(arToEn(e.target.value))} type="password" placeholder="······" style={{...finS,textAlign:'center'}}/></div>
<div><div style={{fontSize:12,fontWeight:600,color:'var(--tx3)',marginBottom:6}}>{lang==='ar'?'تأكيد كلمة المرور':'Confirm Password'}</div>
...
useEffect(()=>{hydrateSvcAdminFromDb().catch(e=>console.warn('[svcAdminSync] startup hydrate failed',e))},[]);const[dashBranch]=useState(null);const[dashBranches,setDashBranches]=useState([]);const[,setSTabInfo]=useState({tab:'general',svcSubTab:'services'});const[sideOpen,setSideOpen]=useState(false);const sideOpenRef=useRef(false);sideOpenRef.current=sideOpen;const[hbHover,setHbHover]=useState(false);const[showServiceRequest,setShowServiceRequest]=useState(false);const[navExpanded,setNavExpanded]=useState({});
...
useEffect(()=>{const handler=(e)=>{navPushFrom('workers',{kind:'worker',id:e.detail?.id});setPg('workers');setTimeout(()=>window.dispatchEvent(new CustomEvent('worker-open',{detail:e.detail})),80)};window.addEventListener('app-navigate-worker',handler);return()=>window.removeEventListener('app-navigate-worker',handler)},[]);
useEffect(()=>{const handler=(e)=>{navPushFrom('temp_workers',{kind:'temp_worker',id:e.detail?.id});setPg('temp_workers');setTimeout(()=>window.dispatchEvent(new CustomEvent('temp-worker-open',{detail:e.detail})),80)};window.addEventListener('app-navigate-temp-worker',handler);return()=>window.removeEventListener('app-navigate-temp-worker',handler)},[]);
...
useEffect(()=>{const handler=(e)=>{const q=e.detail?.q||'';navPushFrom('renewal_calc',{kind:'renewal_quote',id:q});try{window.location.hash='#renewal_calc?q='+encodeURIComponent(q)}catch{}setPg('renewal_calc');setTimeout(()=>{try{window.dispatchEvent(new HashChangeEvent('hashchange'))}catch{}},60)};window.addEventListener('app-navigate-renewal-calc',handler);return()=>window.removeEventListener('app-navigate-renewal-calc',handler)},[]);
...
/* ── تجربة التطبيق على الجوال ─────────────────────────────────────────────
   زر الرجوع في الجوال: القائمة الجانبية المفتوحة تُطوى، ثم (بعد النوافذ
   والتفاصيل المسجَّلة في mobileBack) الصفحة السابقة، ثم الرئيسية، ثم الخروج. */
/* السحب للتحديث (الجوال): سحب الصفحة للأسفل من أعلاها يعيد تركيبها فتُعيد جلب بياناتها */
const[ptrKey,setPtrKey]=useState(0);
useEffect(()=>{if(!window.matchMedia('(max-width: 768px)').matches)return;const el=document.querySelector('.dash-content');const ind=document.querySelector('.m-ptr');if(!el||!ind)return;let y0=null,d=0;
const busy=()=>!!document.querySelector('.fk-modal-ovl,.m-more,.fk-msheet-wrap,.fk-confirm-ovl');
const reset=()=>{ind.style.opacity='0';ind.style.transform='translate(-50%,0)';ind.classList.remove('ready')};
const start=e=>{if(el.scrollTop>2||e.touches.length!==1||busy()){y0=null;return}y0=e.touches[0].clientY;d=0;const top=document.querySelector('.m-shell-top')?.getBoundingClientRect().bottom||60;ind.style.top=(top+6)+'px'};
const move=e=>{if(y0==null)return;if(el.scrollTop>2){y0=null;reset();return}d=Math.max(0,e.touches[0].clientY-y0);ind.style.opacity=String(Math.min(d/70,1));ind.style.transform='translate(-50%,'+Math.min(d*.45,46)+'px) rotate('+(d*2.4)+'deg)';ind.classList.toggle('ready',d>75)};
const end=()=>{if(y0==null)return;if(d>75){ind.classList.add('spin');setPtrKey(k=>k+1);setTimeout(()=>{ind.classList.remove('spin');reset()},800)}else reset();y0=null;d=0};
el.addEventListener('touchstart',start,{passive:true});el.addEventListener('touchmove',move,{passive:true});el.addEventListener('touchend',end,{passive:true});el.addEventListener('touchcancel',end,{passive:true});
...
const pgHistRef=useRef([]);const pgRef=useRef(pg);pgRef.current=pg;
useEffect(()=>{const h=pgHistRef.current;if(h[h.length-1]!==pg){h.push(pg);if(h.length>30)h.shift()}rearmMobileBack()},[pg]);
useEffect(()=>installMobileBack(()=>{const h=pgHistRef.current;if(h.length>1){h.pop();setPg(h[h.length-1]);setSideOpen(false);return true}if(pgRef.current!=='home'){pgHistRef.current=['home'];setPg('home');return true}return false}),[]);
useBackHandler(sideOpen,()=>setSideOpen(false));
/* إيماءات اللمس: سحبٌ من حافة الشاشة يفتح القائمة وسحبُها عكساً يطويها؛ والضغطة
   المطوّلة على iOS (لا يُطلق Safari حدث contextmenu) تفتح قوائم الخيارات التي
   تفتحها الضغطة اليمنى على الحاسب — كقائمة صفوف وخلايا الجداول. */
useEffect(()=>{
const rtl=lang==='ar';
const isIOS=/iP(hone|ad|od)/.test(navigator.userAgent)||(navigator.platform==='MacIntel'&&navigator.maxTouchPoints>1);
let st=null,lp=null,lpFired=false,suppressUntil=0;
const clearLp=()=>{if(lp){clearTimeout(lp.t);lp=null}};
const onStart=e=>{if(e.touches.length!==1){st=null;clearLp();return}const t=e.touches[0];const W=window.innerWidth;
const inSide=!!e.target.closest?.('.dash-side');
const edge=rtl?t.clientX>W-24:t.clientX<24;
st={x:t.clientX,y:t.clientY,open:!sideOpenRef.current&&edge,close:sideOpenRef.current&&inSide,done:false};
lpFired=false;if(isIOS&&!e.target.closest?.('input,textarea,select,[contenteditable]')){clearLp();const target=e.target;lp={x:t.clientX,y:t.clientY,t:setTimeout(()=>{lp=null;lpFired=true;try{navigator.vibrate?.(10)}catch{}target.dispatchEvent(new MouseEvent('contextmenu',{bubbles:true,cancelable:true,clientX:t.clientX,clientY:t.clientY,button:2}))},520)}}};
const onMove=e=>{const t=e.touches[0];if(lp&&(Math.abs(t.clientX-lp.x)>10||Math.abs(t.clientY-lp.y)>10))clearLp();if(!st||st.done)return;const dx=t.clientX-st.x,dy=t.clientY-st.y;if(Math.abs(dy)>40){st=null;return}
if(st.open&&(rtl?-dx:dx)>50){st.done=true;setSideOpen(true)}
else if(st.close&&(rtl?dx:-dx)>60){st.done=true;setSideOpen(false)}};
const onEnd=()=>{clearLp();st=null;if(lpFired){lpFired=false;suppressUntil=Date.now()+400}};
const onClick=e=>{if(Date.now()<suppressUntil){e.preventDefault();e.stopPropagation();suppressUntil=0}};
window.addEventListener('touchstart',onStart,{passive:true});window.addEventListener('touchmove',onMove,{passive:true});window.addEventListener('touchend',onEnd,{passive:true});window.addEventListener('touchcancel',onEnd,{passive:true});window.addEventListener('click',onClick,true);
return()=>{clearLp();window.removeEventListener('touchstart',onStart);window.removeEventListener('touchmove',onMove);window.removeEventListener('touchend',onEnd);window.removeEventListener('touchcancel',onEnd);window.removeEventListener('click',onClick,true)}},[lang]);
...
/* بنود القائمة الظاهرة لهذا المستخدم — تشترك فيها القائمة الجانبية وشريط الجوال السفلي */
const navShown=nav.filter(n=>{if(ADMIN_ONLY.includes(n.id)&&!canSeeAdminOnly(n.id))return false;if(!isVisible(n.id))return false;const s=hubTabs[n.id];return !s||s.some(t=>!t.hdr&&isVisible(t.id))});
...
};const pageConf=pages[pg];return(<div className='dash-wrap' dir={lang==='ar'?'rtl':'ltr'} style={{display:'flex',height:'100%',direction:lang==='ar'?'rtl':'ltr',fontFamily:"'Cairo',sans-serif",background:'var(--bg)',WebkitFontSmoothing:'antialiased',overflow:'hidden'}}>
{/* ═══ MOBILE OVERLAY ═══ */}
...
{n.n>0&&<span style={{fontSize:9,fontWeight:600,background:C.red,color:'#fff',padding:'1px 6px',borderRadius:8,minWidth:16,textAlign:'center'}}>{n.n}</span>}
...
{t.n>0&&<span style={{fontSize:9,fontWeight:600,background:sAct?'rgba(176,125,0,.15)':'var(--bd)',color:sAct?C.gold:'var(--sbtx2)',padding:'1px 7px',borderRadius:8,minWidth:16,textAlign:'center'}}>{t.n}</span>}
</div>})}
</div>}
</div>})}
</div>
...
{/* ═══ MAIN AREA ═══ */}
<div style={{flex:1,display:'flex',flexDirection:'column',background:'var(--bg)',minWidth:0,paddingRight:'max(0px, env(safe-area-inset-right))'}}>
{/* ═══ TOPBAR ═══ */}
<header className='dash-header' style={{minHeight:'calc(56px + env(safe-area-inset-top, 0px))',paddingTop:'max(0px, env(safe-area-inset-top))',paddingLeft:'max(20px, env(safe-area-inset-left))',paddingRight:'max(20px, env(safe-area-inset-right))',background:'var(--sb)',display:'flex',alignItems:'center',gap:14,flexShrink:0,boxShadow:'0 2px 12px rgba(0,0,0,.12)',minWidth:0}}>
...
<span style={{fontSize:13.5,fontWeight:600,color:'var(--accent)',whiteSpace:'nowrap',textShadow:'0 1px 8px rgba(176,125,0,.18)'}}>{pageLabel}</span>
</div>
})()}
...
const offIds=(Array.isArray(user?.branch_ids)&&user.branch_ids.length)?user.branch_ids:(user?.primary_branch_id?[user.primary_branch_id]:[]);
const offBranches=offIds.map(id=>(dashBranches||[]).find(b=>b.id===id)).filter(Boolean);
const codes=offBranches.map(b=>b.branch_code).filter(Boolean);
const officeText=isGM?T('كل المكاتب','All offices'):codes.join(' · ');
const officeTitle=isGM?'':offBranches.map(b=>branchLabel(b)).join('  |  ');
const initial=(nm||'؟').trim().charAt(0);
const openProfile=()=>{const nat=natCache;const natLabel=nat?(lang==='en'?nat.name_en||nat.name_ar:nat.name_ar)||'':'';setShowProfile(true);setProfileTab('info');setProfileErr({});setProfileData({phone:user.personal_phone||user.person?.phone_primary||'',email:user.email||user.person?.email||'',name_ar:user.person?.name_ar||'',name_en:user.person?.name_en||'',id_number:user.person?.id_number||'',nationality:natLabel,nationality_code:nat?.code||'',nationality_flag:nat?.flag_url||'',avatar_url:user.person?.avatar_url||'',_origEmail:user.email||user.person?.email||''});if(!nat){(async()=>{const natId=user.person?.nationality_id;if(natId){const{data}=await sb.from('nationalities').select('id,name_ar,name_en,code,flag_url').eq('id',natId).maybeSingle();if(data){setNatCache(data);const lbl=lang==='en'?data.name_en||data.name_ar:data.name_ar;setProfileData(p=>({...p,nationality:lbl||'',nationality_code:data.code||'',nationality_flag:data.flag_url||''}))}}})();}};
return<div data-avatar onClick={openProfile} title={(lang==='en'?(user?.person?.name_en||user?.person?.name_ar):(user?.person?.name_ar||user?.person?.name_en))||(lang==='ar'?'الملف الشخصي':'Profile')} style={{display:'inline-flex',alignItems:'center',gap:9,cursor:'pointer',padding:'4px 8px 4px 6px',borderRadius:10,background:'var(--hoverBg)',border:'1px solid var(--bd)',transition:'.18s',flexShrink:0}} onMouseEnter={e=>e.currentTarget.style.background='var(--hd-ico-hv-bg)'} onMouseLeave={e=>e.currentTarget.style.background='var(--hoverBg)'}>
<div className='topbar-uitext' style={{display:'flex',flexDirection:'column',alignItems:'center',lineHeight:1.3,gap:5,whiteSpace:'nowrap'}}>
{nm&&<span title={nm} style={{fontSize:12,fontWeight:600,color:'var(--hdtx)'}}>{nm.trim().split(/\s+/)[0]}</span>}
{officeText&&<span title={officeTitle} style={{fontSize:10,fontWeight:600,color:C.gold,direction:'ltr'}}>{officeText}</span>}
</div>
{/* على الجوال يُخفى النص أعلاه ويظهر بدلاً منه أفاتار بالحرف الأول — كي لا يبقى الكرت صندوقاً فارغاً */}
<div className='topbar-ini' style={{display:'none',width:26,height:26,borderRadius:'50%',alignItems:'center',justifyContent:'center',background:'var(--accent-soft)',border:'1px solid var(--accent-bd)',color:C.gold,fontSize:13,fontWeight: 600,flexShrink:0}}>{initial}</div>
</div>;
})()}
{/* شريحة التاريخ — هجري + اليوم/الميلادي */}
<div className='topbar-datetime' style={{display:'inline-flex',alignItems:'center',padding:'5px 12px',borderRadius:10,background:'var(--hoverBg)',border:'1px solid var(--bd)',whiteSpace:'nowrap',flexShrink:0}}>
{(()=>{const d=new Date();const pad=n=>String(n).padStart(2,'0');const greg=d.getFullYear()+'-'+pad(d.getMonth()+1)+'-'+pad(d.getDate());const day=d.toLocaleDateString(lang==='ar'?'ar-SA':'en-US',{weekday:'long'});let hijri='';try{const hp=new Intl.DateTimeFormat('en-US-u-ca-islamic-umalqura',{day:'numeric',month:'numeric',year:'numeric'}).formatToParts(d);const hv=t=>{const p=hp.find(x=>x.type===t);return p?p.value:''};hijri=hv('year')+'-'+pad(+hv('month'))+'-'+pad(+hv('day'))}catch{}return<div style={{display:'flex',flexDirection:'column',alignItems:'flex-end',lineHeight:1.3,gap:3,fontFamily:"'Reem Kufi','Cairo',sans-serif"}}><span style={{fontSize:9.5,fontWeight:500,color:'var(--hdtx2)',letterSpacing:'.5px',direction:'ltr',alignSelf:'center'}}>{hijri}</span><div style={{display:'flex',alignItems:'baseline',gap:7}}><span style={{fontSize:11.5,fontWeight:600,color:C.gold}}>{day}</span><span style={{fontSize:12,fontWeight:600,color:C.gold,letterSpacing:'.5px',direction:'ltr',opacity:.9}}>{greg}</span></div></div>})()}
</div>
{/* شريحة الأوامر — اللغة + الخروج داخل كبسولة واحدة */}
<div style={{display:'inline-flex',alignItems:'center',gap:2,padding:3,borderRadius:10,background:'var(--hoverBg)',border:'1px solid var(--bd)',flexShrink:0}}>
<div onClick={switchLang} title={lang==='ar'?'English':'العربية'} style={{width:32,height:32,borderRadius:8,background:'transparent',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',transition:'.18s'}} onMouseEnter={e=>{e.currentTarget.style.background='var(--hd-ico-hv-bg)';const s=e.currentTarget.querySelector('svg');if(s)s.style.stroke='var(--hd-ico-hv)'}} onMouseLeave={e=>{e.currentTarget.style.background='transparent';const s=e.currentTarget.querySelector('svg');if(s)s.style.stroke='rgba(176,125,0,.72)'}}>
<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="rgba(176,125,0,.72)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{flexShrink:0,transition:'stroke .18s'}}><circle cx="12" cy="12" r="9"/><path d="M3 12h18M12 3a14 14 0 010 18 14 14 0 010-18z"/></svg>
</div>
<div onClick={onLogout} title={lang==='ar'?'تسجيل الخروج':'Sign Out'} style={{width:32,height:32,borderRadius:8,background:'transparent',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',transition:'.18s',position:'relative'}} onMouseEnter={e=>{e.currentTarget.style.background='rgba(192,57,43,.14)';const s=e.currentTarget.querySelector('svg');if(s)s.style.stroke='rgba(232,120,100,.95)'}} onMouseLeave={e=>{e.currentTarget.style.background='transparent';const s=e.currentTarget.querySelector('svg');if(s)s.style.stroke='rgba(192,57,43,1)'}}>
<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="rgba(192,57,43,1)" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round" style={{transition:'stroke .18s',transform:lang==='ar'?'scaleX(-1)':'none'}}><path d="M9 21H5a2 2 0 01-2-2V5a2 2 0 012-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>
</div>
</div>
</header>
{/* ═══ ترويسة الجوال + تبويبات القسم ═══ */}
{(()=>{const hubId=Object.keys(hubTabs).find(h=>(hubTabs[h]||[]).some(t=>t.id===pg));const hub=hubId&&nav.find(n=>n.id===hubId);const tabs=hubId?(hubTabs[hubId]||[]).filter(t=>!t.hdr&&isVisible(t.id)):[];
const offIds=(Array.isArray(user?.branch_ids)&&user.branch_ids.length)?user.branch_ids:(user?.primary_branch_id?[user.primary_branch_id]:[]);const codes=offIds.map(id=>(dashBranches||[]).find(b=>b.id===id)).filter(Boolean).map(b=>b.branch_code).filter(Boolean);
return<div className='m-shell-top'>
<MobileTopBar user={user} lang={lang} title={pg==='home'?null:(hub?hub.l:(nav.find(n=>n.id===pg)?.l||null))} officeText={isGM?T('كل المكاتب','All offices'):codes.join(' · ')} onProfile={()=>document.querySelector('[data-avatar]')?.click()} onLang={switchLang} onLogout={onLogout}/>
{hub&&<MobileHubTabs tabs={tabs} active={pg} onPick={setPage}/>}
</div>})()}
{/* ═══ Content ═══ */}
<div className='m-ptr' aria-hidden><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 12a9 9 0 1 1-2.64-6.36"/><polyline points="21 3 21 9 15 9"/></svg></div>
...
<div key={pg+':'+ptrKey} className='m-page-enter'>
...
{pg==='visa_grid'&&<VisaGridPage sb={sb} toast={tt} user={user} lang={lang} onTabChange={setSTabInfo}/>}
...
{toastMsg&&(()=>{/* النوع الممرَّر (tt(msg,'error'|'delete')) يحكم اللون أولاً — الاستدلال بالكلمات يبقى احتياطاً للنداءات القديمة بلا نوع */const txt=toastMsg.m;const isErr=toastMsg.t==='error'||(!toastMsg.t&&(txt.includes('خطأ')||txt.includes('فشل')||txt.includes('تعذّر')||txt.includes('تعذر')));const isDel=toastMsg.t==='delete'||(!toastMsg.t&&txt.includes('حذف'));const bad=isErr||isDel;const clr=bad?'#b23b2e':'#1f7a45';const bg=bad?'rgba(251,236,234,.97)':'rgba(233,247,238,.97)';const bdr=bad?'rgba(192,57,43,.35)':'rgba(46,160,95,.4)';return<div className='app-toast' style={{position:'fixed',top:20,left:'50%',transform:'translateX(-50%)',zIndex:9999,background:bg,color:clr,fontFamily:"'Cairo',sans-serif",fontSize:12.5,fontWeight:600,padding:'11px 18px',borderRadius:10,boxShadow:'0 10px 30px rgba(90,70,30,.16), 0 0 0 1px '+bdr,backdropFilter:'blur(10px)',display:'flex',alignItems:'center',gap:10,animation:'toastSlide .35s cubic-bezier(.2,.85,.3,1.05)',maxWidth:'calc(100vw - 32px)'}}>{isErr?<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={clr} strokeWidth="2.4" strokeLinecap="round"><circle cx="12" cy="12" r="10"/><path d="M12 8v4M12 16h.01"/></svg>:isDel?<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={clr} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6m3 0V4a2 2 0 012-2h4a2 2 0 012 2v2"/></svg>:<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={clr} strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"><path d="M22 11.08V12a10 10 0 11-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>}<span>{txt}</span><style>{`@keyframes toastSlide{from{opacity:0;transform:translate(-50%,-12px)}to{opacity:1;transform:translate(-50%,0)}}`}</style></div>})()}
...
<FKModal open onClose={()=>{setShowProfile(false);setEmailConfirmStep(false)}} width={600} height="auto" accent="#B07D00"
...
<Css/>
{/* ═══ شريط الجوال السفلي + شاشة «المزيد» ═══ */}
{(()=>{
/* أوّل أربعة أقسامٍ يراها المستخدم بترتيب الأولوية ثم «المزيد». يتبع صلاحيات القائمة نفسها،
   والقسم يُضاء متى كانت الصفحة المفتوحة أحد تبويباته. */
const PREF=['home','finance_hub','facilities_hub','labor_hub','iqama_hub','visa_hub','pricing_hub','offices_hub','persons_hub'];
const items=PREF.map(id=>navShown.find(n=>n.id===id)).filter(Boolean).slice(0,4);
const isOn=n=>pg===n.id||(hubTabs[n.id]||[]).some(t=>t.id===pg);
const moreOn=sideOpen||!items.some(isOn);
const ico=(name,clr,sz)=>{const el=DT(clr)[name]||DT(clr).general;return React.cloneElement(el,{width:sz,height:sz})};
const tabItems=[...items.map(n=>({id:n.id,l:n.l,on:isOn(n)&&!sideOpen,icon:ico(n.i,isOn(n)&&!sideOpen?C.gold:'var(--tx3)',23)})),
{id:'_more',l:T('المزيد','More'),on:moreOn,icon:<svg width="23" height="23" viewBox="0 0 24 24" fill="none" stroke={moreOn?C.gold:'var(--tx3)'} strokeWidth="1.7"><rect x="3.5" y="3.5" width="7" height="7" rx="2.2"/><rect x="13.5" y="3.5" width="7" height="7" rx="2.2"/><rect x="3.5" y="13.5" width="7" height="7" rx="2.2"/><rect x="13.5" y="13.5" width="7" height="7" rx="2.2" fill={moreOn?C.gold:'none'} fillOpacity=".2"/></svg>}];
const sections=navShown.map(n=>{const subs=(hubTabs[n.id]||null);const tabs=subs?subs.filter(t=>t.hdr||isVisible(t.id)).filter((t,i,a)=>!t.hdr||(a[i+1]&&!a[i+1].hdr)):null;return{...n,tabs}});
return<>
<MobileTabBar items={tabItems} onPick={id=>{if(id==='_more')setSideOpen(o=>!o);else setPage(id)}}/>
<MobileMoreSheet open={sideOpen} onClose={()=>setSideOpen(false)} sections={sections} active={pg} onPick={setPage} lang={lang} renderIcon={ico} user={user}
 onProfile={()=>document.querySelector('[data-avatar]')?.click()} onLang={switchLang} onLogout={onLogout}/>
</>})()}
{/* ═══ INSTALL BANNER ═══ */}
{showInstallBanner&&!isStandalone&&<div className='install-banner' style={{position:'fixed',bottom:'calc(70px + max(0px, env(safe-area-inset-bottom)))',left:'max(12px, env(safe-area-inset-left))',right:'max(12px, env(safe-area-inset-right))',zIndex:197,background:'linear-gradient(135deg,#1a1a1a,#252525)',border:'1px solid rgba(176,125,0,.25)',borderRadius:16,padding:'14px 16px',display:'flex',alignItems:'center',gap:12,boxShadow:'0 8px 32px rgba(0,0,0,.5)',fontFamily:"'Cairo',sans-serif"}}>
...
<div style={{flex:1}}>
<div style={{fontSize:13,fontWeight:600,color:'rgba(255,255,255,.9)'}}>{T('ثبّت التطبيق','Install App')}</div>
...
<button onClick={handleInstall} style={{height:34,padding:'0 16px',borderRadius:10,background:C.gold,border:'none',fontFamily:"'Cairo',sans-serif",fontSize:12,fontWeight:600,color:'#141414',cursor:'pointer',flexShrink:0}}>{T('تثبيت','Install')}</button>
...
<button onClick={openAdd} style={{height:40,padding:'0 18px',borderRadius:11,border:'1px solid rgba(176,125,0,.45)',background:'linear-gradient(180deg,rgba(176,125,0,.22) 0%,rgba(176,125,0,.10) 100%)',color:C.gold,fontFamily:F,fontSize:12,fontWeight:600,cursor:'pointer',display:'inline-flex',alignItems:'center',gap:8,boxShadow:'0 2px 8px rgba(176,125,0,.18), inset 0 1px 0 rgba(176,125,0,.18)',transition:'.2s',flexShrink:0}}>
...
<div style={{fontSize:18,fontWeight:600,color:cl,letterSpacing:'-.3px',direction:'ltr',lineHeight:1,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{c.value}</div>
</div>
...
filtered.map(r=><tr key={r.id} onClick={()=>setViewRow(r)} style={{borderBottom:'1px solid rgba(255,255,255,.04)',cursor:'pointer',transition:'.18s'}} onMouseEnter={e=>e.currentTarget.style.background='rgba(176,125,0,.04)'} onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
...
<button onClick={()=>openEdit(r)} style={{width:30,height:30,borderRadius:8,border:'1px solid rgba(176,125,0,.2)',background:'rgba(176,125,0,.08)',color:C.gold,cursor:'pointer',display:'inline-flex',alignItems:'center',justifyContent:'center',margin:'0 2px',transition:'.15s'}}><svg width='12' height='12' viewBox='0 0 24 24' fill='none' stroke='#B07D00' strokeWidth='1.8'><path d='M17 3a2.83 2.83 0 114 4L7.5 20.5 2 22l1.5-5.5L17 3z'/></svg></button>
...
const isGM=user?.role?.name_ar==='المدير العام'||user?.role?.name_en==='General Manager'
// عرض الجوال (≤768px): مكوّنات الحسبات المشتركة من RenewalCalcPage (القائمة والتفاصيل بنمط تطبيق أصلي)
const MC=RenewalCalcPage.M;const isMob=MC.useIsMobile()
...
const mapTcToLegacy=(t,userMap={},branchMap={})=>{const extras=Array.isArray(t.extras)?t.extras:[];const extrasTotal=extras.reduce((s,e)=>s+Number(e?.amount||0),0);const meta={quote_no:t.quote_no,worker_name:t.worker_name,iqama_number:t.iqama_number,phone:t.phone?'+966'+t.phone:null,iqama_expiry:t.iqama_expiry_gregorian,expected_expiry:t.expected_expiry_date,duration_months:t.duration_months,duration_days:t.duration_days,expected_duration_months:t.expected_duration_months,expected_duration_days:t.expected_duration_days,billed_renewal_months:t.billed_renewal_months,government_fees:Number(t.government_fees||0),office_fee_net:Number(t.office_fee_net||0),renewal_months:t.renewal_months,transfer_only:!!t.transfer_only,change_profession:!!t.change_profession,new_occupation:t.new_occupation_name_ar,prof_change_fee:Number(t.prof_change_fee||0),office_fee:Number(t.office_fee||0),transfer_fee:Number(t.transfer_fee||0),absher_discount:Number(t.absher_discount||0),extras,warnings:(t.warnings||[]).map(w=>typeof w==='string'?w:(w?.text||''))};/* حسبة منتهية الصلاحية (>5 أيام من التسعير) بلا فاتورة = تُعتبر ملغاة */const _autoExp=!!t.priced_at&&!t.invoice_id&&!['cancelled','invoiced','completed'].includes(t.status)&&(Date.now()-new Date(t.priced_at).getTime())>(5*86400000);return{id:t.id,status:_autoExp?'cancelled':t.status,_autoExpired:_autoExp,created_at:t.created_at,updated_at:t.updated_at,deleted_at:t.deleted_at,priced_at:t.priced_at,priced_by:t.priced_by,approved_at:t.approved_at,approved_by:t.approved_by,created_by:t.created_by,transfer_fee:Number(t.transfer_fee||0),iqama_cost:Number(t.iqama_renewal_fee||0),work_permit_cost:Number(t.work_permit_fee||0),insurance_cost:Number(t.medical_fee||0),other_costs:Number(t.office_fee||0)+Number(t.prof_change_fee||0)+Number(t.late_fine_amount||0)+extrasTotal,other_costs_desc:[t.prof_change_fee>0?'تغيير المهنة':null,'رسوم المكتب',...extras.map(e=>e.name)].filter(Boolean).join(' + '),government_fees:0,total_cost:Number(t.subtotal||0),client_charge:Number(t.total_amount||0),profit:0,transfer_type:t.transfer_only?'transfer_only':'sponsorship',new_employer_name:t.worker_name,workers:null,facilities:null,cancelled_at:t.cancelled_at,cancelled_by:t.cancelled_by,cancel_reason:_autoExp?(t.cancel_reason||'انتهت صلاحية التسعيرة دون إصدار فاتورة'):t.cancel_reason,priced_user:flattenTcUser(userMap[t.priced_by]),approved_user:flattenTcUser(userMap[t.approved_by]),created_user:flattenTcUser(userMap[t.created_by]),invoiced_user:flattenTcUser(userMap[t.invoiced_by]),cancelled_user:flattenTcUser(userMap[t.cancelled_by]),notes:JSON.stringify(meta),_meta:meta,_tc:t,_officeCode:(branchMap||{})[t.branch_id]||null}}
const TC_SELECT='*'
...
useLiveRefresh(['transfer_calculation','invoices'],refetchTc)
...
// سلسلة الرجوع الذكية: تسجيل الحسبة المفتوحة كموقع حالي، فيعود إليها زر الرجوع
// الذهبي من أي صفحة قُفز إليها (فاتورة/عامل…) عبر hash إعادة الفتح أعلاه.
useEffect(()=>{if(detailsRow){const q=detailsRow._tc?.quote_no||detailsRow._meta?.quote_no||'';navSetHere({hash:'#transfer_calc?q='+encodeURIComponent(q),label:{ar:'حسبة النقل '+(q||''),en:'Transfer quote '+(q||'')}})}else navSetHere(null);return()=>navSetHere(null)},[detailsRow])
...
// خصم المكتب عند تصديق نقل الكفالة مع أرضية يختارها المُصدِّق (بدون/مبلغ ثابت/يومي = سعر اليوم × أيام التجديد).
// الأرضية = أقل رسوم مكتب بعد الخصم؛ فأقصى خصم = رسوم المكتب − الأرضية. التسعير مخصَّص لمكتب الحسبة.
const kafalaApprovalDiscount=(af)=>{
const kc=getKafalaPricingConfig(af?._branchId||null);
const discEnabled=kc.kafalaOfficeDiscountEnabled!==false;
const officeFee=Number(af?._officeFee||0),renewalMonths=Number(af?._renewalMonths||0),floorDays=renewalMonths*30;
const dailyRate=parseFloat(af?.floorDaily)||0,mode=af?.floorMode||'none';
const floor=mode==='fixed'?Math.max(0,Math.round(parseFloat(af?.floorFixed)||0)):mode==='daily'?Math.round(dailyRate*floorDays):0;
const maxDiscount=discEnabled?Math.max(0,officeFee-Math.min(floor,officeFee)):0;
const want=Math.max(0,Math.round(Number(af?.manual_discount||0)));
const applied=Math.min(want,maxDiscount);
const newTotal=Math.max(0,Number(af?._subtotal||0)-Number(af?._absher||0)-applied);
return{discEnabled,officeFee,floorDays,dailyRate,mode,floor,maxDiscount,want,applied,capped:want>maxDiscount,newTotal};
};
const submitApproval=async()=>{if(!approveForm||approveSaving)return;setApproveSaving(true);try{const fields={};['worker_name','phone','dob','nationality_id','gender','work_permit_expiry','has_notice_period','employer_consent','manual_discount','approval_note'].forEach(k=>{if(approveForm[k]!==undefined&&approveForm[k]!==null&&approveForm[k]!=='')fields[k]=approveForm[k]});fields.manual_discount=kafalaApprovalDiscount(approveForm).applied;
// sb.functions.invoke يضيف الترويسة (apikey + توكن الجلسة المحدَّث) تلقائياً — بدلاً من fetch يدوي بـ sb.supabaseUrl بلا apikey كان يفشل بـ invalid_token.
const{data,error}=await sb.functions.invoke('update-quotation',{body:{action:'approve_with_data',id:approveForm._id,fields}});
if(error){let body={};try{body=await error.context.json()}catch{}throw new Error(body.detail||body.error||error.message||'فشل التصديق')}
if(!data?.ok)throw new Error(data?.detail||data?.error||'فشل التصديق');setApproveSaved(true);await refetchTc()}catch(e){toast((lang==='ar'?'خطأ: ':'Error: ')+(e.message||'').slice(0,80))}setApproveSaving(false)}
...
const CARD_FIELDS={worker:['worker_name','iqama_number','phone','nationality_id','dob'],professional:['occupation_name_ar','sponsor_changes','change_profession','new_occupation_name_ar','hrsd_worker_status','resident_status_ar','iqama_expiry_gregorian','iqama_expiry_hijri','renewal_months'],conditions:['renewal_months','has_notice_period','employer_consent'],pricing:['transfer_fee','iqama_renewal_fee','work_permit_fee','prof_change_fee','medical_fee','office_fee','late_fine_amount','absher_discount','manual_discount']}
...
// فتح نافذة تصديق الحسبة — مشترك بين زر الحاسبي وشريط إجراءات الجوال
const tcOpenApprove=(dr)=>{const tc=dr._tc||{};const expired=tc.priced_at?(Date.now()-new Date(tc.priced_at).getTime())>(5*86400000):false;if(expired){toast(T('انتهت صلاحية التسعيرة — لا يمكن التصديق','Quote expired'));return}setApproveSaved(false);const _kc=getKafalaPricingConfig(tc.branch_id||null);setApproveForm({_id:dr.id,worker_name:(tc.worker_name&&tc.worker_name!=='—'&&tc.worker_name!=='-')?tc.worker_name:'',phone:tc.phone||'',dob:tc.dob||'',nationality_id:tc.nationality_id||((nationalities||[]).find(n=>n.name_ar===tc.nationality)?.id)||'',gender:tc.gender||'',work_permit_expiry:tc.work_permit_expiry||'',has_notice_period:tc.has_notice_period,employer_consent:tc.employer_consent,manual_discount:Number(tc.manual_discount||0),_subtotal:Number(tc.subtotal||0),_absher:Number(tc.absher_discount||0),_currentTotal:Number(tc.total_amount||0),_officeFee:Number(tc.office_fee||0),_renewalMonths:Number(tc.renewal_months||0),_branchId:tc.branch_id||null,floorMode:isGM?'none':(_kc.kafalaOfficeDiscountEnabled===false?'none':(_kc.kafalaFloorMode||'daily')),floorFixed:String((tc.exemption===false?(Number(_kc.kafalaFloorFixedNoExempt)||_kc.kafalaFloorFixed):_kc.kafalaFloorFixed)||''),floorDaily:String((tc.exemption===false?(Number(_kc.kafalaFloorDailyNoExempt)||_kc.officeDailyRate):_kc.officeDailyRate)||''),_workerName:tc.worker_name,_quoteNo:tc.quote_no})}
...
.inv-id .no-box{margin-top:7px;border:1.2px solid var(--gold-deep);background:var(--panel-2);padding:6px 14px;display:inline-block}
.inv-id .no-lbl{font-size:9.5px;color:var(--muted);letter-spacing:1.5px}
...
.office-code{display:inline-flex;align-items:center;gap:7px;margin-top:8px;padding:5px 13px;border:1.2px solid var(--gold-deep);background:var(--panel-2);color:var(--gold-ink);font-family:'Reem Kufi',sans-serif;font-weight:600;font-size:12px;letter-spacing:1px}
...
.hero{background:var(--panel-2);color:var(--ink);position:relative;padding:6mm 8mm 5.5mm;display:flex;gap:8mm;align-items:stretch;border:1.4px solid var(--gold-deep)}
...
.hero-amount .val{font-family:'Reem Kufi',sans-serif;font-weight:600;font-size:48px;line-height:1;color:var(--gold-deep);letter-spacing:.5px}
...
.hero-sub{margin-top:7px;font-size:11px;color:var(--ink-soft)}
...
.hero-fact .v.remain{color:var(--gold-deep);font-family:'Reem Kufi',sans-serif;font-size:19px}
.hero-fact .dur-note{font-size:8.5px;color:var(--muted);font-weight:400;margin-top:2px;letter-spacing:.2px}
...
.summary-card{border:1.4px solid var(--gold-deep);background:var(--panel-2);color:var(--ink);padding:5mm}
.summary-card .sum-row{display:flex;justify-content:space-between;align-items:baseline;padding:5px 0;border-bottom:1px solid rgba(20,17,11,.10)}
...
.summary-card .sum-row.remain{border-bottom:0;margin-top:2px;padding-top:7px;border-top:1.5px solid var(--gold-deep)}
.summary-card .sum-row.remain .k{color:var(--gold-ink);font-size:13px}
.summary-card .sum-row.remain .v{color:var(--gold-deep);font-family:'Reem Kufi',sans-serif;font-size:25px}
.progress{margin-top:4mm}
.progress .track{height:7px;background:#e7dcbf;position:relative;overflow:hidden}
...
.progress .cap{display:flex;justify-content:space-between;margin-top:5px;font-size:10px;color:var(--ink-soft)}
...
.notice{margin-top:4.5mm;background:var(--panel-2);color:var(--ink);padding:4mm 6mm;position:relative;border:1px solid var(--hair)}
...
.notice .ttl{font-family:'Reem Kufi',sans-serif;font-weight:600;color:var(--gold-deep);font-size:12px;letter-spacing:.5px;margin-bottom:3px;display:flex;align-items:center;gap:7px}
.notice .ar{font-size:10.5px;line-height:1.55;color:var(--ink-soft)}
...
{!detailsRow&&<>
<div className="m-hide" style={{marginBottom:22,position:'relative'}}>
...
{(()=>{
if(tcLoading)return isMob?<MC.CalcMList T={T} title={T('حسبة نقل الكفالات','Transfer Calculator')} loading/>:<TcSkeleton listRows={6}/>
...
if(isMob){/* الجوال: أرقام مختصرة + فلاتر الحالة + بطاقات مجمّعة حسب اليوم + زر عائم */
const tagsOf=r=>{const m=metaOf(r);const t=[];if(r.transfer_type==='final_exit')t.push(T('خروج نهائي','Final Exit'));else if(m.transfer_only)t.push(T('نقل فقط','Transfer Only'));else if(Number(m.renewal_months||0)>0)t.push(T(m.renewal_months+' شهر',m.renewal_months+'mo'));if(!m.transfer_only&&Number(r.work_permit_cost||0)>0)t.push(T('رخصة','Permit'));if(Number(r.insurance_cost||0)>0)t.push(T('تأمين','Insurance'));if(m.change_profession)t.push(T('تغيير مهنة','Occ. change'));return t}
const natFlagOf=r=>{const tcc=r._tc||{};const n=tcc.nationality_id?(nationalities||[]).find(x=>x.id===tcc.nationality_id):(tcc.nationality?(nationalities||[]).find(x=>x.name_ar===tcc.nationality):null);return n?.flag_url||null}
const tog=k=>()=>setListFilter(listFilter===k?'all':k)
return<MC.CalcMList T={T} title={T('حسبة نقل الكفالات','Transfer Calculator')}
stats={[{label:T('متوسط رسوم المكتب/شهر','Avg office fee/mo'),value:nm(officeStats.perMonth),unit:T('ريال','SAR'),tone:'gold',sub:T(`${officeStats.count} تسعيرة`,`${officeStats.count} quotes`)},{label:T('مسعّرة','Priced'),value:nm(sCounts.priced),tone:'#b7860b',onClick:tog('priced')},{label:T('مصدّقة','Approved'),value:nm(sCounts.approved),tone:'blue',onClick:tog('approved')},{label:T('مفوترة','Invoiced'),value:nm(sCounts.invoiced+sCounts.completed),tone:'green',onClick:tog('invoiced')},{label:T('ملغاة','Cancelled'),value:nm(sCounts.cancelled),tone:'red',onClick:tog('cancelled')}]}
search={{value:searchQ,onChange:setSearchQ,placeholder:T('اسم العامل، الإقامة، رقم التسعيرة…','Worker, iqama, quote no…')}}
chips={{value:listFilter,onChange:setListFilter,options:[{value:'all',label:T('الكل','All'),count:searched.length},{value:'priced',label:T('مسعّرة','Priced'),count:sCounts.priced},{value:'approved',label:T('مصدّقة','Approved'),count:sCounts.approved},{value:'invoiced',label:T('مفوترة','Invoiced'),count:sCounts.invoiced},{value:'cancelled',label:T('ملغاة','Cancelled'),count:sCounts.cancelled}]}}
groups={tcGroupOrder.map(k=>({key:k,label:tcDayLabel(k),sub:tcDayFull(k),today:k===todayStr,rows:tcGroups[k].map(r=>{const meta=metaOf(r);return MC.calcMCard({T,id:r.id,name:r.workers?.name_ar||meta.worker_name||r.new_employer_name||T('عامل','Worker'),quoteNo:noDash(meta.quote_no||('Q-'+String(r.id).slice(0,8).toUpperCase())),office:r._officeCode||r.priced_user?.branch?.code||r.approved_user?.branch?.code||r.created_user?.branch?.code||null,total:r.client_charge,status:r.status,stLabel:stLabel[r.status]||r.status,iqama:r.workers?.iqama_number||meta.iqama_number,tags:tagsOf(r),pricedAt:r.priced_at,createdAt:r.created_at,flag:natFlagOf(r),onClick:()=>setDetailsRow({...r,_meta:meta})})})}))}
...
fab={canPerm(user,'quotations.create')?{label:T('حسبة جديدة','New quote'),onClick:()=>onNewCalc?.()}:null}/>}
return<>
...
<span style={{fontSize:42,fontWeight:600,color:C.gold,letterSpacing:'-1.5px',lineHeight:1,fontVariantNumeric:'tabular-nums'}}>{nm(officeStats.perMonth)}</span>
</div>
...
<span style={{fontSize:13,color:C.gold,fontWeight:600,direction:'ltr',fontVariantNumeric:'tabular-nums'}}>{nm(officeStats.count)}</span>
</div>
</div>
...
<button onClick={e=>{e.stopPropagation();setViewRow({...r,_meta:meta})}} title={showValidity?(isExpired?T('انتهت — معاينة الحسبة','Expired — Quote preview'):T(`صالحة ${remDays}ي ${remHrs}س — معاينة الحسبة`,`${remDays}d ${remHrs}h — Quote preview`)):isCancelled?T('ملغاة — معاينة الحسبة','Cancelled — Quote preview'):T('معاينة الحسبة','Quote preview')} style={{position:'absolute',top:10,insetInlineEnd:10,width:28,height:28,borderRadius:'50%',background:showValidity?'transparent':(isCancelled?'rgba(192,57,43,.12)':isInvoiced?'rgba(39,160,70,.12)':'rgba(176,125,0,.12)'),border:showValidity?'none':'1px solid '+(isCancelled?'rgba(192,57,43,.35)':isInvoiced?'rgba(39,160,70,.35)':'rgba(176,125,0,.3)'),color:isCancelled?C.red:isInvoiced?C.ok:C.gold,cursor:'pointer',display:'inline-flex',alignItems:'center',justifyContent:'center',padding:0,zIndex:2,transition:'.15s'}} onMouseEnter={e=>{if(!showValidity){e.currentTarget.style.background=isCancelled?'rgba(192,57,43,.22)':isInvoiced?'rgba(39,160,70,.22)':'rgba(176,125,0,.22)';e.currentTarget.style.borderColor=isCancelled?'rgba(192,57,43,.6)':isInvoiced?'rgba(39,160,70,.6)':'rgba(176,125,0,.55)'}else{e.currentTarget.style.transform='scale(1.08)'}}} onMouseLeave={e=>{if(!showValidity){e.currentTarget.style.background=isCancelled?'rgba(192,57,43,.12)':isInvoiced?'rgba(39,160,70,.12)':'rgba(176,125,0,.12)';e.currentTarget.style.borderColor=isCancelled?'rgba(192,57,43,.35)':isInvoiced?'rgba(39,160,70,.35)':'rgba(176,125,0,.3)'}else{e.currentTarget.style.transform='scale(1)'}}}>
{showValidity?(()=>{
const total=5
...
<text x={cx} y={cy} textAnchor="middle" dominantBaseline="central" fontSize="10" fontWeight="600" fontFamily="'Cairo',sans-serif" fill={onClr}>{active}</text>
</svg>
...
return<div className='calc-stamp-cell' style={{display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,padding:'0 6px'}}>
...
return<div className='calc-stamp-cell' style={{display:'flex',flexDirection:stamps.length>=2?'column':'row',alignItems:'center',justifyContent:'center',flexShrink:0,padding:'0 6px',gap:stamps.length>=2?0:0}}>
...
if(isMob){/* الجوال: تفاصيل بملء الشاشة — نفس القيم المعروضة في الحاسبي (البنود · الملخص · الأقسام · التعليقات · الطباعة) */
const tc=dr._tc||{};const sar=v=>nm(v)+' '+T('ريال','SAR');const ren=Number(tc.renewal_months||0);const renSuffix=ren>0?T(` (${ren} شهر)`,` (${ren} mo)`):''
...
const billedMos3=billedMos>0?Math.ceil(billedMos/3)*3:0;const renIqamaSuffix=(ren>0&&billedMos3>0)?T(` (${billedMos3} شهر)`,` (${billedMos3} mo)`):''
const total=Number(tc.total_amount||dr.client_charge||0),absher=Number(tc.absher_discount||0),manualDisc=Number(tc.manual_discount||0),officeFee=Number(tc.office_fee||mm.office_fee||0),subtotalV=Number(tc.subtotal||0)
const durMo=Number(tc.expected_duration_months??mm.duration_months??tc.duration_months??0),durDays=Number(tc.expected_duration_days??mm.duration_days??tc.duration_days??0),renMo=Number(mm.renewal_months||tc.renewal_months||0)
const durLabel=(durMo>0||durDays>0)?[durMo>0?durMo+' '+T('شهر','mo'):null,durDays>0?durDays+' '+T('يوم','d'):null].filter(Boolean).join(T(' و ',' ')):(renMo>0?renMo+' '+T('شهر','mo'):null)
const natObj=tc.nationality_id?(nationalities||[]).find(x=>x.id===tc.nationality_id):(tc.nationality?(nationalities||[]).find(x=>x.name_ar===tc.nationality):null)
const ageStr=(()=>{if(!tc.dob)return null;const b=new Date(tc.dob);if(isNaN(b))return null;const t=new Date();let y=t.getFullYear()-b.getFullYear();if(t.getMonth()<b.getMonth()||(t.getMonth()===b.getMonth()&&t.getDate()<b.getDate()))y--;return y+T(' سنة',' yr')})()
const fv=k=>fieldVisible(user,'transfer_calc',k)
const canEd=card=>(canCardBtn(user,'transfer_calc',card,'edit')&&modalAllowed(user,'transfer_calc','card_edit'))?()=>openCardEdit(card):null
const expired=tc.priced_at?(Date.now()-new Date(tc.priced_at).getTime())>(5*86400000):false;const hasApprovePerm=Array.isArray(user?.perms)&&user.perms.some(p=>p.module==='quotations'&&p.action==='approve');const canApprove=isGM||hasApprovePerm
const showApprove=dr.status==='priced'&&canApprove&&modalAllowed(user,'transfer_calc','approve');const showCancel=['priced','approved'].includes(dr.status)&&canApprove&&modalAllowed(user,'transfer_calc','cancel')&&!(expired&&!tc.invoice_id)
const branch=dr._officeCode||dr.priced_user?.branch?.code||dr.approved_user?.branch?.code||dr.created_user?.branch?.code
const timeline=[...quoteNotes.map(n=>{const p=n.author?.person;return{kind:'note',ts:n.created_at,text:n.note,attachments:n.attachments,who:((lang==='en'?(p?.name_en||p?.name_ar):(p?.name_ar||p?.name_en))||'').trim().split(/\s+/).filter(Boolean).slice(0,2).join(' ')}}),...[{ts:tc.priced_at,label:T('تم التسعير','Priced'),color:'#b7860b'},{ts:tc.approved_at,label:T('تم التصديق','Approved'),color:C.blue},{ts:tc.invoiced_at,label:T('تمت الفوترة','Invoiced'),color:C.ok},{ts:tc.cancelled_at,label:T('تم الإلغاء','Cancelled'),color:C.red}].filter(m=>m.ts).map(m=>({kind:'ms',...m}))].sort((a,b)=>new Date(a.ts)-new Date(b.ts))
return<MC.CalcMDetail T={T} isAr={isAr} kind={T('حسبة نقل كفالة','Transfer quote')} onBack={()=>setDetailsRow(null)} navKind="transfer_quote" navId={mm.quote_no||''}
status={{key:dr.status,text:stLabel[dr.status]||dr.status}} quoteNo={noDash(quoteNo)} flag={natObj?.flag_url||null}
workerName={fv('worker_name')?(tc.worker_name||mm.worker_name):'—'} workerSub={fv('iqama_number')?(tc.iqama_number||mm.iqama_number):null} meta={[branch,fmtD(dr.priced_at||dr.created_at)]}
total={total}
split={cardVisible(user,'transfer_calc','financial_summary')?[{label:T('الرسوم المكتبية','Office fee'),value:nm(tc.office_fee_net??Math.max(0,officeFee-manualDisc))},{label:T('الرسوم الحكومية','Government fees'),value:nm(tc.government_fees??Math.max(0,subtotalV-officeFee-absher))}]:null}
validity={['priced','approved'].includes(dr.status)?MC.mValidity(tc.priced_at,T):null}
pricing={cardVisible(user,'transfer_calc','pricing')?{title:T('التسعيرة','Pricing'),edit:canEd('pricing'),totalLabel:T('الإجمالي النهائي','Final total'),total,
items:[Number(tc.transfer_fee||0)>0&&{label:T('رسوم نقل الكفالة','Sponsorship Transfer Fee'),value:sar(tc.transfer_fee)},Number(tc.iqama_renewal_fee||0)>0&&{label:T('تجديد الإقامة','Iqama Renewal')+renIqamaSuffix,value:sar(tc.iqama_renewal_fee)},Number(tc.late_fine_amount||0)>0&&{label:T('غرامة تأخير التجديد','Renewal Late Fine'),value:sar(tc.late_fine_amount),tone:C.red},Number(tc.work_permit_fee||0)>0&&{label:T('رخصة العمل','Work Permit')+renSuffix,value:sar(tc.work_permit_fee)},Number(tc.prof_change_fee||0)>0&&{label:T('تغيير المهنة','Change Occupation'),value:sar(tc.prof_change_fee)},Number(tc.medical_fee||0)>0&&{label:T('التأمين الطبي','Medical Insurance'),value:sar(tc.medical_fee)},...(Array.isArray(tc.extras)?tc.extras:[]).filter(e=>Number(e?.amount)>0).map(e=>({label:e.name||T('بند إضافي','Extra'),value:sar(e.amount),tone:C.blue})),officeFee>0&&{label:T('رسوم المكتب','Office Fees'),value:sar(officeFee)}],
summary:[subtotalV>0&&{label:T('الإجمالي الابتدائي','Subtotal'),value:sar(subtotalV)},absher>0&&{label:T('خصم أبشر','Absher Discount'),value:'− '+sar(absher),tone:C.ok},manualDisc>0&&{label:T('خصم المكتب','Office Discount'),value:'− '+sar(manualDisc),tone:C.ok}]}:null}
sections={[
cardVisible(user,'transfer_calc','worker')&&{title:T('العامل','Worker'),edit:canEd('worker'),rows:[fv('worker_name')&&{label:T('الإسم','Name'),value:tc.worker_name||mm.worker_name},fv('iqama_number')&&{label:T('رقم الإقامة','Iqama Number'),value:tc.iqama_number||mm.iqama_number,ltr:true},fv('phone')&&{label:T('رقم الجوال','Mobile'),value:tc.phone?'0'+tc.phone:null,ltr:true},fv('nationality_id')!==false&&{label:T('الجنسية','Nationality'),value:natObj?(isAr?natObj.name_ar:(natObj.name_en||natObj.name_ar)):tc.nationality},fv('dob')&&{label:T('تاريخ الميلاد','Date of Birth'),value:tc.dob?fmtD(tc.dob)+(ageStr?' · '+ageStr:''):null}]},
cardVisible(user,'transfer_calc','professional')&&{title:T('النقل','Transfer'),edit:canEd('professional'),rows:[fv('occupation_name_ar')&&{label:T('المهنة الحالية','Current Occupation'),value:tc.occupation_name_ar||mm.occupation},tc.change_profession&&fv('new_occupation_name_ar')&&{label:T('المهنة الجديدة','New Occupation'),value:tc.new_occupation_name_ar,tone:C.gold},fv('sponsor_changes')&&{label:T('عدد مرات نقل الخدمات','Service Transfer Count'),value:typeof tc.sponsor_changes==='number'?String(tc.sponsor_changes):null},fv('hrsd_worker_status')&&{label:T('حالة العامل','Worker Status'),value:tc.hrsd_worker_status},fv('resident_status_ar')&&{label:T('حالة المقيم','Resident Status'),value:tc.resident_status_ar},fv('iqama_expiry_gregorian')&&{label:T('انتهاء الإقامة','Iqama Expiry'),value:tc.iqama_expiry_gregorian?fmtD(tc.iqama_expiry_gregorian):null,ltr:true,tone:tc.iqama_expired===true?C.red:(tc.iqama_expired===false?C.ok:undefined)},fv('renewal_period')&&{label:T('مدة التجديد','Renewal Period'),value:(!tc.transfer_only&&tc.renew_iqama!==false&&ren>0)?ren+' '+T('شهر','mo'):(tc.transfer_only?T('نقل فقط','Transfer only'):null)},fv('exemption')&&{label:T('الإعفاء','Exemption'),value:tc.transfer_only?null:(tc.exemption===false?T('لا','No'):T('نعم','Yes'))}]},
cardVisible(user,'transfer_calc','financial_summary')&&{title:T('المدة والفاتورة','Duration & invoice'),rows:[{label:T('المدة المتوقعة','Expected Duration'),value:durLabel,tone:C.gold},{label:T('الإنتهاء المتوقع','Expected Expiry'),value:tc.expected_expiry_date?fmtD(tc.expected_expiry_date):null,ltr:true},tc.invoice_id&&(dr.status==='invoiced'||dr.status==='completed')&&{label:T('الفاتورة','Invoice'),value:detailsInvoiceNo||('INV-'+String(tc.invoice_id).slice(0,8).toUpperCase()),onClick:()=>{try{window.dispatchEvent(new CustomEvent('app-navigate-invoice',{detail:{id:tc.invoice_id}}))}catch{}}},dr.status==='cancelled'&&{label:T('سبب الإلغاء','Cancel reason'),value:dr.cancel_reason,tone:C.red}]},
]}
comments={cardVisible(user,'transfer_calc','comments')?{items:timeline,onAdd:modalAllowed(user,'transfer_calc','add_comment')?()=>setQuoteNoteModal(true):null}:null}
print={[{k:'ar',l:'عربي',cc:'sa'},{k:'en',l:'English',cc:'gb'},{k:'hi',l:'हिन्दी',cc:'in'},{k:'ur',l:'اردو',cc:'pk'},{k:'bn',l:'বাংলা',cc:'bd'}].map(o=>({...o,onClick:()=>printTransferDoc(dr,o.k)}))}
actions={[showApprove&&{label:expired?T('انتهت الصلاحية','Expired'):T('تصديق الحسبة','Approve quote'),onClick:()=>tcOpenApprove(dr),disabled:saving||expired},showCancel&&{label:T('إلغاء','Cancel'),kind:'danger',onClick:()=>{setCancelSaved(false);setCancelForm({_id:dr.id,_workerName:tc.worker_name,_quoteNo:tc.quote_no,reason:''})},disabled:saving}]}>
...
<BackButton onBack={()=>setDetailsRow(null)} label={T('رجوع','Back')} navKind="transfer_quote" navId={mm.quote_no||''} isAr={isAr} />
</div>
...
{(()=>{const nameOf=u=>u?(lang==='en'?(u.name_en||u.name_ar):u.name_ar):null;const done=s=>(['priced','approved','invoiced','completed'].includes(dr.status)&&s==='priced')||(['approved','invoiced','completed'].includes(dr.status)&&s==='approved')||(['invoiced','completed'].includes(dr.status)&&s==='invoiced');const stamps=[];if(dr.status==='cancelled'){/* الحسبة الملغاة تُبقي أختام المراحل التي تمّت فعلاً (تُقرأ من طوابعها الزمنية لا من الحالة) ثم يُضاف ختم «ملغاة». */if(dr.priced_at)stamps.push({lbl:stLabel.priced,clr:stClr.priced,by:nameOf(dr.priced_user||dr.created_user)});if(dr.approved_at)stamps.push({lbl:stLabel.approved,clr:stClr.approved,by:nameOf(dr.approved_user)});if(dr.invoiced_at||dr.invoice_id)stamps.push({lbl:stLabel.invoiced,clr:stClr.invoiced,by:nameOf(dr.invoiced_user)||detailsInvoicedBy});const cb=nameOf(dr.cancelled_user);const cd=dr.cancelled_at?(()=>{try{const d=new Date(dr.cancelled_at);return d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0')+'-'+String(d.getDate()).padStart(2,'0')}catch{return''}})():'';stamps.push({lbl:stLabel.cancelled,clr:stClr.cancelled,by:[cb,cd].filter(Boolean).join(' · ')||null})}else{if(done('priced'))stamps.push({lbl:stLabel.priced,clr:stClr.priced,by:nameOf(dr.priced_user||dr.created_user)});if(done('approved'))stamps.push({lbl:stLabel.approved,clr:stClr.approved,by:nameOf(dr.approved_user)});if(done('invoiced'))stamps.push({lbl:stLabel.invoiced,clr:stClr.invoiced,by:nameOf(dr.invoiced_user)||detailsInvoicedBy})}if(!stamps.length)return null;return<div style={{display:'flex',alignItems:'center',gap:12,flexWrap:'wrap',justifyContent:'center',flexShrink:0}}>{stamps.map((s,i)=><div key={i} style={{transform:'rotate(-5deg)',border:`2.5px solid ${s.clr}`,borderRadius:8,color:s.clr,padding:'6px 16px',display:'flex',flexDirection:'column',alignItems:'center',gap:2,opacity:.92,boxShadow:`inset 0 0 0 2px ${s.clr}33`,flexShrink:0}}><span style={{fontSize:15,fontWeight:600,letterSpacing:'1.5px',lineHeight:1.1}}>{s.lbl}</span>{s.by&&<span style={{fontSize:8,fontWeight:600,letterSpacing:'.3px',opacity:.85}}>{s.by}</span>}</div>)}</div>})()}
{(()=>{
...
{fk:'dob',label:T('تاريخ الميلاد','Date of Birth'),node:<span style={{display:'flex',alignItems:'center',justifyContent:'flex-end',gap:8,direction:'ltr'}}>{ageStr&&<span style={{display:'inline-flex',alignItems:'center',gap:5,fontSize:11,fontWeight:600,color:C.gold,background:'rgba(176,125,0,.08)',borderRadius:20,padding:'3px 10px',direction:dir,flexShrink:0}}>{ageStr}</span>}<span style={{fontSize:13,color:'var(--tx2)',fontWeight:600,direction:'ltr',fontFamily:'monospace',fontVariantNumeric:'tabular-nums'}}>{tc.dob?fmtD(tc.dob):'—'}</span></span>},
],natFlag,'worker',['worker_name','iqama_number','phone','nationality_id','nationality','dob'])}
{cardVisible(user,'transfer_calc','professional')&&gridCard(T('النقل','Transfer'),null,[
...
/* الإعفاء المحفوظ مع الحسبة — يحدّد سعر رخصة العمل ورسوم المكتب وأرضية الخصم عند التصديق */
{fk:'exemption',label:T('الإعفاء','Exemption'),value:tc.transfer_only?'—':(tc.exemption===false?T('لا','No'):T('نعم','Yes')),color:tc.transfer_only?'var(--tx5)':(tc.exemption===false?C.red:C.ok)},
],null,'professional',['occupation_name_ar','sponsor_changes','change_profession','new_occupation_name_ar','hrsd_worker_status','resident_status_ar','iqama_expiry_gregorian','iqama_expiry_hijri','renewal_months','exemption'])}
...
{officeFeeV>0&&<div style={{display:'flex',justifyContent:'space-between',padding:'5px 0'}}><span style={{fontSize:13,color:'var(--tx3)',fontWeight:600}}>{T('رسوم المكتب','Office Fees')}</span><span style={{fontSize:14,color:'var(--tx)',fontWeight:600,fontVariantNumeric:'tabular-nums'}}>{nmSar(officeFeeV)}</span></div>}
<div style={{display:'flex',justifyContent:'space-between',padding:'5px 0'}}><span style={{fontSize:13,color:C.gold,fontWeight:600}}>{T('الإجمالي الابتدائي','Subtotal')}</span><span style={{fontSize:14,color:C.gold,fontWeight:600,fontVariantNumeric:'tabular-nums'}}>{nmSar(subtotalV)}</span></div>
{Number(tc.absher_discount||0)>0&&<div style={{display:'flex',justifyContent:'space-between',padding:'5px 0'}}><span style={{fontSize:13,color:'#27a046',fontWeight:600}}>{T('خصم أبشر','Absher Discount')}</span><span style={{fontSize:14,color:'#27a046',fontWeight:600,fontVariantNumeric:'tabular-nums'}}>{nmSar(Number(tc.absher_discount||0))}</span></div>}
{Number(tc.manual_discount||0)>0&&<div style={{display:'flex',justifyContent:'space-between',padding:'5px 0'}}><span style={{fontSize:13,color:'#27a046',fontWeight:600}}>{T('خصم المكتب','Office Discount')}</span><span style={{fontSize:14,color:'#27a046',fontWeight:600,fontVariantNumeric:'tabular-nums'}}>{nmSar(Number(tc.manual_discount||0))}</span></div>}
</div>
<div style={{margin:'10px 22px 18px',display:'flex',justifyContent:'space-between',alignItems:'center',padding:'14px 18px',background:'linear-gradient(135deg, rgba(176,125,0,.15) 0%, rgba(176,125,0,.04) 100%)',borderRadius:12,border:'1px solid rgba(176,125,0,.45)',boxShadow:'inset 0 1px 0 var(--bd)'}}><span style={{color:C.gold,fontWeight:600,fontSize:14.5}}>{T('الإجمالي النهائي','Final Total')}</span><span style={{color:C.gold,fontWeight: 600,fontSize:24,fontVariantNumeric:'tabular-nums'}}>{nm(totalV)} <span style={{fontSize:12,fontWeight:600,opacity:.7}}>{T('ريال','SAR')}</span></span></div>
...
{ts:_t.cancelled_at,label:T('تم الإلغاء','Cancelled'),color:C.red},
...
{(()=>{const expired=tc.priced_at?(Date.now()-new Date(tc.priced_at).getTime())>(5*86400000):false;const hasApprovePerm=Array.isArray(user?.perms)&&user.perms.some(p=>p.module==='quotations'&&p.action==='approve');const canApprove=isGM||hasApprovePerm;const showApprove=dr.status==='priced'&&canApprove&&modalAllowed(user,'transfer_calc','approve');/* حسبة منتهية بدون فاتورة = ملغاة فعلياً ⇒ يختفي زر الإلغاء */const showCancel=['priced','approved'].includes(dr.status)&&canApprove&&modalAllowed(user,'transfer_calc','cancel')&&!(expired&&!tc.invoice_id);const blocked=expired;return<>{showApprove&&<button onClick={()=>tcOpenApprove(dr)} disabled={saving||blocked} onMouseEnter={e=>{if(!blocked)e.currentTarget.style.filter='brightness(.93)'}} onMouseLeave={e=>{e.currentTarget.style.filter='none'}} style={{height:38,padding:'0 14px',borderRadius:9,background:C.blue,border:'1px solid '+C.blue,boxShadow:'0 2px 7px rgba(0,0,0,.12), inset 0 1px 0 rgba(176,125,0,.1)',color:'#fff',cursor:blocked?'not-allowed':'pointer',display:'inline-flex',alignItems:'center',justifyContent:'center',gap:7,fontFamily:F,fontSize:12.5,fontWeight:600,opacity:blocked?.55:1,gridColumn:showCancel?'span 1':'span 2',whiteSpace:'nowrap',transition:'filter .15s ease'}}><span>{T('تصديق الحسبة','Approve Quote')}</span><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg></button>}{showCancel&&<button onClick={()=>{setCancelSaved(false);setCancelForm({_id:dr.id,_workerName:tc.worker_name,_quoteNo:tc.quote_no,reason:''})}} disabled={saving} onMouseEnter={e=>{e.currentTarget.style.filter='brightness(.93)'}} onMouseLeave={e=>{e.currentTarget.style.filter='none'}} style={{height:38,padding:'0 14px',borderRadius:9,background:C.red,border:'1px solid '+C.red,boxShadow:'0 2px 7px rgba(0,0,0,.12), inset 0 1px 0 rgba(176,125,0,.1)',color:'#fff',cursor:'pointer',display:'inline-flex',alignItems:'center',justifyContent:'center',gap:7,fontFamily:F,fontSize:12.5,fontWeight:600,gridColumn:showApprove?'span 1':'span 2',whiteSpace:'nowrap',transition:'filter .15s ease'}}><span>{T('إلغاء الحسبة','Cancel Quote')}</span><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg></button>}</>})()}
...
<button key={o.k} onClick={()=>printTransferDoc(dr,o.k)} title={T('طباعة بـ ','Print in ')+o.l} style={{height:38,padding:'0 10px',borderRadius:10,background:'rgba(176,125,0,.06)',border:'1px solid rgba(176,125,0,.22)',color:C.gold,cursor:'pointer',display:'inline-flex',alignItems:'center',justifyContent:'center',gap:8,fontFamily:F,fontSize:12,fontWeight:600,boxShadow:'0 2px 7px rgba(0,0,0,.12), inset 0 1px 0 rgba(176,125,0,.1)',transition:'.15s'}} onMouseEnter={e=>{e.currentTarget.style.background='rgba(176,125,0,.14)';e.currentTarget.style.borderColor='rgba(176,125,0,.45)'}} onMouseLeave={e=>{e.currentTarget.style.background='rgba(176,125,0,.06)';e.currentTarget.style.borderColor='rgba(176,125,0,.22)'}}>
...
return <FKModal open onClose={()=>{if(approveSaving)return;setApproveForm(null);if(approveSaved){setApproveSaved(false);setDetailsRow(null)}}} width={560} variant="edit"
 ...
 <button key={mode} type="button" onClick={()=>setF('floorMode',mode)} style={{flex:1,height:34,borderRadius:8,border:`1px solid ${f.floorMode===mode?FKC.gold:'var(--bd)'}`,background:f.floorMode===mode?'rgba(176,125,0,.1)':'var(--inputBg)',color:f.floorMode===mode?FKC.gold:'var(--tx3)',fontFamily:F,fontSize:11.5,fontWeight:600,cursor:'pointer',outline:'none'}}>{lbl}</button>
 ))}
 </div>
 ...
 <div style={{display:'flex',alignItems:'center',gap:10}}><span style={{fontSize:13,fontWeight:600,color:'var(--tx2)'}}>{T('رسوم المكتب بعد الخصم','Office fee after discount')}</span><span style={{flex:1}}/><span style={{fontSize:16,fontWeight:600,color:FKC.gold,direction:'ltr',fontVariantNumeric:'tabular-nums'}}>{nm(Math.max(0,(f._officeFee||0)-_appliedDisc))+' '+T('ريال','SAR')}</span></div>
 <div style={{display:'flex',alignItems:'center',gap:10,paddingTop:6,borderTop:'1px dashed rgba(176,125,0,.28)'}}><span style={{fontSize:11,fontWeight:600,color:'var(--tx4)'}}>{T('الإجمالي بعد الخصم','Total after discount')}</span><span style={{flex:1}}/><span style={{fontSize:12.5,fontWeight:600,color:'var(--tx3)',direction:'ltr',fontVariantNumeric:'tabular-nums'}}>{nm(Math.max(0,f._subtotal-(f._absher||0)-_appliedDisc))+' '+T('ريال','SAR')}</span></div>
 </div>
 </div>
 </ModalSection></>)}
 ]}/>
...
<button onClick={()=>{setSaveErr(null);setF({title:'',type:'client_visit',date:todayISO(),time:'09:00',client_id:'',worker_id:'',assigned_to:'',location:'',notes:'',status:'scheduled'});setPop('new')}} style={{height:40,padding:'0 18px',borderRadius:11,border:'1px solid rgba(176,125,0,.45)',background:'linear-gradient(180deg,rgba(176,125,0,.22) 0%,rgba(176,125,0,.10) 100%)',color:C.gold,fontFamily:"'Cairo',sans-serif",fontSize:12,fontWeight:600,cursor:'pointer',display:'inline-flex',alignItems:'center',gap:8,boxShadow:'0 2px 8px rgba(176,125,0,.18), inset 0 1px 0 rgba(176,125,0,.18)',transition:'.2s',flexShrink:0}}>
<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={C.gold} strokeWidth="2.5"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
{T('موعد جديد','New')}
</button>
</div>
