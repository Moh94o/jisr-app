...
import { canViewPage, can as canPerm, tabOffices, cardVisible, canCardBtn, fieldVisible, fieldEditable, modalAllowed } from './lib/permissions.js'
...
const lblS={fontSize:11,fontWeight:600,color:'rgba(255,255,255,.42)'};
const valS={fontSize:14,fontWeight:600,color:'rgba(255,255,255,.92)',marginTop:4,textAlign:'right'};
...
<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{position:'absolute',insetInlineStart:14,top:'50%',transform:'translateY(-50%)',color:'var(--tx4)'}}><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
<input className="tc-noring tc-search" value={searchQ} onChange={e=>setSearchQ(e.target.value)} placeholder={T('ابحث باسم العامل أو رقم الإقامة أو رقم التسعيرة...','Search by worker name, iqama, or quote no...')} style={{width:'100%',height:44,paddingBlock:0,paddingInlineStart:38,paddingInlineEnd:14,background:'rgba(0,0,0,.18)',border:'1px solid rgba(255,255,255,.05)',borderRadius:12,fontFamily:"'Cairo',sans-serif",fontSize:13,fontWeight:400,color:'#fff',outline:'none',direction:dir,boxSizing:'border-box',transition:'.2s'}}/>
</div>
...
{fk:'iqama_expiry_hijri',label:T('انتهاء الإقامة (هجري)','Iqama Expiry (Hijri)'),value:tc.iqama_expiry_hijri,mono:true,color:iqColor},
...
<div style={{margin:'10px 22px 18px',display:'flex',justifyContent:'space-between',alignItems:'center',padding:'14px 18px',background:'#1b1b1b',borderRadius:12,borderInlineEnd:'3px solid '+C.gold}}><span style={{color:C.gold,fontWeight:700,fontSize:14.5}}>{T('الإجمالي النهائي','Final Total')}</span><span style={{color:C.gold,fontWeight:800,fontSize:24,fontVariantNumeric:'tabular-nums'}}>{nm(totalV)} <span style={{fontSize:12,fontWeight:600}}>{T('ريال','SAR')}</span></span></div>
...
{cardVisible(user,'transfer_calc','comments')&&(<div style={{background:'linear-gradient(180deg,#2A2A2A 0%,#222 100%)',border:'1px solid rgba(255,255,255,.06)',borderRadius:16,overflow:'hidden'}}>
...
return <FKModal open onClose={()=>{if(approveSaving)return;setApproveForm(null);if(approveSaved){setApproveSaved(false);setDetailsRow(null)}}} width={560} variant="edit"
 ...
 <div style={{gridColumn:'1/-1',display:'flex',alignItems:'center',gap:10,padding:'12px 14px',borderRadius:9,background:'rgba(212,160,23,.08)',border:'1px solid rgba(212,160,23,.3)',minHeight:44}}><span style={{fontSize:13,fontWeight:600,color:'rgba(255,255,255,.7)'}}>{T('الإجمالي بعد الخصم','Total after discount')}</span><span style={{flex:1}}/><span style={{fontSize:16,fontWeight:700,color:FKC.gold,direction:'ltr',fontVariantNumeric:'tabular-nums'}}>{nm(Math.max(0,f._subtotal-(f._absher||0)-_appliedDisc))+' '+T('ريال','SAR')}</span></div>
 </div>
 </ModalSection></>)}
 ]}/>
})()}
