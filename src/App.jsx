...
<Css/>
{/* ═══ MOBILE BOTTOM NAV ═══ */}
<nav className='mob-bottom-nav' style={{paddingLeft:'max(0px, env(safe-area-inset-left))',paddingRight:'max(0px, env(safe-area-inset-right))',paddingBottom:'max(0px, env(safe-area-inset-bottom))'}}>{(()=>{
...
return[...items.map(n=>({id:n.id,l:n.l,on:isOn(n),icon:(()=>{const el=DT(isOn(n)?C.gold:'var(--sbtx2)')[n.i];return el?React.cloneElement(el,{width:22,height:22}):null})()})),
{id:'_more',l:T('المزيد','More'),on:moreOn,icon:<svg width="22" height="22" viewBox="0 0 24 24" fill="none"><circle cx="5" cy="12" r="2" fill={moreOn?C.gold:'var(--sbtx2)'}/><circle cx="12" cy="12" r="2" fill={moreOn?C.gold:'var(--sbtx2)'}/><circle cx="19" cy="12" r="2" fill={moreOn?C.gold:'var(--sbtx2)'}/></svg>}
].map(n=><div key={n.id} role="button" aria-label={n.l} onClick={()=>{n.id==='_more'?setSideOpen(!sideOpen):setPage(n.id)}} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',gap:3,padding:'6px 0',cursor:'pointer',position:'relative',minWidth:0}}>
{n.on&&n.id!=='_more'&&<div style={{position:'absolute',top:0,width:20,height:3,borderRadius:'0 0 3px 3px',background:C.gold,transition:'all .2s ease'}}/>}
