...
Promise.all([settingsP,sessionP]).then(async([settingsRes,sessionRes])=>{resolved=true;clearTimeout(timeout);const done=settingsRes.data?.setting_value==='true';setGmDone(done);const session=sessionRes.data?.session;if(!session){setView('login');return}try{const{data:u}=await client.from('users').select('*,person:persons!users_person_id_fkey(*),role:lookup_items!users_role_id_fkey(code,value_ar,value_en)').eq('auth_user_id',session.user.id).single();if(u){if(!u.is_active){await client.auth.signOut();setView('login')}else{if(u.preferred_lang)setLangPersist(u.preferred_lang);setUser(u);setView('app')}}else setView('login')}catch(e){setView('login')}}).catch(()=>{resolved=true;clearTimeout(timeout);setView('login')})},[]);const handleLogin=async(idNumber,pass)=>{const withTimeout=(promise,ms=10000)=>Promise.race([promise,new Promise((_,rej)=>setTimeout(()=>rej(new Error(lang==='ar'?'انتهت مهلة الاتصال — حاول مرة أخرى':'Connection timed out — try again')),ms))]);const{data:emailLookup,error:pe}=await withTimeout(sb.rpc('lookup_email_by_id_number',{p_id_number:idNumber}));if(pe||!emailLookup)throw new Error(lang==='ar'?'رقم الهوية غير مسجل أو الحساب غير مفعّل':'ID number not registered or account inactive');const{data,error}=await withTimeout(sb.auth.signInWithPassword({email:emailLookup,password:pass}));if(error)throw error;const{data:u,error:e2}=await withTimeout(sb.from('users').select('*,person:persons!users_person_id_fkey(*),role:lookup_items!users_role_id_fkey(code,value_ar,value_en)').eq('auth_user_id',data.user.id).single());if(e2||!u)throw new Error('User not found');if(!u.is_active){await sb.auth.signOut();throw new Error(lang==='ar'?'حسابك قيد المراجعة — يرجى انتظار موافقة المسؤول':'Your account is under review — please wait for admin approval')}sb.from('users').update({last_login_at:new Date().toISOString()}).eq('id',u.id).then(()=>{});setUser(u);if(u.preferred_lang)setLangPersist(u.preferred_lang);tt(lang==='ar'?'تم تسجيل الدخول بنجاح':'Logged in successfully');setView('app')};const handleSetup=async(form)=>{
...
function AnalogClock({size=30}){
  ...
  return(
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{display:'block',flexShrink:0}}>
      <circle cx={c} cy={c} r={r-0.5} fill="rgba(0,0,0,.25)" stroke="rgba(212,160,23,.4)" strokeWidth="1"/>
      {ticks}
...
const[reg,setReg]=useState({nationality_id:'',nationality_ar:'',name_ar:'',name_en:'',email:'',phone:'',id_number:'',pw:'',pw2:''});
...
const{error:rpcError}=await sb.rpc('register_new_user',{p_auth_user_id:auth.user.id,p_name_ar:nameAr,p_name_en:nameEn,p_id_number:idDigits,p_id_type_code:idTypeCode,p_nationality_id:reg.nationality_id,p_personal_phone:phoneFull,p_email:email});
...
setReg({nationality_id:'',nationality_ar:'',name_ar:'',name_en:'',email:'',phone:'',id_number:'',pw:'',pw2:''});
...
return(<div className='login-wrap' style={{display:'flex',height:'100vh',direction:L.dir,fontFamily:F,background:'var(--bg)',overflow:'hidden'}}><div className='login-form' style={{width:'100%',maxWidth:520,flexShrink:0,background:'var(--sf)',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'clamp(24px,5vw,44px) clamp(20px,5vw,48px)',position:'relative',boxShadow:lang==='ar'?'-28px 0 70px rgba(0,0,0,.38)':'28px 0 70px rgba(0,0,0,.38)',overflow:'hidden'}}><LangBtn L={L} switchLang={switchLang} abs/><div style={{textAlign:'center',marginBottom:'clamp(16px,4vw,32px)',width:'100%'}}><div style={{fontSize:'clamp(22px,4vw,30px)',fontWeight:900,color:'rgba(255,255,255,.98)'}}>{L.title}</div><div style={{fontSize:'clamp(11px,1.8vw,14px)',fontWeight:500,color:'rgba(255,255,255,.65)',marginTop:6}}>{L.sub}</div></div><form onSubmit={go} style={{width:'100%',display:'flex',flexDirection:'column',gap:'clamp(10px,1.8vw,16px)'}}><div><div style={{fontSize:'clamp(11px,1.6vw,13px)',fontWeight:700,color:'rgba(255,255,255,.78)',marginBottom:7}}>{L.email}</div><div style={{position:'relative'}}><span style={{position:'absolute',top:'50%',transform:'translateY(-50%)',[lang==='ar'?'right':'left']:16,pointerEvents:'none',display:'flex'}}><svg width="18" height="18" viewBox="0 0 24 24" fill="none"><rect x="2" y="5" width="20" height="14" rx="2" fill="rgba(212,160,23,.15)" stroke="rgba(212,160,23,.5)" strokeWidth="1.5"/><circle cx="9" cy="11" r="2" fill="rgba(212,160,23,.7)"/><path d="M5 17c0-1.5 1.5-3 4-3s4 1.5 4 3" stroke="rgba(212,160,23,.7)" strokeWidth="1.5"/><line x1="15" y1="10" x2="19" y2="10" stroke="rgba(212,160,23,.7)" strokeWidth="1.5"/><line x1="15" y1="13" x2="19" y2="13" stroke="rgba(212,160,23,.7)" strokeWidth="1.5"/></svg></span><input value={em} onChange={e=>{setEm(e.target.value.replace(/\D/g,'').slice(0,10));setLoginErr('')}} type="text" inputMode="numeric" maxLength={10} placeholder="1XXXXXXXXX" required style={finS}/></div></div><div><div style={{fontSize:'clamp(11px,1.6vw,13px)',fontWeight:700,color:'rgba(255,255,255,.78)',marginBottom:7}}>{L.pass}</div><div style={{position:'relative'}}><span style={{position:'absolute',top:'50%',transform:'translateY(-50%)',[lang==='ar'?'right':'left']:16,pointerEvents:'none',display:'flex'}}>{ICO.lock}</span><input value={pw} onChange={e=>{setPw(arToEn(e.target.value));setLoginErr('')}} type={showPw?'text':'password'} placeholder="······" required style={finS}/><button type="button" onClick={()=>setShowPw(!showPw)} style={{position:'absolute',top:'50%',transform:'translateY(-50%)',[lang==='ar'?'left':'right']:14,background:'none',border:'none',cursor:'pointer',display:'flex',padding:4}}>{showPw?ICO.eyeOn:ICO.eyeOff}</button></div></div><div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}><label style={{display:'flex',alignItems:'center',gap:8,cursor:'pointer'}} onClick={()=>setRem(!rem)}><div style={{width:18,height:18,borderRadius:5,border:rem?'none':'1.5px solid rgba(255,255,255,.3)',background:rem?C.gold:'transparent',display:'flex',alignItems:'center',justifyContent:'center',transition:'.2s',flexShrink:0}}>{rem&&<svg width="11" height="11" viewBox="0 0 24 24" fill="none"><path d="M5 12l5 5L19 7" stroke="#141414" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/></svg>}</div><span style={{fontSize:'clamp(10px,1.5vw,12px)',fontWeight:600,color:rem?'rgba(255,255,255,.75)':'rgba(255,255,255,.55)'}}>{L.remember}</span></label><button type="button" onClick={()=>{setForgotEmail(em);setForgotSent(false);setShowForgot(true)}} style={{fontSize:'clamp(10px,1.5vw,12px)',fontWeight:700,color:'rgba(212,160,23,.65)',textDecoration:'none',background:'none',border:'none',cursor:'pointer',fontFamily:F,padding:0}}>{L.forgot}</button></div><button type="submit" disabled={busy} style={{...goldS,height:'clamp(46px,8vw,54px)',minHeight:46,fontSize:'clamp(13px,2vw,16px)',marginTop:4,opacity:busy?.7:1,gap:10,flexShrink:0}}>{busy?<div style={{width:20,height:20,border:'2.5px solid rgba(14,14,14,.3)',borderTopColor:C.dk,borderRadius:'50%',animation:'spin .7s linear infinite'}}/>:<>{L.login} {ICO.unlock}</>}</button>{loginErr&&<div style={{background:'rgba(192,57,43,.08)',border:'1px solid rgba(192,57,43,.15)',borderRadius:10,padding:'10px 14px',display:'flex',alignItems:'center',gap:8,marginTop:4}}>
...
<button onClick={()=>{setReg({nationality_id:'',nationality_ar:'',name_ar:'',name_en:'',email:'',phone:'',id_number:'',pw:'',pw2:''});setRegHrsd({phase:'idle',sessionToken:null,captchaImage:null,captchaInput:'',attempts:0,error:null,manual:false,triedIqama:''});setRegErr({});setRegDone(false);setShowReg(true);
...
{/* Register — single-step popup like GM Setup */}
{showReg && (()=>{
const isSaudi=reg.nationality_ar==='سعودي';
const isNonSaudi=!!reg.nationality_ar&&!isSaudi;
const showNameField=isSaudi||regHrsd.manual;
const modalH=showNameField?500:440;
const inpS={width:'100%',height:'clamp(38px,5vw,42px)',padding:'0 14px',border:'1px solid transparent',borderRadius:9,fontFamily:F,fontSize:'clamp(12px,1.8vw,13px)',fontWeight:600,color:'var(--tx)',background:'rgba(0,0,0,.18)',outline:'none',textAlign:'center',boxSizing:'border-box',boxShadow:'inset 0 1px 2px rgba(0,0,0,.2)'};
const lblS={fontSize:'clamp(10px,1.5vw,12px)',fontWeight:700,color:'rgba(255,255,255,.58)',marginBottom:'clamp(3px,.5vw,5px)'};
...
const NatField=<div><div style={lblS}>{lang==='ar'?'الجنسية':'Nationality'} <span style={{color:'#e74c3c'}}>*</span></div>
<div style={{position:'relative'}}>
<div onClick={()=>setNatOpen(!natOpen)} style={{...inpS,position:'relative',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',color:reg.nationality_ar?'rgba(255,255,255,.95)':'rgba(255,255,255,.38)'}}>
...
{natOpen&&<><div onClick={()=>{setNatOpen(false);setNatSearch('')}} style={{position:'fixed',inset:0,zIndex:19}}/><div style={{position:'absolute',top:'calc(100% + 4px)',right:0,left:0,background:'#1a1a1a',border:'1px solid rgba(212,160,23,.2)',borderRadius:9,maxHeight:160,display:'flex',flexDirection:'column',zIndex:20,boxShadow:'0 8px 32px rgba(0,0,0,.6)',overflow:'hidden'}}>
<div style={{padding:'6px 8px',flexShrink:0}}>
<div style={{position:'relative'}}>
<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,.3)" strokeWidth="2" strokeLinecap="round" style={{position:'absolute',left:8,top:'50%',transform:'translateY(-50%)',pointerEvents:'none'}}><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/></svg>
<input value={natSearch} onChange={e=>setNatSearch(e.target.value)} placeholder={lang==='ar'?'بحث...':'Search...'} autoFocus style={{width:'100%',height:30,padding:'0 10px 0 28px',border:'1px solid transparent',borderRadius:7,background:'rgba(0,0,0,.18)',boxShadow:'inset 0 1px 2px rgba(0,0,0,.2)',fontFamily:F,fontSize:'clamp(10px,1.4vw,11px)',fontWeight:500,color:'var(--tx2)',outline:'none',textAlign:'center',boxSizing:'border-box'}}/>
</div></div>
<div style={{flex:1,overflowY:'auto',scrollbarWidth:'none'}}>
{filteredNats.map(n=><div key={n.ar} className="nat2-item" onClick={()=>{setReg(p=>({...p,nationality_ar:n.ar,nationality_id:n.id||'',name_ar:'',name_en:'',id_number:''}));setNatOpen(false);setNatSearch('');setRegHrsd({phase:'idle',sessionToken:null,captchaImage:null,captchaInput:'',attempts:0,error:null,manual:false,triedIqama:''})}} style={{padding:'10px 14px',fontSize:'clamp(11px,1.5vw,13px)',fontWeight:reg.nationality_ar===n.ar?700:500,color:reg.nationality_ar===n.ar?'#D4A017':'rgba(255,255,255,.7)',cursor:'pointer',textAlign:'center',borderBottom:'1px solid var(--bd2)',background:reg.nationality_ar===n.ar?'rgba(212,160,23,.06)':'transparent'}}>{lang==='ar'?n.ar:n.en}</div>)}
</div></div></>}
</div></div>;
const IdField=<div><div style={lblS}>{idLabel} <span style={{color:'#e74c3c'}}>*</span></div><input value={reg.id_number} onChange={e=>setReg(p=>({...p,id_number:e.target.value.replace(/\D/g,'').slice(0,10)}))} maxLength={10} autoComplete="off" name="reg-id" style={{...inpS,direction:'ltr'}} placeholder={idPlaceholder}/></div>;
const NameField=<div><div style={lblS}>{lang==='ar'?'الاسم':'Name'} <span style={{color:'#e74c3c'}}>*</span></div><input value={reg.name_ar} onChange={e=>setReg(p=>({...p,name_ar:e.target.value.replace(/[^\u0600-\u06FF\s]/g,'')}))} maxLength={30} autoComplete="off" name="reg-name" style={{...inpS,direction:'rtl'}} placeholder={lang==='ar'?'الاسم الأول والأخير':'First Last'}/></div>;
const EmailField=<div><div style={lblS}>{lang==='ar'?'البريد الإلكتروني':'Email'} <span style={{color:'#e74c3c'}}>*</span></div><input value={reg.email} onChange={e=>setReg(p=>({...p,email:e.target.value}))} type="email" autoComplete="off" name="reg-email-new" style={{...inpS,direction:'ltr'}} placeholder="example@jisr.sa"/></div>;
const PhoneField=<div><div style={lblS}>{lang==='ar'?'رقم الجوال':'Phone'} <span style={{color:'#e74c3c'}}>*</span></div>
<div style={{display:'flex',direction:'ltr',border:'1px solid transparent',borderRadius:9,overflow:'hidden',background:'rgba(0,0,0,.18)',boxShadow:'inset 0 1px 2px rgba(0,0,0,.2)'}}>
<div style={{height:'clamp(38px,5vw,42px)',padding:'0 10px',background:'rgba(255,255,255,.04)',display:'flex',alignItems:'center',fontSize:'clamp(10px,1.4vw,12px)',fontWeight:700,color:'#D4A017',flexShrink:0}}>+966</div>
<input value={reg.phone} onChange={e=>setReg(p=>({...p,phone:e.target.value.replace(/\D/g,'').slice(0,9)}))} maxLength={9} autoComplete="off" name="reg-phone" style={{width:'100%',height:'clamp(38px,5vw,42px)',padding:'0 12px',border:'none',background:'transparent',fontFamily:F,fontSize:'clamp(11px,1.6vw,13px)',fontWeight:600,color:'var(--tx)',outline:'none',textAlign:'left',letterSpacing:'.5px'}} placeholder="5XXXXXXXX"/>
</div></div>;
const PwField=<div><div style={lblS}>{lang==='ar'?'كلمة المرور':'Password'} <span style={{color:'#e74c3c'}}>*</span></div>
<div style={{position:'relative'}}>
<input value={reg.pw} onChange={e=>setReg(p=>({...p,pw:arToEn(e.target.value)}))} type={regShowPw?'text':'password'} autoComplete="new-password" name="reg-pw-new" style={{...inpS,paddingLeft:36,direction:'ltr'}} placeholder={lang==='ar'?'8 أحرف على الأقل':'Min 8 chars'}/>
...
const ConfirmField=<div><div style={lblS}>{lang==='ar'?'تأكيد كلمة المرور':'Confirm'} <span style={{color:'#e74c3c'}}>*</span></div>
<div style={{position:'relative'}}>
<input value={reg.pw2} onChange={e=>setReg(p=>({...p,pw2:arToEn(e.target.value)}))} type={regShowPw2?'text':'password'} autoComplete="new-password" name="reg-pw2-new" style={{...inpS,paddingLeft:36,direction:'ltr'}} placeholder={lang==='ar'?'تأكيد':'Confirm'}/>
...
return<>
<div onClick={()=>{setShowReg(false);setNatOpen(false)}} style={{position:'fixed',inset:0,background:'rgba(10,10,10,.8)',backdropFilter:'blur(8px)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:998,padding:16,fontFamily:F,direction:lang==='ar'?'rtl':'ltr'}}>
<div onClick={e=>e.stopPropagation()} style={{background:'#1a1a1a',borderRadius:18,width:'min(680px,92vw)',height:`min(${modalH}px,92vh)`,display:'flex',flexDirection:'column',boxShadow:'0 24px 60px rgba(0,0,0,.5)',border:'1px solid rgba(212,160,23,.08)'}}>
<style>{`.reg-nav-btn{height:40px;padding:0 6px;background:transparent;border:none;color:#D4A017;font-family:${F};font-size:14px;font-weight:700;cursor:pointer;display:inline-flex;align-items:center;gap:10px;transition:.2s}.reg-nav-btn .nav-ico{width:32px;height:32px;border-radius:50%;background:rgba(212,160,23,.1);display:flex;align-items:center;justify-content:center;transition:.2s;color:#D4A017}.reg-nav-btn:hover:not(:disabled) .nav-ico{background:#D4A017;color:#000}.reg-nav-btn:disabled{opacity:.4;cursor:not-allowed}.reg-nav-btn:disabled:hover .nav-ico{background:rgba(212,160,23,.1);color:#D4A017}.nat2-item{transition:background .15s,color .15s}.nat2-item:hover{background:rgba(212,160,23,.1)!important;color:#D4A017!important}`}</style>
{!regDone?<>
...
<span style={{fontSize:'clamp(14px,2vw,16px)',fontWeight:800}}>{lang==='ar'?'حساب جديد':'New Account'}</span>
</div>
<button onClick={()=>setShowReg(false)} style={{width:32,height:32,borderRadius:8,background:'rgba(255,255,255,.05)',border:'1px solid transparent',color:'rgba(255,255,255,.6)',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',padding:0}}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M18 6L6 18M6 6l12 12"/></svg></button>
</div>
<div style={{padding:'16px 22px',display:'flex',flexDirection:'column',flex:1}}>
<div style={{border:'1.5px solid rgba(212,160,23,.35)',borderRadius:12,padding:'18px 16px 16px',position:'relative',flex:1,display:'flex',flexDirection:'column'}}>
<div style={{position:'absolute',top:-9,right:14,background:'#1a1a1a',padding:'0 8px',fontSize:'clamp(11px,1.6vw,12px)',fontWeight:800,color:'#D4A017'}}>{lang==='ar'?'بيانات الحساب':'Account Info'}</div>
<div style={{display:'flex',flexDirection:'column',flex:1,marginTop:4,gap:12}}>
{showNameField?<>
<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:24}}>{NatField}{NameField}</div>
<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:24}}>{IdField}{PhoneField}</div>
<div>{EmailField}</div>
<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:24}}>{PwField}{ConfirmField}</div>
</>:<>
<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:24}}>{NatField}{IdField}</div>
<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:24}}>{PhoneField}{EmailField}</div>
<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:24}}>{PwField}{ConfirmField}</div>
</>}
</div></div></div>
<div style={{flexShrink:0,padding:'0 22px 16px',display:'flex',alignItems:'center'}}>
<span style={{flex:1}}/>
<div style={{flex:1,display:'flex',justifyContent:'center'}}>{firstErr&&<div style={{display:'flex',alignItems:'center',gap:5,fontSize:'clamp(9px,1.1vw,10px)',color:'rgba(192,57,43,.9)',fontWeight:700,pointerEvents:'none',whiteSpace:'nowrap'}}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" style={{flexShrink:0}}><circle cx="12" cy="12" r="10"/><path d="M12 8v4M12 16h.01"/></svg><span style={{overflow:'hidden',textOverflow:'ellipsis',maxWidth:240}}>{firstErr}</span></div>}</div>
...
if(showNameField){const nameAr=collapseSpaces(reg.name_ar);if(!nameAr)err.name_ar=ar?'الرجاء إدخال الاسم':'Enter name';else if(!/^[\u0600-\u06FF\s]+$/.test(nameAr))err.name_ar=ar?'حروف عربية فقط':'Arabic only';else if(nameAr.split(/\s+/).length<2)err.name_ar=ar?'الاسم الأول والأخير':'First and last name';}
...
if(isNonSaudi&&!regHrsd.manual&&!reg.name_ar)err.id_number=ar?'يجب التحقق من الاسم عبر وزارة العمل':'Verify name with MOL first';
setRegErr(err);if(Object.keys(err).length>0)return;
setRegErr({});doRegister();
}} disabled={regBusy} className="reg-nav-btn"><span>{regBusy?(lang==='ar'?'جاري...':'...'):(lang==='ar'?'تسجيل':'Create')}</span><span className="nav-ico">{regBusy?<div style={{width:14,height:14,border:'2px solid rgba(212,160,23,.3)',borderTopColor:'#D4A017',borderRadius:'50%',animation:'spin .7s linear infinite'}}/>:<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>}</span></button>
</div></div>
</>:<>
...
{regHrsd.phase!=='idle'&&regHrsd.phase!=='fetched'&&!regHrsd.manual&&<div style={{position:'fixed',inset:0,background:'rgba(10,10,10,.85)',backdropFilter:'blur(10px)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000,padding:16,fontFamily:F,direction:lang==='ar'?'rtl':'ltr'}}>
<div style={{background:'#1a1a1a',borderRadius:18,width:'min(420px,92vw)',padding:'24px',boxShadow:'0 24px 60px rgba(0,0,0,.5)',border:'1px solid rgba(212,160,23,.2)'}}>
<div style={{display:'flex',alignItems:'center',gap:10,color:'#D4A017',marginBottom:12}}>
<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><circle cx="12" cy="12" r="10" fill="rgba(212,160,23,.12)"/><path d="M9 12l2 2 4-4" strokeLinecap="round" strokeLinejoin="round"/></svg>
<span style={{fontSize:'clamp(13px,1.8vw,15px)',fontWeight:800}}>{lang==='ar'?'تحقق الاسم من وزارة العمل':'Verify name from MOL'}</span>
</div>
<div style={{fontSize:'clamp(10px,1.4vw,12px)',color:'rgba(255,255,255,.6)',marginBottom:16,lineHeight:1.7}}>{lang==='ar'?'أدخل رمز التحقق المعروض أدناه لجلب اسمك الرسمي':'Enter the captcha below to fetch your official name'}</div>
<div style={{minHeight:80,display:'flex',alignItems:'center',justifyContent:'center',background:'rgba(0,0,0,.3)',borderRadius:9,marginBottom:12,padding:8}}>
{regHrsd.phase==='loading'||regHrsd.phase==='verifying'?<div style={{display:'flex',alignItems:'center',gap:10,color:'rgba(255,255,255,.5)',fontSize:12}}><div style={{width:18,height:18,border:'2px solid rgba(212,160,23,.3)',borderTopColor:'#D4A017',borderRadius:'50%',animation:'spin .7s linear infinite'}}/><span>{lang==='ar'?(regHrsd.phase==='loading'?'جاري التحميل...':'جاري التحقق...'):(regHrsd.phase==='loading'?'Loading...':'Verifying...')}</span></div>:regHrsd.captchaImage?<img src={regHrsd.captchaImage} alt="captcha" style={{maxWidth:'100%',maxHeight:60,borderRadius:6}}/>:null}
</div>
<input value={regHrsd.captchaInput} onChange={e=>setRegHrsd(c=>({...c,captchaInput:e.target.value}))} onKeyDown={e=>{if(e.key==='Enter')regSubmitHrsd()}} placeholder={lang==='ar'?'رمز التحقق':'Captcha'} disabled={regHrsd.phase==='verifying'||regHrsd.phase==='loading'} style={{width:'100%',height:44,padding:'0 14px',border:'1px solid rgba(255,255,255,.05)',borderRadius:9,fontFamily:F,fontSize:14,fontWeight:600,color:'var(--tx)',background:'rgba(0,0,0,.18)',outline:'none',textAlign:'center',boxSizing:'border-box',marginBottom:12}}/>
{regHrsd.error&&<div style={{fontSize:'clamp(10px,1.4vw,11px)',color:'rgba(192,57,43,.85)',marginBottom:12,textAlign:'center'}}>{regHrsd.error}</div>}
<div style={{display:'flex',gap:8}}>
<button onClick={()=>setRegHrsd(c=>({...c,phase:'idle',manual:true,error:null}))} style={{flex:1,height:40,borderRadius:9,background:'transparent',border:'1px solid rgba(255,255,255,.1)',color:'rgba(255,255,255,.65)',fontFamily:F,fontSize:13,fontWeight:700,cursor:'pointer'}}>{lang==='ar'?'إدخال يدوي':'Manual entry'}</button>
<button onClick={regSubmitHrsd} disabled={regHrsd.phase==='verifying'||regHrsd.phase==='loading'||!regHrsd.captchaInput||regHrsd.captchaInput.length<3} style={{flex:1.2,height:40,borderRadius:9,background:'#D4A017',border:'none',color:'#000',fontFamily:F,fontSize:13,fontWeight:800,cursor:'pointer',opacity:(regHrsd.phase==='verifying'||regHrsd.phase==='loading'||!regHrsd.captchaInput||regHrsd.captchaInput.length<3)?.5:1}}>{lang==='ar'?'تحقق':'Verify'}</button>
</div>
</div>
</div>}
</>;
...
else if(!isWord2(f.ar))err.ar=ar?'الاسم يجب أن يتكون من كلمتين':'Name must be 2 words';
...
const ErrBadge=firstErr?<div style={{position:'absolute',left:'50%',top:'50%',transform:'translate(-50%,-50%)',display:'flex',alignItems:'center',gap:6,fontSize:'clamp(10px,1.4vw,12px)',color:'rgba(192,57,43,.85)',fontWeight:600,maxWidth:'60%',pointerEvents:'none',whiteSpace:'nowrap'}}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" style={{flexShrink:0}}><circle cx="12" cy="12" r="10"/><path d="M12 8v4M12 16h.01"/></svg><span style={{overflow:'hidden',textOverflow:'ellipsis'}}>{firstErr}</span></div>:null;

return(<><div onClick={onBack} style={{position:'fixed',inset:0,background:'rgba(10,10,10,.8)',backdropFilter:'blur(8px)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:998,padding:16,fontFamily:F,direction:L.dir}}>
<div onClick={e=>e.stopPropagation()} style={{background:'#1a1a1a',borderRadius:18,width:'min(680px,92vw)',height:'min(420px,92vh)',display:'flex',flexDirection:'column',boxShadow:'0 24px 60px rgba(0,0,0,.5)',border:'1px solid rgba(212,160,23,.08)'}}>
<style>{`.reg-nav-btn{height:40px;padding:0 6px;background:transparent;border:none;color:#D4A017;font-family:${F};font-size:14px;font-weight:700;cursor:pointer;display:inline-flex;align-items:center;gap:10px;transition:.2s}.reg-nav-btn .nav-ico{width:32px;height:32px;border-radius:50%;background:rgba(212,160,23,.1);display:flex;align-items:center;justify-content:center;transition:.2s;color:#D4A017}.reg-nav-btn:hover:not(:disabled) .nav-ico{background:#D4A017;color:#000}.reg-nav-btn:disabled{opacity:.4;cursor:not-allowed}.reg-nav-btn:disabled:hover .nav-ico{background:rgba(212,160,23,.1);color:#D4A017}`}</style>
{!done?<>
<div style={{padding:'16px 22px 10px',display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0}}>
<div style={{display:'flex',alignItems:'center',gap:10,color:'#D4A017'}}>
{ICO.bolt}
<span style={{fontSize:'clamp(14px,2vw,16px)',fontWeight:800}}>{lang==='ar'?'المدير العام':'General Manager'}</span>
</div>
<button onClick={onBack} style={{width:32,height:32,borderRadius:8,background:'rgba(255,255,255,.05)',border:'1px solid transparent',color:'rgba(255,255,255,.6)',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',padding:0}}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M18 6L6 18M6 6l12 12"/></svg></button>
</div>
<div style={{padding:'16px 22px',display:'flex',flexDirection:'column',flex:1}}>
<div style={{border:'1.5px solid rgba(212,160,23,.35)',borderRadius:12,padding:'18px 16px 16px',position:'relative',flex:1,display:'flex',flexDirection:'column'}}>
<div style={{position:'absolute',top:-9,right:14,background:'#1a1a1a',padding:'0 8px',fontSize:'clamp(11px,1.6vw,12px)',fontWeight:800,color:'#D4A017'}}>{lang==='ar'?'بيانات الحساب':'Account Info'}</div>
...
<div><div style={lblS}>{lang==='ar'?'الاسم':'Name'} <span style={{color:'#e74c3c'}}>*</span></div><input value={f.ar} onChange={e=>s('ar',e.target.value.replace(/[^\u0600-\u06FF\s]/g,''))} maxLength={30} autoComplete="off" name="setup-name" style={{...inpS,direction:'rtl'}} placeholder={lang==='ar'?'الاسم الأول والأخير':'First Last'}/></div>
<div><div style={lblS}>{lang==='ar'?'رقم الهوية الوطنية':'National ID'} <span style={{color:'#e74c3c'}}>*</span></div><input value={f.id} onChange={e=>s('id',e.target.value.replace(/\D/g,'').slice(0,10))} maxLength={10} autoComplete="off" name="setup-id" style={{...inpS,direction:'ltr'}} placeholder="1XXXXXXXXX"/></div>
</div>
<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:24}}>
<div><div style={lblS}>{lang==='ar'?'البريد الإلكتروني':'Email'} <span style={{color:'#e74c3c'}}>*</span></div><input value={f.em} onChange={e=>s('em',e.target.value)} type="email" autoComplete="off" name="setup-email-new" style={{...inpS,direction:'ltr'}} placeholder="admin@jisr.sa"/></div>
<div><div style={lblS}>{lang==='ar'?'رقم الجوال':'Phone'} <span style={{color:'#e74c3c'}}>*</span></div>
<div style={{display:'flex',direction:'ltr',border:'1px solid transparent',borderRadius:9,overflow:'hidden',background:'rgba(0,0,0,.18)',boxShadow:'inset 0 1px 2px rgba(0,0,0,.2)'}}>
<div style={{height:'clamp(38px,5vw,42px)',padding:'0 10px',background:'rgba(255,255,255,.04)',display:'flex',alignItems:'center',fontSize:'clamp(10px,1.4vw,12px)',fontWeight:700,color:'#D4A017',flexShrink:0}}>+966</div>
<input value={formatPhone(f.ph)} onChange={e=>s('ph',e.target.value.replace(/\D/g,'').slice(0,9))} maxLength={11} autoComplete="off" name="setup-phone" style={{width:'100%',height:'clamp(38px,5vw,42px)',padding:'0 12px',border:'none',background:'transparent',fontFamily:F,fontSize:'clamp(11px,1.6vw,13px)',fontWeight:600,color:'var(--tx)',outline:'none',textAlign:'left',letterSpacing:'.5px'}} placeholder="5x xxx xxxx"/>
</div></div>
</div>
<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:24}}>
<div><div style={lblS}>{lang==='ar'?'كلمة المرور':'Password'} <span style={{color:'#e74c3c'}}>*</span></div><div style={{position:'relative'}}><input value={f.pw} onChange={e=>s('pw',arToEn(e.target.value))} type={setupShowPw?'text':'password'} autoComplete="new-password" name="setup-pw-new" style={{...inpS,paddingLeft:36,direction:'ltr'}} placeholder={lang==='ar'?'8 أحرف على الأقل':'Min 8 chars'}/><button type="button" onClick={()=>setSetupShowPw(!setupShowPw)} style={{position:'absolute',top:'50%',transform:'translateY(-50%)',left:10,background:'none',border:'none',cursor:'pointer',display:'flex',padding:2}}>{setupShowPw?ICO.eyeOn:ICO.eyeOff}</button></div>{(()=>{const st=f.pw?passwordStrength(f.pw):{level:0};const c=st.level===1?'#e74c3c':st.level===2?'#f39c12':'#27a060';return<div style={{display:'flex',gap:3,marginTop:6,alignItems:'center',height:3}}>{[1,2,3].map(i=><div key={i} style={{flex:1,height:3,borderRadius:2,background:f.pw&&i<=st.level?c:'rgba(255,255,255,.06)',transition:'.2s'}}/>)}</div>})()}</div>
<div><div style={lblS}>{lang==='ar'?'تأكيد كلمة المرور':'Confirm'} <span style={{color:'#e74c3c'}}>*</span></div><div style={{position:'relative'}}><input value={f.pw2} onChange={e=>s('pw2',arToEn(e.target.value))} type={setupShowPw2?'text':'password'} autoComplete="new-password" name="setup-pw2-new" style={{...inpS,paddingLeft:36,direction:'ltr'}} placeholder={lang==='ar'?'تأكيد':'Confirm'}/><button type="button" onClick={()=>setSetupShowPw2(!setupShowPw2)} style={{position:'absolute',top:'50%',transform:'translateY(-50%)',left:10,background:'none',border:'none',cursor:'pointer',display:'flex',padding:2}}>{setupShowPw2?ICO.eyeOn:ICO.eyeOff}</button></div><div style={{height:3,marginTop:6}}/></div>
</div>
</div>
</div>
</div>
...
<span style={{flex:1}}/>
<div style={{flex:1,display:'flex',justifyContent:'center'}}>{firstErr&&<div style={{display:'flex',alignItems:'center',gap:5,fontSize:'clamp(9px,1.1vw,10px)',color:'rgba(192,57,43,.9)',fontWeight:700,pointerEvents:'none',whiteSpace:'nowrap'}}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" style={{flexShrink:0}}><circle cx="12" cy="12" r="10"/><path d="M12 8v4M12 16h.01"/></svg><span style={{overflow:'hidden',textOverflow:'ellipsis',maxWidth:240}}>{firstErr}</span></div>}</div>
<div style={{flex:1,display:'flex',justifyContent:'flex-end'}}><button onClick={go} disabled={busy} className="reg-nav-btn"><span>{busy?(lang==='ar'?'جاري...':'...'):(lang==='ar'?'تسجيل':'Create')}</span><span className="nav-ico">{busy?<div style={{width:14,height:14,border:'2px solid rgba(212,160,23,.3)',borderTopColor:'#D4A017',borderRadius:'50%',animation:'spin .7s linear infinite'}}/>:<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>}</span></button></div>
</div>
</>:<>
<div style={{padding:'36px 32px',textAlign:'center',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center'}}>
<div style={{width:72,height:72,borderRadius:'50%',background:'rgba(39,160,70,.08)',border:'2px solid rgba(39,160,70,.2)',display:'flex',alignItems:'center',justifyContent:'center',marginBottom:18}}><svg width="32" height="32" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" stroke="rgba(39,160,70,.5)" strokeWidth="1.5"/><path d="M8 12l3 3 5-6" stroke="rgba(39,160,70,.9)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/></svg></div>
<div style={{fontSize:'clamp(18px,3vw,22px)',fontWeight:900,color:'var(--tx)',marginBottom:10}}>{L.successTitle}</div>
<div style={{fontSize:'clamp(13px,1.8vw,15px)',color:'rgba(255,255,255,.85)',lineHeight:1.8,maxWidth:380}}>{lang==='ar'?`أهلاً وسهلاً ${f.ar}`:`Welcome, ${f.en||f.ar}`}</div>
</div>
</>}
</div>
</div>
...
<aside className={'dash-side'+(sideOpen?' side-open':'')} style={{width:210,background:'var(--sb)',display:'flex',flexDirection:'column',flexShrink:0,borderLeft:'0.5px solid rgba(212,160,23,.15)'}}>
{/* Logo */}
...
{/* ═══ TOPBAR ═══ */}
<header className='dash-header' style={{height:56,background:'var(--sb)',display:'flex',alignItems:'center',gap:14,padding:'0 20px',flexShrink:0,boxShadow:'0 2px 12px rgba(0,0,0,.12)',borderBottom:'0.5px solid rgba(212,160,23,.15)',minWidth:0}}>
...
<div onClick={()=>{const nat=natCache;const natLabel=nat?(lang==='en'?nat.name_en||nat.name_ar:nat.name_ar)||'':'';setShowProfile(true);setProfileTab('info');setProfileErr({});setProfileData({phone:user.person?.personal_phone||'',email:user.person?.email||'',name_ar:user.person?.name_ar||'',name_en:user.person?.name_en||'',id_number:user.person?.id_number||'',nationality:natLabel,nationality_code:nat?.code||'',nationality_flag:nat?.flag_url||'',avatar_url:user.person?.avatar_url||'',_origEmail:user.person?.email||''});
// Fallback: fetch nationality only if cache wasn't ready yet
if(!nat){(async()=>{const natId=user.person?.nationality_id;if(natId){const{data}=await sb.from('nationalities').select('id,name_ar,name_en,code,flag_url').eq('id',natId).maybeSingle();if(data){setNatCache(data);const lbl=lang==='en'?data.name_en||data.name_ar:data.name_ar;setProfileData(p=>({...p,nationality:lbl||'',nationality_code:data.code||'',nationality_flag:data.flag_url||''}))}}})();}}} title={(lang==='en'?(user?.person?.name_en||user?.person?.name_ar):(user?.person?.name_ar||user?.person?.name_en))||(lang==='ar'?'الملف الشخصي':'Profile')} style={{width:34,height:34,borderRadius:10,background:'linear-gradient(180deg,rgba(212,160,23,.10) 0%,rgba(212,160,23,.02) 100%)',border:'1px solid rgba(212,160,23,.22)',boxShadow:'inset 0 1px 0 rgba(255,255,255,.05),inset 0 -1px 1px rgba(0,0,0,.25),0 1px 3px rgba(0,0,0,.2)',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',transition:'.2s',padding:0,overflow:'hidden',position:'relative'}} onMouseEnter={e=>{e.currentTarget.style.background='linear-gradient(180deg,rgba(212,160,23,.22) 0%,rgba(212,160,23,.08) 100%)';e.currentTarget.style.borderColor='rgba(212,160,23,.5)';e.currentTarget.style.boxShadow='inset 0 1px 0 rgba(255,255,255,.08),inset 0 -1px 1px rgba(0,0,0,.25),0 2px 8px rgba(212,160,23,.18)';const av=e.currentTarget.querySelector('[data-avatar]');if(av)av.style.borderColor='rgba(212,160,23,.8)';const s=e.currentTarget.querySelector('svg');if(s)s.style.stroke=C.gold}} onMouseLeave={e=>{e.currentTarget.style.background='linear-gradient(180deg,rgba(212,160,23,.10) 0%,rgba(212,160,23,.02) 100%)';e.currentTarget.style.borderColor='rgba(212,160,23,.22)';e.currentTarget.style.boxShadow='inset 0 1px 0 rgba(255,255,255,.05),inset 0 -1px 1px rgba(0,0,0,.25),0 1px 3px rgba(0,0,0,.2)';const av=e.currentTarget.querySelector('[data-avatar]');if(av)av.style.borderColor='rgba(212,160,23,.45)';const s=e.currentTarget.querySelector('svg');if(s)s.style.stroke='rgba(255,255,255,.6)'}}>
{(user?.avatar_url||user?.person?.avatar_url)?<img data-avatar src={user.avatar_url||user.person.avatar_url} width="24" height="24" style={{borderRadius:'50%',objectFit:'cover',flexShrink:0,border:'1px solid rgba(212,160,23,.45)',transition:'border-color .2s',boxShadow:'0 1px 2px rgba(0,0,0,.3)'}} alt=""/>:<div data-avatar style={{width:24,height:24,borderRadius:'50%',background:'radial-gradient(circle at 30% 30%,rgba(255,255,255,.08),rgba(0,0,0,.15))',border:'1px solid rgba(212,160,23,.45)',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,transition:'border-color .2s',boxShadow:'inset 0 1px 1px rgba(0,0,0,.2)'}}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,.6)" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" style={{transition:'stroke .2s'}}><circle cx="12" cy="8" r="4"/><path d="M4 21v-1a6 6 0 0116 0v1"/></svg></div>}
</div>
...
{/* ═══ Content ═══ */}
<div className='dash-content' style={{flex:1,overflowY:'auto',overflowX:'hidden',padding:32,msOverflowStyle:'none',scrollbarWidth:'none',WebkitOverflowScrolling:'touch'}}>
...
try{const{error}=await sb.from('users').update({avatar_url:dataUrl,updated_at:new Date().toISOString()}).eq('id',user.id);if(error)throw error;user.avatar_url=dataUrl;setProfileData(p=>({...p,avatar_url:dataUrl}));tt(T2('تم تحديث الصورة','Avatar updated'))}catch(err){tt('خطأ: '+err.message)}
...
{(profileData.avatar_url||user?.avatar_url||user?.person?.avatar_url)?<img src={profileData.avatar_url||user.avatar_url||user.person.avatar_url} width="40" height="40" style={{borderRadius:'50%',objectFit:'cover',border:'1.5px solid rgba(212,160,23,.5)',display:'block'}} alt=""/>:<div style={{width:40,height:40,borderRadius:'50%',background:'rgba(255,255,255,.04)',border:'1.5px solid rgba(212,160,23,.5)',display:'flex',alignItems:'center',justifyContent:'center'}}><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,.55)" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="8" r="4"/><path d="M4 21v-1a6 6 0 0116 0v1"/></svg></div>}
<div style={{position:'absolute',bottom:-2,right:-2,width:16,height:16,borderRadius:'50%',background:C.gold,border:'2px solid #1a1a1a',display:'flex',alignItems:'center',justifyContent:'center'}}><svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="#000" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><path d="M12 5v14M5 12h14"/></svg></div>
</div>
<div style={{flex:1,minWidth:0}}>
<div style={{fontSize:14,fontWeight:800,color:'rgba(255,255,255,.92)',fontFamily:"'Noto Kufi Arabic','Cairo',sans-serif",lineHeight:1.2}}>{user?.role?.value_ar||T2('المدير العام','General Manager')}</div>
{user?.branch_id&&dashBranches.find(b=>b.id===user.branch_id)?<div style={{fontSize:10,color:C.gold,marginTop:6,fontWeight:600,display:'flex',alignItems:'center',gap:5}}><span style={{color:'rgba(212,160,23,.55)',fontSize:9}}>•</span><span style={{color:'rgba(255,255,255,.5)',fontWeight:500}}>{dashBranches.find(b=>b.id===user.branch_id)?.name_ar}</span></div>:null}
</div>
...
<div style={{fontSize:12,color:'var(--tx4)',marginTop:12,lineHeight:1.5}}>{T('حساب تكاليف نقل خدمات العمال وإصدار التسعيرات ومتابعة حالتها','Worker transfer cost calculation, quote issuance and status tracking')}</div>
</div>
{(()=>{
...
return<>
{/* ═══ KPI dashboard cards ═══ */}
<div style={{display:'grid',gridTemplateColumns:'minmax(0,2.6fr) minmax(0,1fr)',gap:14,marginBottom:20}}>
...
<div style={{fontSize:18,fontWeight:600,color:s.c,letterSpacing:'-.3px',direction:'ltr',lineHeight:1}}>{s.v}</div>
</div>
<div style={{fontSize:10.5,color:'var(--tx2)',fontWeight:600}}>{s.l}</div>
</div>))}
...
<span style={{fontSize:12.5,fontWeight:600,color:'var(--tx2)',letterSpacing:'.1px'}}>{T('متوسط رسوم المكتب','Avg office fee')}</span>
{/* Hero amount */}
<div style={{display:'flex',alignItems:'baseline',gap:8,marginTop:2}}>
<span style={{fontSize:56,fontWeight:600,color:C.gold,letterSpacing:'-1.4px',lineHeight:1,textShadow:`0 0 22px ${C.gold}33`,direction:'ltr'}}>{nm(officeStats.perMonth)}</span>
<span style={{fontSize:16,fontWeight:600,color:C.gold,opacity:.75}}>{T('ريال','SAR')}</span>
</div>
<div style={{fontSize:11.5,fontWeight:600,color:'var(--tx4)',letterSpacing:'.3px'}}>{T('متوسط شهري','Monthly average')}</div>
</div>
</div>
...
input.tc-search.tc-search.tc-search.tc-search:focus{border-color:rgba(212,160,23,.6)!important;box-shadow:none!important}
...
<div style={{display:'flex',alignItems:'center',gap:10,marginTop:20,marginBottom:20,flexWrap:'wrap'}}>
...
<input className="tc-noring tc-search" value={searchQ} onChange={e=>setSearchQ(e.target.value)} placeholder={T('ابحث باسم العامل أو رقم الإقامة أو رقم التسعيرة...','Search by worker name, iqama, or quote no...')} style={{width:'100%',height:38,padding:'0 14px 0 36px',background:'#141414',border:'1px solid rgba(255,255,255,.06)',borderRadius:10,fontFamily:"'Cairo',sans-serif",fontSize:12,fontWeight:400,color:'var(--tx)',outline:'none',direction:lang==='en'?'ltr':'rtl',boxSizing:'border-box'}}/>
</div>
<button onClick={()=>setAdvOpen(o=>!o)} style={{height:38,padding:'0 14px',borderRadius:10,border:'1px solid '+(advOpen||Object.values(advFilter).some(Boolean)?'rgba(212,160,23,.45)':'rgba(255,255,255,.06)'),background:advOpen||Object.values(advFilter).some(Boolean)?'rgba(212,160,23,.1)':'#141414',color:advOpen||Object.values(advFilter).some(Boolean)?C.gold:'rgba(255,255,255,.7)',fontFamily:"'Cairo',sans-serif",fontSize:12,fontWeight:400,cursor:'pointer',display:'inline-flex',alignItems:'center',gap:6,flexShrink:0}}>
{T('بحث متقدم','Advanced Search')}
<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="4" y1="21" x2="4" y2="14"/><line x1="4" y1="10" x2="4" y2="3"/><line x1="12" y1="21" x2="12" y2="12"/><line x1="12" y1="8" x2="12" y2="3"/><line x1="20" y1="21" x2="20" y2="16"/><line x1="20" y1="12" x2="20" y2="3"/><line x1="1" y1="14" x2="7" y2="14"/><line x1="9" y1="8" x2="15" y2="8"/><line x1="17" y1="16" x2="23" y2="16"/></svg>
{Object.values(advFilter).filter(Boolean).length>0&&<span style={{background:C.gold,color:'#000',fontSize:9,fontWeight:600,padding:'1px 6px',borderRadius:999}}>{Object.values(advFilter).filter(Boolean).length}</span>}
</button>
</div>
{advOpen&&<div style={{marginBottom:14,padding:'14px 16px',background:'var(--bg)',border:'1px solid rgba(212,160,23,.18)',borderRadius:12,display:'flex',justifyContent:'flex-end'}}>
<button onClick={()=>setAdvFilter({from:'',to:'',service:'',employee:'',officeMin:'',officeMax:''})} style={{height:36,padding:'0 16px',borderRadius:8,border:'1px solid rgba(192,57,43,.25)',background:'rgba(192,57,43,.06)',color:C.red,fontFamily:"'Cairo',sans-serif",fontSize:11,fontWeight:600,cursor:'pointer'}}>{T('مسح الفلاتر','Clear filters')}</button>
</div>}
...
return<div key={r.id} onClick={()=>{setDetailsRow({...r,_meta:meta})}} style={{background:'linear-gradient(180deg,rgba(0,0,0,.3) 0%,rgba(0,0,0,.2) 100%)',borderRadius:16,overflow:'visible',transition:'.25s cubic-bezier(.4,0,.2,1)',border:'1px solid '+(isExpired?'rgba(192,57,43,.35)':'rgba(255,255,255,.07)'),position:'relative',cursor:'pointer',padding:'18px 22px',display:'grid',gridTemplateColumns:'1fr auto auto',gap:22,alignItems:'center',opacity:isExpired?.7:1}}
...
<span style={{fontSize:12,color:r.status==='invoiced'||r.status==='completed'?C.ok:C.gold,fontWeight:600,fontFamily:"'JetBrains Mono',monospace",direction:'ltr',letterSpacing:'.4px'}}>{invoiceNo||quoteNo}</span>
...
<div style={{lineHeight:1,fontVariantNumeric:'tabular-nums',textAlign:'center'}}><bdi style={{fontSize:28,fontWeight:600,color:C.gold,letterSpacing:'-.6px'}}>{nm(Math.round(Number(cc)||0))}</bdi> <span style={{fontSize:13,fontWeight:600,color:C.gold,opacity:.7,letterSpacing:'.3px'}}>{T('ريال','SAR')}</span></div>
</div>
</>})()}
</div>})}</div></div>})}</div>}
</>})()}
...
const sec=(title,rows,icon)=>{const filtered=rows.filter(Boolean);if(!filtered.length)return null;return<div style={{background:'linear-gradient(180deg,rgba(0,0,0,.3) 0%,rgba(0,0,0,.2) 100%)',borderRadius:16,border:'1px solid rgba(255,255,255,.07)',padding:'18px 22px',position:'relative',marginTop:14}}>
<div style={{display:'flex',alignItems:'center',gap:8,marginBottom:14,paddingBottom:12,borderBottom:'1px solid rgba(255,255,255,.05)'}}>
<span style={{display:'inline-flex',alignItems:'center',justifyContent:'center',width:28,height:28,borderRadius:8,background:'rgba(212,160,23,.1)',color:C.gold}}>{icon||icoNote}</span>
<span style={{fontSize:13.5,fontWeight:600,color:'rgba(255,255,255,.93)',letterSpacing:'-.2px'}}>{title}</span>
</div>
...
const badgeLabel=srcKey==='employee'&&editorName?editorName:srcMeta.l;const tooltip=auditEntries.length?(modified?T(`عُدّل: ${formatAuditValue(auditEntries[0].new_value)} ← ${formatAuditValue(latest.new_value)}${editorName?`\nبواسطة: ${editorName}`:''}\nالمصدر: ${srcMeta.l}`,`Modified: ${formatAuditValue(auditEntries[0].new_value)} → ${formatAuditValue(latest.new_value)}${editorName?`\nBy: ${editorName}`:''}\nSource: ${srcMeta.l}`):T(`المصدر: ${srcMeta.l}${editorName?`\nبواسطة: ${editorName}`:''}`,`Source: ${srcMeta.l}${editorName?`\nBy: ${editorName}`:''}`)):'';return<div key={i} title={tooltip} style={{background:row[3]||'rgba(255,255,255,.02)',borderRadius:10,padding:'10px 14px',border:'1px solid '+(row[4]||'rgba(255,255,255,.04)'),position:'relative'}}>
<div style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:6,marginBottom:5}}>
<div style={{fontSize:10,color:'var(--tx5)',fontWeight:600,letterSpacing:'.3px'}}>{row[0]}</div>
{fieldKey&&auditEntries.length>0&&<span style={{display:'inline-flex',alignItems:'center',gap:3,padding:'1px 5px',borderRadius:4,fontSize:8.5,fontWeight:600,background:srcMeta.c+'18',color:srcMeta.c,letterSpacing:'.2px',whiteSpace:'nowrap',maxWidth:120,overflow:'hidden',textOverflow:'ellipsis'}}>{modified?'✎ ':''}{badgeLabel}</span>}
</div>
...
<div>
<div style={{fontSize:24,fontWeight:600,color:'rgba(255,255,255,.93)',letterSpacing:'-.3px'}}>{T('تفاصيل الحسبة','Full Details')}</div>
...
<div title={expired?T('انتهت الصلاحية','Expired'):T(`متبقي ${remDays} يوم و ${remHrs} ساعة`,`${remDays}d ${remHrs}h left`)} style={{width:78,height:64,borderRadius:12,background:'rgba(0,0,0,.25)',border:'1px solid rgba(255,255,255,.06)',padding:8,display:'flex',flexDirection:'column',justifyContent:'center',alignItems:'center',gap:6,boxSizing:'border-box'}}>
...
{(()=>{const tc=dr._tc||{};const muqeemOk=!!tc.muqeem_fetched_at;const chiOk=!!tc.chi_verified_at;const hrsdOk=!!tc.hrsd_verified_at;const Pill=({ok,label,clr})=><span style={{display:'inline-flex',alignItems:'center',gap:6,padding:'6px 12px',borderRadius:8,fontSize:11,fontWeight:600,background:ok?(clr+'14'):'rgba(255,255,255,.04)',border:`1px solid ${ok?clr+'55':'rgba(255,255,255,.08)'}`,color:ok?clr:'var(--tx5)'}}>{ok?<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>:<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>}<span>{label}</span></span>;return<div style={{display:'flex',gap:8,flexWrap:'wrap',marginBottom:14,alignItems:'center',direction:'rtl'}}><span style={{fontSize:10,color:'var(--tx5)',fontWeight:600,marginInlineEnd:4}}>{T('مصادر البيانات:','Sources:')}</span><Pill ok={muqeemOk} label={T('مقيم','Muqeem')} clr="#F47B20"/><Pill ok={chiOk} label={T('الضمان الصحي','CHI')} clr="#5188C9"/><Pill ok={hrsdOk} label={T('وزارة العمل','HRSD')} clr="#2DB174"/>
<span style={{flex:1,minWidth:8}}/>
<span style={{display:'inline-flex',alignItems:'center',gap:6,fontSize:10,color:'var(--tx5)',fontWeight:600,marginInlineEnd:4}}><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="6 9 6 2 18 2 18 9"/><path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"/><rect x="6" y="14" width="12" height="8"/></svg>{T('طباعة:','Print:')}</span>
{[{k:'ar',l:'عربي',cc:'sa'},{k:'en',l:'English',cc:'gb'},{k:'bn',l:'বাংলা',cc:'bd'},{k:'ur',l:'اُردُو',cc:'pk'}].map(o=>
<button key={o.k} onClick={()=>printCalc(dr,o.k)} title={T('طباعة بـ ','Print in ')+o.l} style={{height:32,padding:'0 10px',borderRadius:8,background:'rgba(212,160,23,.06)',border:'1px solid rgba(212,160,23,.25)',color:C.gold,fontFamily:"'Cairo',sans-serif",fontSize:11,fontWeight:600,cursor:'pointer',transition:'.12s',display:'inline-flex',alignItems:'center',justifyContent:'center',gap:6}} onMouseEnter={e=>{e.currentTarget.style.background='rgba(212,160,23,.18)';e.currentTarget.style.borderColor=C.gold}} onMouseLeave={e=>{e.currentTarget.style.background='rgba(212,160,23,.06)';e.currentTarget.style.borderColor='rgba(212,160,23,.25)'}}>
...
{sec(T('التأمين الصحي','Health Insurance'),[
...
{/* Bottom action row — approve / invoice CTA after the user has scrolled through everything */}
{(dr.status==='priced'||dr.status==='approved')&&(()=>{const tc=dr._tc||{};const reqMissing=!tc.nationality_id||!tc.work_permit_expiry;const expired=tc.priced_at?(Date.now()-new Date(tc.priced_at).getTime())>(5*86400000):false;const canApprove=user?.is_all_branches||user?.quotationPerms?.approve;const approveBlocked=reqMissing||expired;const reason=expired?T('انتهت صلاحية التسعيرة','Quote expired'):(reqMissing?T('املأ الجنسية وانتهاء رخصة العمل قبل التصديق','Fill nationality and work permit expiry before approving'):'');return<div style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:16,marginTop:18,padding:'14px 18px',borderRadius:14,background:'linear-gradient(180deg,rgba(0,0,0,.35) 0%,rgba(0,0,0,.18) 100%)',border:'1px solid rgba(255,255,255,.06)'}}>
<div style={{flex:1,fontSize:11.5,color:'var(--tx5)',fontWeight:600,lineHeight:1.6}}>{dr.status==='priced'?(approveBlocked?<span style={{color:C.red}}>{reason}</span>:T('راجع كل البيانات أعلاه ثم اضغط تصديق الحسبة','Review all data above before approving the quote')):T('بإمكانك إصدار فاتورة لهذه التسعيرة','You can issue an invoice for this approved quote')}</div>
{dr.status==='priced'&&canApprove&&<button onClick={()=>{if(expired){toast(T('انتهت صلاحية التسعيرة — لا يمكن التصديق','Quote expired — cannot approve'));return}setApproveForm({_id:dr.id,worker_name:tc.worker_name||'',phone:tc.phone||'',dob:tc.dob||'',nationality_id:tc.nationality_id||'',gender:tc.gender||'',work_permit_expiry:tc.work_permit_expiry||'',has_notice_period:tc.has_notice_period,employer_consent:tc.employer_consent,manual_discount:Number(tc.manual_discount||0),_subtotal:Number(tc.subtotal||0),_currentTotal:Number(tc.total_amount||0),_workerName:tc.worker_name,_quoteNo:tc.quote_no})}} disabled={saving||expired} title={expired?T('انتهت صلاحية التسعيرة','Quote expired'):T('فتح نافذة التصديق','Open approval form')} style={{height:42,padding:'0 22px',borderRadius:10,border:'none',background:expired?'rgba(192,57,43,.18)':C.blue,color:expired?C.red:'#fff',fontFamily:F,fontSize:13,fontWeight:600,cursor:(saving||expired)?'not-allowed':'pointer',display:'inline-flex',alignItems:'center',gap:8,opacity:(saving||expired)?.65:1,transition:'.15s'}}>
...
{dr.status==='approved'&&<button onClick={async()=>{await changeStatus(dr.id,'invoiced');setDetailsRow(null)}} disabled={saving} style={{height:42,padding:'0 22px',borderRadius:10,border:'none',background:C.ok,color:'#fff',fontFamily:F,fontSize:13,fontWeight:600,cursor:saving?'not-allowed':'pointer',display:'inline-flex',alignItems:'center',gap:8,opacity:saving?.65:1,transition:'.15s'}}>
...
function LangBtn({L,switchLang,abs}){const isToEn=L.otherLang==='English';const s=abs?{position:'absolute',top:18,[isToEn?'left':'right']:18,zIndex:10}:{};return<div onClick={switchLang} style={{...s,display:'flex',alignItems:'center',gap:8,padding:'0 13px',height:34,background:'rgba(255,255,255,.05)',backdropFilter:'blur(10px)',WebkitBackdropFilter:'blur(10px)',border:'1px solid rgba(255,255,255,.1)',borderRadius:10,cursor:'pointer',fontFamily:F,direction:'ltr'}}><img src={isToEn?'https://flagcdn.com/w40/gb.png':'https://flagcdn.com/w40/sa.png'} width="18" height="13" style={{borderRadius:2,objectFit:'cover'}} alt=""/><span style={{fontSize:11,fontWeight:700,color:'rgba(255,255,255,.85)',letterSpacing:.5}}>{isToEn?'EN':'AR'}</span></div>}
