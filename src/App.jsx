...
client.from('system_settings').select('setting_key,setting_value').eq('setting_key','gm_setup_complete').single().then(({data,error})=>{if(error){setView('login');return}const done=data?.setting_value==='true';setGmDone(done);client.auth.getSession().then(({data:{session}})=>{if(session){client.from('users').select('*,roles:role_id(name_ar,name_en,color)').eq('auth_id',session.user.id).single().then(({data:u})=>{if(u){if(u.preferred_lang)setLangPersist(u.preferred_lang);setUser(u);setView('app')}else setView('login')})}else setView('login')})})},[]);const handleLogin=async(email,pass)=>{const{data,error}=await sb.auth.signInWithPassword({email,password:pass});if(error)throw error;const{data:u,error:e2}=await sb.from('users').select('*,roles:role_id(name_ar,name_en,color)').eq('auth_id',data.user.id).single();if(e2||!u)throw new Error('User not found');if(!u.is_active){await sb.auth.signOut();throw new Error(lang==='ar'?'حسابك قيد المراجعة — يرجى انتظار موافقة المسؤول':'Your account is under review — please wait for admin approval')}sb.from('users').update({last_login_at:new Date().toISOString()}).eq('id',u.id).then(()=>{});setUser(u);if(u.preferred_lang)setLangPersist(u.preferred_lang);tt(lang==='ar'?'تم تسجيل الدخول بنجاح':'Logged in successfully');setView('app')};const handleSetup=async(form)=>{
...
setBusy(true);try{await onLogin(em,pw)}catch(err){const msg=err.message?.includes('Invalid')||err.message?.includes('invalid')?(lang==='ar'?'تعذّر تسجيل الدخول — تحقق من البريد وكلمة المرور':'Login failed — check email and password'):err.message||'Error';setLoginErr(msg)}setBusy(false)};
...
{id:'tasks_regular',l:T('اعتيادية','Regular'),i:'calendar'},
{id:'tasks_periodic',l:T('دورية','Periodic'),i:'calendar'},
{id:'sla_monitor',l:T('مراقبة SLA','SLA Monitor'),i:'chart'},
{id:'calendar_unified',l:T('التقويم','Calendar'),i:'calendar'},
...
{(()=>{
const crumbs=[pgTitle]
if(pg==='settings'){
const tabMap={general:T('الإعدادات العامة','General Settings'),services:T('الخدمات','Services'),categories:T('الخانات والعناصر','Lists & Items'),countries_nat:T('الدول والجنسيات والسفارات','Countries & Nationalities'),regions_cities:T('المناطق والمدن','Regions & Cities'),documents:T('الوثائق','Documents')}
crumbs.push(tabMap[sTabInfo.tab]||'')
if(sTabInfo.tab==='services'&&sTabInfo.svcSubTab==='templates'){
crumbs.push(T('قوالب المعاملات','Transaction Templates'))
}
}
if(pg==='admin'||pg.startsWith('admin_')){
const tabMap={branches:T('المكاتب','Offices'),bank_accounts:T('الحسابات البنكية','Bank Accounts'),users:T('الموظفين','Employees'),roles:T('الأدوار والصلاحيات','Roles & Permissions')}
crumbs.push(tabMap[sTabInfo.tab]||'')
}
return crumbs.filter(Boolean).map((c,i,arr)=><React.Fragment key={i}>
{i>0&&<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,.2)" strokeWidth="2"><polyline points="15 18 9 12 15 6"/></svg>}
<span style={{fontSize:i===0?16:12,fontWeight:i===arr.length-1?700:500,color:i===arr.length-1?'rgba(255,255,255,.93)':'rgba(255,255,255,.38)'}}>{c}</span>
</React.Fragment>)
})()}
</div>
<div style={{flex:1}}/>
<div style={{display:'flex',alignItems:'center',gap:4}}>
{/* التاريخ والوقت — بدون صندوق */}
<div className='topbar-datetime' style={{display:'flex',alignItems:'center',gap:5,whiteSpace:'nowrap',padding:'0 8px'}}>
...
{/* تحديث أسبوعي */}
<div className='topbar-weekly' onClick={async()=>{try{const{data}=await sb.rpc('weekly_update');tt(T('تم التحديث الأسبوعي ✓ — المهام والإحصائيات','Weekly update done ✓'));loadStats()}catch(e){tt(T('خطأ في التحديث','Update error'))}}} title={T('تحديث أسبوعي — توليد مهام + تحديث إحصائيات','Weekly Update — Tasks + Stats refresh')} style={{height:34,padding:'0 12px',borderRadius:8,background:'rgba(39,160,70,.08)',border:'1px solid rgba(39,160,70,.12)',display:'flex',alignItems:'center',gap:5,cursor:'pointer',transition:'.15s'}}>
<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="rgba(39,160,70,.6)" strokeWidth="2"><path d="M23 4v6h-6M1 20v-6h6"/><path d="M3.51 9a9 9 0 0114.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0020.49 15"/></svg>
<span style={{fontSize:10,fontWeight:700,color:'rgba(39,160,70,.7)'}}>{T('تحديث أسبوعي','Weekly Update')}</span>
{lastWeeklyUpdate&&<span style={{fontSize:8,color:'rgba(39,160,70,.4)',marginRight:2}}>{Math.floor((Date.now()-lastWeeklyUpdate.getTime())/86400000)}{T('ي','d')}</span>}
</div>
{/* الإشعارات */}
<div style={{position:'relative'}}>
...
<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke={searchOpen?'rgba(201,168,76,.5)':'rgba(255,255,255,.25)'} strokeWidth="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>
<input value={searchQ} onChange={e=>{setSearchQ(e.target.value);setSearchOpen(true)}} onFocus={()=>searchQ.length>=2&&setSearchOpen(true)} placeholder={lang==='ar'?'بحث في المنشآت والعمال والعملاء ...':'Search facilities, workers, clients ...'} style={{background:'none',border:'none',outline:'none',color:'var(--tx2)',fontFamily:'inherit',fontSize:11,width:'100%'}}/>
...
{(pg==='transactions_internal'||pg==='transactions_external')&&<TransactionsPage sb={sb} toast={tt} user={user} lang={lang} onTabChange={setSTabInfo}/>}
...
{/* ═══ PROFILE MODAL ═══ */}
{showProfile&&profileData&&<div onClick={()=>setShowProfile(false)} style={{position:'fixed',inset:0,background:'rgba(0,0,0,.6)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:9998,fontFamily:F,direction:lang==='ar'?'rtl':'ltr'}}>
<div onClick={e=>e.stopPropagation()} style={{background:'var(--sf)',borderRadius:16,width:'min(520px,92vw)',maxHeight:'88vh',display:'flex',flexDirection:'column',overflow:'hidden',boxShadow:'0 20px 60px rgba(0,0,0,.5)',border:'1px solid rgba(201,168,76,.12)'}}>
<div style={{padding:'20px 22px 14px',borderBottom:'1px solid rgba(201,168,76,.12)',display:'flex',alignItems:'center',gap:12}}>
<div style={{width:44,height:44,borderRadius:'50%',background:'rgba(201,168,76,.1)',border:'1.5px solid rgba(201,168,76,.15)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:18,fontWeight:800,color:C.gold,flexShrink:0}}>{user?.name_ar?.[0]||'م'}</div>
<div style={{flex:1}}><div style={{fontSize:14,fontWeight:800,color:'var(--tx)'}}>{user?.name_ar}</div><div style={{fontSize:10,color:'rgba(201,168,76,.5)',marginTop:2}}>{user?.roles?.name_ar||(lang==='ar'?'المدير العام':'General Manager')}</div></div>
<button onClick={()=>setShowProfile(false)} style={{width:30,height:30,borderRadius:8,background:'rgba(255,255,255,.07)',border:'1px solid rgba(255,255,255,.1)',color:'var(--tx3)',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}><svg width="12" height="12" viewBox="0 0 24 24" fill="none"><path d="M18 6L6 18M6 6l12 12" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg></button>
</div>
<div style={{display:'flex',borderBottom:'1px solid var(--bd)',padding:'0 22px'}}>
{[{k:'info',ar:'البيانات الأساسية',en:'Basic Info'},{k:'bank',ar:'الحساب البنكي',en:'Bank Account'}].map(t=>
<div key={t.k} onClick={()=>setProfileTab(t.k)} style={{padding:'10px 16px',fontSize:11,fontWeight:profileTab===t.k?700:500,color:profileTab===t.k?C.gold:'rgba(255,255,255,.4)',borderBottom:profileTab===t.k?'2px solid '+C.gold:'2px solid transparent',cursor:'pointer',transition:'.2s'}}>{lang==='ar'?t.ar:t.en}</div>)}
</div>
<div className='dash-content' style={{flex:1,overflowY:'auto',padding:'16px 22px 20px'}}>
...
{profileTab==='bank'&&profileBank&&<div style={{display:'flex',flexDirection:'column',gap:10}}>
...
{!profileBank.bank_name&&!profileBank.iban&&!profileBank.account_number&&<div style={{textAlign:'center',padding:'16px 0',fontSize:10,color:'var(--tx5)'}}>{lang==='ar'?'لا يوجد حساب بنكي — أضف بياناتك':'No bank account — add your details'}</div>}
</div>}
</div>
<div style={{padding:'12px 22px 16px',borderTop:'1px solid rgba(201,168,76,.12)',display:'flex',gap:10,flexDirection:'row-reverse'}}>
...
if(profileTab==='bank'&&profileBank){
...
const[pop,setPop]=useState(false);const[form,setForm]=useState({});const[saving,setSaving]=useState(false);const[viewRow,setViewRow]=useState(null)
...
const openAdd=()=>{setForm({worker_id:'',facility_id:'',transfer_type:'sponsorship',visa_cost:'2000',iqama_cost:'650',work_permit_cost:'500',insurance_cost:'800',ticket_cost:'0',gosi_cost:'300',government_fees:'200',other_costs:'0',other_costs_desc:'',transfer_fee:'500',client_charge:'',status:'draft',new_employer_name:'',notes:''});setPop(true)}
...
{pop&&<div onClick={()=>setPop(false)} style={{position:'fixed',inset:0,background:'rgba(14,14,14,.8)',backdropFilter:'blur(8px)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000,padding:16}}><div onClick={e=>e.stopPropagation()} style={{background:'var(--sf)',borderRadius:16,width:'min(700px,95vw)',maxHeight:'92vh',display:'flex',flexDirection:'column',overflow:'hidden',boxShadow:'0 20px 48px rgba(0,0,0,.5)',border:'1px solid rgba(201,168,76,.12)'}}>
<div style={{height:3,background:'linear-gradient(90deg,transparent,'+C.gold+' 30%,#dcc06e 50%,'+C.gold+' 70%,transparent)'}}/><div style={{background:'var(--bg)',padding:'16px 22px',display:'flex',justifyContent:'space-between',alignItems:'center'}}><div style={{fontSize:15,fontWeight:700,color:'var(--tx)'}}>{form._id?T('تعديل الحسبة','Edit'):T('حسبة تنازل جديدة','New Transfer Calc')}</div><button onClick={()=>setPop(false)} style={{width:28,height:28,borderRadius:8,background:'rgba(255,255,255,.06)',border:'1px solid rgba(255,255,255,.1)',color:'var(--tx3)',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}>✕</button></div>
<div style={{flex:1,overflowY:'auto',padding:'18px 22px'}}>
<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:16}}>
<div><div style={{fontSize:10,fontWeight:600,color:'var(--tx4)',marginBottom:4}}>{T('العامل','Worker')}</div><select value={form.worker_id||''} onChange={e=>setForm(p=>({...p,worker_id:e.target.value}))} style={fS}><option value="">—</option>{workers.map(w=><option key={w.id} value={w.id}>{w.name_ar}</option>)}</select></div>
<div><div style={{fontSize:10,fontWeight:600,color:'var(--tx4)',marginBottom:4}}>{T('المنشأة','Facility')}</div><select value={form.facility_id||''} onChange={e=>setForm(p=>({...p,facility_id:e.target.value}))} style={fS}><option value="">—</option>{facilities.map(f=><option key={f.id} value={f.id}>{f.name_ar}</option>)}</select></div>
<div><div style={{fontSize:10,fontWeight:600,color:'var(--tx4)',marginBottom:4}}>{T('نوع النقل','Type')}</div><select value={form.transfer_type||''} onChange={e=>setForm(p=>({...p,transfer_type:e.target.value}))} style={fS}><option value="sponsorship">{T('نقل كفالة','Sponsorship')}</option><option value="final_exit">{T('خروج نهائي','Final Exit')}</option></select></div>
<div><div style={{fontSize:10,fontWeight:600,color:'var(--tx4)',marginBottom:4}}>{T('الحالة','Status')}</div><select value={form.status||''} onChange={e=>setForm(p=>({...p,status:e.target.value}))} style={fS}><option value="draft">{T('مسودة','Draft')}</option><option value="pending">{T('معلّقة','Pending')}</option><option value="approved">{T('مقبولة','Approved')}</option><option value="completed">{T('مكتملة','Done')}</option></select></div>
<div style={{gridColumn:'1/-1'}}><div style={{fontSize:10,fontWeight:600,color:'var(--tx4)',marginBottom:4}}>{T('صاحب العمل الجديد','New Employer')}</div><input value={form.new_employer_name||''} onChange={e=>setForm(p=>({...p,new_employer_name:e.target.value}))} style={{...fS,textAlign:'right'}}/></div>
</div>
<div style={{fontSize:12,fontWeight:700,color:C.gold,marginBottom:10,paddingBottom:6,borderBottom:'1px solid rgba(201,168,76,.1)'}}>{T('التكاليف','Costs')}</div>
<div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8,marginBottom:16}}>
{[['visa_cost',T('التأشيرة','Visa')],['iqama_cost',T('الإقامة','Iqama')],['work_permit_cost',T('رخصة العمل','Permit')],['insurance_cost',T('التأمين','Insurance')],['ticket_cost',T('التذكرة','Ticket')],['gosi_cost',T('التأمينات','GOSI')],['government_fees',T('رسوم حكومية','Gov Fees')],['other_costs',T('أخرى','Other')],['transfer_fee',T('رسوم النقل','Transfer Fee')]].map(([k,l])=><div key={k}><div style={{fontSize:9,color:'var(--tx4)',marginBottom:3}}>{l}</div><input value={form[k]||''} onChange={e=>setForm(p=>({...p,[k]:e.target.value}))} style={{...fS,height:36,fontSize:11}} type="number"/></div>)}
</div>
<div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:10,padding:'14px',borderRadius:12,background:'rgba(201,168,76,.04)',border:'1px solid rgba(201,168,76,.1)',marginBottom:12}}>
<div style={{textAlign:'center'}}><div style={{fontSize:9,color:C.red,marginBottom:4}}>{T('إجمالي التكلفة','Total Cost')}</div><div style={{fontSize:22,fontWeight:900,color:C.red}}>{nm(totalCost())}</div></div>
<div><div style={{fontSize:9,color:C.gold,marginBottom:4,textAlign:'center'}}>{T('المطلوب من العميل','Client Charge')}</div><input value={form.client_charge||''} onChange={e=>setForm(p=>({...p,client_charge:e.target.value}))} style={{...fS,height:38,fontSize:16,fontWeight:800,color:C.gold,background:'rgba(201,168,76,.08)',border:'1.5px solid rgba(201,168,76,.2)'}} type="number"/></div>
<div style={{textAlign:'center'}}><div style={{fontSize:9,color:profit()>=0?C.ok:C.red,marginBottom:4}}>{T('الربح','Profit')}</div><div style={{fontSize:22,fontWeight:900,color:profit()>=0?C.ok:C.red}}>{nm(profit())}</div></div>
</div>
<div><div style={{fontSize:10,fontWeight:600,color:'var(--tx4)',marginBottom:4}}>{T('ملاحظات','Notes')}</div><textarea value={form.notes||''} onChange={e=>setForm(p=>({...p,notes:e.target.value}))} rows={2} style={{...fS,height:'auto',padding:10,resize:'vertical',textAlign:'right'}}/></div>
</div>
...
<div style={{flex:1,padding:'12px 16px',borderRadius:12,background:'rgba(39,160,70,.06)',border:'1px solid rgba(39,160,70,.1)',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
<span style={{fontSize:10,color:C.ok}}>{T('إيداعات','Deposits')}</span><span style={{fontSize:18,fontWeight:800,color:C.ok}}>{nm(totalDeposits)}</span></div>
<div style={{flex:1,padding:'12px 16px',borderRadius:12,background:'rgba(192,57,43,.06)',border:'1px solid rgba(192,57,43,.1)',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
<span style={{fontSize:10,color:C.red}}>{T('مسحوبات','Withdrawals')}</span><span style={{fontSize:18,fontWeight:800,color:C.red}}>{nm(totalWithdrawals)}</span></div>
<div style={{flex:1,padding:'12px 16px',borderRadius:12,background:'rgba(201,168,76,.06)',border:'1px solid rgba(201,168,76,.1)',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
<span style={{fontSize:10,color:C.gold}}>{T('الصافي','Net')}</span><span style={{fontSize:18,fontWeight:800,color:C.gold}}>{nm(totalDeposits-totalWithdrawals)}</span></div>
</div>
{/* Filter cards */}
<div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(min(160px,100%),1fr))',gap:10,marginBottom:16}}>
{[['all',T('الكل','All'),data.length,'rgba(255,255,255,.4)'],['pending',T('بانتظار','Pending'),data.filter(r=>r.match_status==='pending').length,C.gold],['matched',T('مطابقة','Matched'),data.filter(r=>r.match_status==='matched').length,C.ok],['unmatched',T('غير مطابقة','Unmatched'),data.filter(r=>r.match_status==='unmatched').length,C.red]].map(([k,l,n,c])=><div key={k} onClick={()=>setFilter(k)} style={{padding:'14px',borderRadius:12,background:filter===k?c+'12':'rgba(255,255,255,.02)',border:'1.5px solid '+(filter===k?c+'30':'rgba(255,255,255,.06)'),cursor:'pointer',textAlign:'center'}}><div style={{fontSize:24,fontWeight:900,color:c,marginBottom:4}}>{n}</div><div style={{fontSize:10,fontWeight:600,color:c,opacity:.7}}>{l}</div></div>)}
</div>
<div style={{display:'flex',flexDirection:'column',gap:8}}>
...
<div style={{flex:1}}>
<div style={{display:'flex',alignItems:'center',gap:8,marginBottom:3}}><span style={{fontSize:13,fontWeight:700,color:'var(--tx2)'}}>{r.description||T('عملية','Transaction')}</span><span style={{fontSize:10,fontWeight:600,padding:'2px 8px',borderRadius:5,background:sc+'15',color:sc}}>{r.match_status}</span></div>
...
<tbody>{data.map(r=><tr key={r.id} onClick={()=>{setF({...r});setPop(r.id)}} style={{cursor:'pointer',borderBottom:'1px solid var(--bd2)'}}><td style={{padding:'10px 12px',fontSize:12,color:'var(--tx4)'}}>{r.date}</td><td style={{padding:'10px 12px',fontSize:12,fontWeight:600}}>{cats[r.category]||r.category}</td><td style={{padding:'10px 12px',fontSize:12,color:'var(--tx2)'}}>{r.description||'—'}</td><td style={{padding:'10px 12px',fontSize:12,color:'var(--tx4)'}}>{r.vendor_name||'—'}</td><td style={{padding:'10px 12px',fontSize:13,fontWeight:700,direction:'ltr',textAlign:'left'}}>{Number(r.amount).toLocaleString()}</td></tr>)}</tbody>
</table></div>}
{pop&&<div onClick={()=>setPop(null)} style={{position:'fixed',inset:0,background:'rgba(14,14,14,.8)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000,padding:16}}>
