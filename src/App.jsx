...
import { canViewPage, can as canPerm, tabOffices, cardVisible, canCardBtn, fieldVisible, fieldEditable, modalAllowed, mergeRoleVis, isInvoiceIssuer } from './lib/permissions.js'
...
<div className='topbar-uitext' style={{display:'flex',flexDirection:'column',alignItems:'flex-end',lineHeight:1.3,gap:5,whiteSpace:'nowrap'}}>
{nm&&<span style={{fontSize:12,fontWeight:600,color:'var(--hdtx)'}}>{nm}</span>}
{officeText&&<span title={officeTitle} style={{fontSize:10,fontWeight:600,color:C.gold,direction:'ltr'}}>{officeText}</span>}
</div>
<div style={{width:32,height:32,borderRadius:'50%',flexShrink:0,overflow:'hidden',display:'flex',alignItems:'center',justifyContent:'center',background:avatarUrl?'transparent':'linear-gradient(135deg,rgba(176,125,0,.92),rgba(176,125,0,.5))',color:'#fff',fontSize:13,fontWeight:700,fontFamily:F}}>{avatarUrl?<img src={avatarUrl} alt="" style={{width:'100%',height:'100%',objectFit:'cover',display:'block'}}/>:initial}</div>
</div>;
})()}
...
const refetchTc=async()=>{let tcQ=sb.from('transfer_calculation').select(TC_SELECT).is('deleted_at',null).order('created_at',{ascending:false});if(tcOrScope)tcQ=tcQ.or(tcOrScope);const res=await Promise.all([tcQ,sb.from('branches').select('id,code:branch_code').is('deleted_at',null).order('branch_code'),sb.from('nationalities').select('id,name_ar,name_en,code,flag_url').order('name_ar'),sb.from('users').select(USER_SELECT).is('deleted_at',null)]);swrSet(tcCacheKey,res);applyTcBundle(res)}
...
<button key={o.k} onClick={()=>printTransferDoc(dr,o.k)} title={T('طباعة بـ ','Print in ')+o.l} style={{height:40,padding:'0 10px',borderRadius:10,background:'rgba(176,125,0,.06)',border:'1px solid rgba(176,125,0,.22)',color:C.gold,cursor:'pointer',display:'inline-flex',alignItems:'center',justifyContent:'center',gap:8,fontFamily:F,fontSize:12,fontWeight:600,transition:'.15s'}} onMouseEnter={e=>{e.currentTarget.style.background='rgba(176,125,0,.14)';e.currentTarget.style.borderColor='rgba(176,125,0,.45)'}} onMouseLeave={e=>{e.currentTarget.style.background='rgba(176,125,0,.06)';e.currentTarget.style.borderColor='rgba(176,125,0,.22)'}}>
