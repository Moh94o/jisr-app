...
{id:'pricing_hub',l:T('الحسبة','Calc'),i:'calc'},
...
<div className='mob-hamburger' onClick={()=>setSideOpen(!sideOpen)} onMouseEnter={()=>setHbHover(true)} onMouseLeave={()=>setHbHover(false)} style={{display:'none',width:40,height:40,borderRadius:10,background:sideOpen?'rgba(212,160,23,.12)':(hbHover?'rgba(212,160,23,.07)':'transparent'),flexDirection:'column',alignItems:'center',justifyContent:'center',gap:5,cursor:'pointer',flexShrink:0,transition:'.18s'}}><span style={{width:22,height:2.2,borderRadius:2,background:'linear-gradient(90deg,#F0CB6A,#D4A017)',transition:'.22s'}}/><span style={{width:22,height:2.2,borderRadius:2,background:'linear-gradient(90deg,#F0CB6A,#D4A017)',transition:'.22s'}}/><span style={{width:22,height:2.2,borderRadius:2,background:'linear-gradient(90deg,#F0CB6A,#D4A017)',transition:'.22s'}}/></div>
...
{(()=>{const d=new Date();const pad=n=>String(n).padStart(2,'0');const greg=d.getFullYear()+'-'+pad(d.getMonth()+1)+'-'+pad(d.getDate());const day=d.toLocaleDateString(lang==='ar'?'ar-SA':'en-US',{weekday:'long'});let hijri='';try{const hp=new Intl.DateTimeFormat('en-US-u-ca-islamic-umalqura',{day:'numeric',month:'numeric',year:'numeric'}).formatToParts(d);const hv=t=>{const p=hp.find(x=>x.type===t);return p?p.value:''};hijri=hv('year')+'-'+pad(+hv('month'))+'-'+pad(+hv('day'))}catch{}return<div style={{display:'flex',flexDirection:'column',alignItems:'flex-end',lineHeight:1.3,gap:3,fontFamily:"'Reem Kufi','Cairo',sans-serif"}}><span style={{fontSize:9.5,fontWeight:500,color:'rgba(255,255,255,.42)',letterSpacing:'.5px',direction:'ltr',alignSelf:'center'}}>{hijri}</span><div style={{display:'flex',alignItems:'baseline',gap:7}}><span style={{fontSize:11.5,fontWeight:700,color:C.gold}}>{day}</span><span style={{fontSize:12,fontWeight:600,color:C.gold,letterSpacing:'.5px',direction:'ltr',opacity:.9}}>{greg}</span></div></div>})()}
<AnalogClock size={32}/>
</div>
...
const billedMosV=(()=>{let billed=renMoV;const exp=iqExpG?new Date(iqExpG):null;if(exp&&!isNaN(exp)){const ref=dateVal?new Date(dateVal):new Date();ref.setHours(0,0,0,0);exp.setHours(0,0,0,0);if(exp<ref){const end=new Date(ref);end.setMonth(end.getMonth()+renMoV);let mm=(end.getFullYear()-exp.getFullYear())*12+(end.getMonth()-exp.getMonth());let d=end.getDate()-exp.getDate();if(d<0){mm-=1;d+=new Date(end.getFullYear(),end.getMonth(),0).getDate()}billed=d>0?mm+1:mm}}return billed})()
...
<div style={{flex:1,minWidth:0}}>
<div style={{fontSize:24,fontWeight:600,color:'rgba(255,255,255,.93)',letterSpacing:'-.3px',lineHeight:1.2}}>{T('حسبة نقل الكفالات','Transfer Calculator')}</div>
...
{label:T('المهنة الحالية','Current Occupation'),value:tc.occupation_name_ar||mm.occupation},
...
{gridCard(T('النقل','Transfer'),null,[
{label:T('مدة التجديد','Renewal Period'),value:Number(tc.renewal_months||0)>0?(tc.renewal_months+' '+T('شهر','mo')):'—',color:Number(tc.renewal_months||0)>0?C.gold:'var(--tx5)'},
...
{(()=>{const ren=Number(tc.renewal_months||0);const renSuffix=ren>0?T(` (${ren} شهر)`,` (${ren} mo)`):'';
...
const billedMos=(()=>{let billed=ren;const exp=tc.iqama_expiry_gregorian?new Date(tc.iqama_expiry_gregorian):null;if(exp&&!isNaN(exp)){const ref=tc.priced_at?new Date(tc.priced_at):new Date();ref.setHours(0,0,0,0);exp.setHours(0,0,0,0);if(exp<ref){const end=new Date(ref);end.setMonth(end.getMonth()+ren);let m=(end.getFullYear()-exp.getFullYear())*12+(end.getMonth()-exp.getMonth());let d=end.getDate()-exp.getDate();if(d<0){m-=1;d+=new Date(end.getFullYear(),end.getMonth(),0).getDate()}billed=d>0?m+1:m}}return billed})();
...
<div style={{margin:'10px 22px 18px',display:'flex',justifyContent:'space-between',alignItems:'center',padding:'14px 18px',background:'#1b1b1b',borderRadius:12,borderRight:'3px solid '+C.gold}}><span style={{color:C.gold,fontWeight:700,fontSize:14.5}}>{T('الإجمالي النهائي','Final Total')}</span><span style={{color:C.gold,fontWeight:800,fontSize:24,fontVariantNumeric:'tabular-nums'}}>{nm(totalV)} <span style={{fontSize:12,fontWeight:600}}>{T('ريال','SAR')}</span></span></div>
...
<div key={'ms-'+i} style={{display:'flex',alignItems:'center',gap:10,margin:'4px 0',direction:'rtl'}}>
...
{(()=>{const subtotalV=Number(tc.subtotal||0);const pricerNm=dr.priced_user?(lang==='en'?(dr.priced_user.name_en||dr.priced_user.name_ar):dr.priced_user.name_ar):null;const twoNames=(pricerNm||'').trim().split(/\s+/).filter(Boolean).slice(0,2).join(' ');
const Pill=({color,label,value})=><div style={{position:'relative',padding:'12px 14px',borderRadius:12,background:'rgba(0,0,0,.18)',border:'1px solid rgba(255,255,255,.05)',overflow:'hidden'}}><div style={{position:'absolute',top:0,bottom:0,insetInlineStart:0,width:4,background:color}}/><div style={{fontSize:12,color:'var(--tx3)',fontWeight:600,marginBottom:5}}>{label}</div><div style={{display:'flex',alignItems:'baseline',gap:5,direction:'rtl'}}><span style={{fontSize:19,fontWeight:800,color,fontVariantNumeric:'tabular-nums',letterSpacing:'-.5px',direction:'ltr',unicodeBidi:'isolate'}}>{value}</span><span style={{fontSize:11,fontWeight:700,color,opacity:.72}}>{T('ريال','SAR')}</span></div></div>;
...
<Pill color={C.gold} label={T('الرسوم المكتبية','Office Fee')} value={nm(Math.max(0,officeFee-manualDisc))}/>
<Pill color={'var(--tx2)'} label={T('الرسوم الحكومية','Government Fees')} value={nm(Math.max(0,subtotalV-officeFee-absher))}/>
</div>
...
<YesNo label={T('تغيير المهنة','Change Occupation')} value={f.change_profession} onChange={v=>setF('change_profession',v)}/>
...
const _kcfg=getKafalaPricingConfig();const _discEnabled=_kcfg.kafalaOfficeDiscountEnabled!==false;const _floor=Math.round((parseFloat(_kcfg.officeDailyRate)||0)*((Number(f._renewalMonths||0))*30));const _maxDisc=_discEnabled?Math.max(0,Number(f._officeFee||0)-_floor):0;const _appliedDisc=Math.min(Math.max(0,Math.round(Number(f.manual_discount||0))),_maxDisc);const _capped=Math.round(Number(f.manual_discount||0))>_maxDisc;
...
return <FKModal open onClose={()=>{if(approveSaving)return;setApproveForm(null);if(approveSaved){setApproveSaved(false);setDetailsRow(null)}}} width={560} variant="edit"
 ...
 <div style={GRID}>
 {_discEnabled?<><CurrencyField label={T('خصم المكتب (اختياري · مبلغ)','Office Discount (optional · SAR)')} value={f.manual_discount||''} onChange={v=>setF('manual_discount',v)}/>
 <div style={{gridColumn:'1/-1',fontSize:11,color:'var(--tx4)',fontWeight:600}}>{T(`الحد الأقصى للخصم ${nm(_maxDisc)} ريال · الأرضية ${nm(_floor)} (سعر اليوم × ${Number(f._renewalMonths||0)} شهر × 30)`,`Max discount ${nm(_maxDisc)} SAR · floor ${nm(_floor)}`)}</div>
 {_capped&&<div style={{gridColumn:'1/-1',fontSize:11,fontWeight:700,color:'#e0564f',background:'rgba(192,57,43,.08)',border:'1px solid rgba(192,57,43,.3)',borderRadius:7,padding:'6px 9px'}}>{T(`تجاوزت الأرضية — سيُطبَّق الحد الأقصى ${nm(_maxDisc)} ريال فقط`,`Below floor — only ${nm(_maxDisc)} SAR will apply`)}</div>}</>:<div style={{gridColumn:'1/-1',fontSize:11.5,color:'var(--tx4)',fontWeight:600,padding:'10px 12px',borderRadius:9,background:'rgba(255,255,255,.03)',border:'1px solid rgba(255,255,255,.07)'}}>{T('خصم المكتب غير مُتاح (معطّل من إعدادات الخدمات).','Office discount is disabled in service settings.')}</div>}
 <div style={{gridColumn:'1/-1',display:'flex',alignItems:'center',gap:10,padding:'12px 14px',borderRadius:9,background:'rgba(212,160,23,.08)',border:'1px solid rgba(212,160,23,.3)',minHeight:44}}><span style={{fontSize:13,fontWeight:600,color:'rgba(255,255,255,.7)'}}>{T('الإجمالي بعد الخصم','Total after discount')}</span><span style={{flex:1}}/><span style={{fontSize:16,fontWeight:700,color:FKC.gold,direction:'rtl',fontVariantNumeric:'tabular-nums'}}>{nm(Math.max(0,f._subtotal-_appliedDisc))+' '+T('ريال','SAR')}</span></div>
 </div>
 </ModalSection></>)}
 ]}/>
})()}
