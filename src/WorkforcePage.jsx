...
const SectionHdr=({t})=><div style={{fontSize:12,fontWeight:700,color:C.gold,marginBottom:10,paddingBottom:6,borderBottom:'1px solid rgba(212,160,23,.12)'}}>{t}</div>
...
<div><div style={{fontSize:13,fontWeight:700,color:C.gold}}>إجمالي العمال</div><div style={{fontSize:10,color:'rgba(212,160,23,.5)',marginTop:2}}>مسجل في النظام</div></div>
...
{SB}
<button onClick={()=>setShowAdv(!showAdv)} style={{height:38,padding:'0 14px',borderRadius:10,border:'1px solid '+(showAdv?'rgba(212,160,23,.2)':'rgba(255,255,255,.08)'),background:showAdv?'rgba(212,160,23,.06)':'rgba(255,255,255,.04)',color:showAdv?C.gold:'var(--tx4)',fontFamily:F,fontSize:10,fontWeight:600,cursor:'pointer',display:'flex',alignItems:'center',gap:5,whiteSpace:'nowrap'}}>
...
{showAdv&&<div style={{marginBottom:14,padding:'14px 16px',background:'rgba(255,255,255,.02)',borderRadius:12,border:'1px solid rgba(212,160,23,.1)'}}>
...
return<div key={r.id} onClick={()=>{setViewRow(r);setViewTab('basic')}} style={{background:'var(--bg)',border:'1px solid '+borderClr,borderRadius:14,overflow:'hidden',cursor:'pointer',transition:'.15s'}} onMouseEnter={e=>e.currentTarget.style.borderColor='rgba(212,160,23,.2)'} onMouseLeave={e=>e.currentTarget.style.borderColor=borderClr}>
{/* Header */}
...
{occMap[r.occupation_id]&&<span style={{fontSize:8,padding:'2px 7px',borderRadius:4,background:'rgba(212,160,23,.06)',border:'1px solid rgba(212,160,23,.08)',color:'rgba(212,160,23,.7)',fontWeight:600}}>{occMap[r.occupation_id]}</span>}
...
<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#D4A017" strokeWidth="1.8"><path d="M17 3a2.83 2.83 0 114 4L7.5 20.5 2 22l1.5-5.5L17 3z"/></svg>تعديل</div>
...
{r.outside_kingdom&&r.worker_status==='active'&&<div style={{padding:'6px 12px',background:'rgba(212,160,23,.04)',borderTop:'1px solid rgba(212,160,23,.1)',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
...
<span style={{width:30,height:30,borderRadius:7,border:'1px solid rgba(212,160,23,.3)',background:'rgba(212,160,23,.15)',color:'#D4A017',fontFamily:F,fontSize:12,fontWeight:800,display:'flex',alignItems:'center',justifyContent:'center'}}>{page+1}</span>
...
<div onClick={e=>e.stopPropagation()} style={{background:'var(--sf)',borderRadius:16,width:'min(920px,95vw)',height:'min(650px,88vh)',display:'flex',flexDirection:'column',overflow:'hidden',boxShadow:'0 20px 48px rgba(0,0,0,.5)',border:'1px solid rgba(212,160,23,.15)'}}>
{/* Header */}
...
<div style={{display:'flex',gap:12,alignItems:'center'}}><div style={{width:48,height:48,borderRadius:14,background:gC+'15',border:'1.5px solid '+gC+'25',display:'flex',alignItems:'center',justifyContent:'center',fontSize:20,fontWeight:800,color:gC}}>{(viewRow.name_ar||'ع')[0]}</div><div><div style={{display:'flex',alignItems:'center',gap:10,marginBottom:4}}><div style={{fontSize:18,fontWeight:800,color:'var(--tx)'}}>{viewRow.name_ar}</div><Badge v={viewRow.worker_status}/></div>{viewRow.name_en&&<div style={{fontSize:12,color:'var(--tx4)',direction:'ltr'}}>{viewRow.name_en}</div>}{fac&&<div style={{fontSize:10,color:'rgba(212,160,23,.5)',marginTop:2}}>{fac.name_ar}</div>}</div></div>
<div style={{display:'flex',gap:6}}><button onClick={()=>{setViewRow(null);openEdit(viewRow)}} style={{height:32,padding:'0 16px',borderRadius:8,border:'1px solid rgba(212,160,23,.2)',background:'rgba(212,160,23,.08)',color:C.gold,fontFamily:F,fontSize:11,fontWeight:700,cursor:'pointer'}}>تعديل</button><button onClick={()=>setViewRow(null)} style={{width:32,height:32,borderRadius:8,background:'rgba(255,255,255,.07)',border:'1px solid rgba(255,255,255,.1)',color:'var(--tx3)',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}>{'\u2715'}</button></div></div>
{/* Body */}
<div style={{flex:1,display:'flex',overflow:'hidden'}}>
{/* Sidebar tabs */}
<div style={{width:180,background:'var(--bg)',borderLeft:'1px solid rgba(255,255,255,.04)',padding:'12px 8px',overflowY:'auto',flexShrink:0,scrollbarWidth:'none'}}>{vt.map(t=><div key={t.id} onClick={()=>setViewTab(t.id)} style={{padding:'10px 12px',borderRadius:8,marginBottom:3,fontSize:11,fontWeight:viewTab===t.id?700:500,color:viewTab===t.id?C.gold:'rgba(255,255,255,.38)',background:viewTab===t.id?'rgba(212,160,23,.08)':'transparent',border:viewTab===t.id?'1px solid rgba(212,160,23,.12)':'1px solid transparent',cursor:'pointer',display:'flex',justifyContent:'space-between',alignItems:'center',transition:'.15s'}}><span>{t.l}</span>{t.n!==undefined&&<span style={{fontSize:9,fontWeight:700,color:viewTab===t.id?C.gold:'rgba(255,255,255,.2)',background:viewTab===t.id?'rgba(212,160,23,.15)':'rgba(255,255,255,.04)',padding:'1px 6px',borderRadius:4,minWidth:18,textAlign:'center'}}>{t.n}</span>}</div>)}</div>
{/* Content */}
