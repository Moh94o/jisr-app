      ...
      {loading ? (
        ...
        <>
          <style>{`
            ...
            .wf-tbl thead th{position:sticky;top:0;background:var(--card-bg);color:var(--tx);font-size:14px;font-weight:600;text-align:center;padding:14px 4px 11px;box-shadow:inset 0 -2px 0 rgba(176,125,0,.55);white-space:nowrap;z-index:2;letter-spacing:.2px}
...
function WorkerDetail({ worker: w, facility: f, sb, toast, T, isAr, onBack, onEdit, onDelete, onTransfer, canEdit, canDelete, user, attKey }) {
  ...
  const FileTile = ({ label, att }) => (
    <div style={{ background: 'rgba(0,0,0,.25)', border: att ? '1px solid rgba(176,125,0,.25)' : '1px dashed rgba(255,255,255,.12)', borderRadius: 12, padding: '14px 10px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, textAlign: 'center', minWidth: 0 }}>
      {att ? (
            ...
            {chi.phase === 'captcha' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                ...
                <input value={chi.captchaInput} onChange={e => setChi(c => ({ ...c, captchaInput: e.target.value.replace(/\s/g, '').slice(0, 8) }))}
                  ...
                  style={{ height: 48, width: 240, alignSelf: 'center', padding: '0 18px', border: '1px solid var(--bd)', borderRadius: 12, fontFamily: F, fontSize: 20, fontWeight: 600, color: 'var(--tx)', outline: 'none', background: 'rgba(0,0,0,.25)', textAlign: 'center', letterSpacing: '8px', direction: 'ltr' }} />
