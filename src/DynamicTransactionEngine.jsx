...
const GoldBar = () => <div style={{ height:3, background:'linear-gradient(90deg,transparent,#c9a84c 30%,#dcc06e 50%,#c9a84c 70%,transparent)' }}/>
    ...
    <div style={{flex:1}}>
      ...
      <div style={{display:'flex',justifyContent:'space-between',marginTop:14,gap:8}}>
        ...
        <div style={{display:'flex',gap:6}}>
          <button onClick={()=>saveStepFields(step,'save')} disabled={saving} style={{height:38,padding:'0 16px',borderRadius:10,border:'1px solid rgba(201,168,76,.2)',background:'rgba(201,168,76,.08)',color:C.gold,fontFamily:F,fontSize:11,fontWeight:700,cursor:'pointer',opacity:saving?.5:1}}>{saving?'...':T('💾 حفظ','💾 Save')}</button>
  ...
  return<div>
    ...
    <div style={{display:'flex',gap:8,marginBottom:12,flexWrap:'wrap',alignItems:'center'}}>
      ...
      <div style={{position:'relative'}}>
        <div onClick={()=>setSvcDropOpen(!svcDropOpen)} style={{height:36,padding:'0 12px',borderRadius:8,border:activeTab!=='all'?'1.5px solid rgba(201,168,76,.3)':'1.5px solid rgba(255,255,255,.08)',background:activeTab!=='all'?'rgba(201,168,76,.08)':'rgba(255,255,255,.04)',display:'flex',alignItems:'center',gap:6,cursor:'pointer',minWidth:130}}>
      ...
      return<div onClick={()=>setViewRow(null)} style={{position:'fixed',inset:0,background:'rgba(14,14,14,.75)',backdropFilter:'blur(6px)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:999,padding:16}}><div onClick={e=>e.stopPropagation()} style={{background:'var(--sf)',borderRadius:16,width:'min(960px,96vw)',height:'min(620px,92vh)',display:'flex',flexDirection:'column',overflow:'hidden',boxShadow:'0 20px 48px rgba(0,0,0,.5)',border:'1px solid rgba(201,168,76,.15)'}}>
        ...
        <div style={{flex:1,display:'flex',overflow:'hidden'}}>
          ...
          <div style={{width:140,background:'var(--bg)',borderLeft:isAr?'none':'1px solid rgba(255,255,255,.04)',borderRight:isAr?'1px solid rgba(255,255,255,.04)':'none',padding:'12px 8px',flexShrink:0}}>
            {vtabs.map(t=><div key={t.id} onClick={()=>setViewTab(t.id)} style={{padding:'10px 10px',borderRadius:8,marginBottom:3,fontSize:10,fontWeight:viewTab===t.id?700:500,color:viewTab===t.id?C.gold:'rgba(255,255,255,.38)',background:viewTab===t.id?'rgba(201,168,76,.08)':'transparent',cursor:'pointer',display:'flex',justifyContent:'space-between',alignItems:'center',gap:4}}>
              <span style={{display:'flex',alignItems:'center',gap:4}}><span style={{fontSize:12}}>{t.icon}</span>{t.l}</span>
              {t.n!==undefined&&t.n>0&&<span style={{fontSize:9,fontWeight:700,color:viewTab===t.id?C.gold:'rgba(255,255,255,.2)',background:viewTab===t.id?'rgba(201,168,76,.15)':'rgba(255,255,255,.04)',padding:'1px 6px',borderRadius:4}}>{t.n}</span>}
            </div>)}
          </div>
          ...
          <div className="dash-content" style={{flex:1,overflowY:'auto',padding:'18px 22px'}}>
            ...
            {viewTab==='info'&&<div>
              ...
              {(viewRow.client_name||viewRow.facility_name||viewRow.worker_name)&&<div style={{marginBottom:16}}>
                ...
                <div style={{display:'flex',gap:10,flexWrap:'wrap'}}>
                  {viewRow.client_name&&<div style={{background:'rgba(201,168,76,.04)',border:'1px solid rgba(201,168,76,.1)',borderRadius:10,padding:'10px 14px',flex:1,minWidth:140}}>
            ...
            {viewTab==='finance'&&<div style={{display:'flex',flexDirection:'column',gap:14}}>
              {/* Invoice summary */}
              {viewRow.invoice_number&&<div style={{background:'rgba(201,168,76,.04)',border:'1px solid rgba(201,168,76,.1)',borderRadius:12,padding:14,display:'flex',alignItems:'center',gap:12}}>
